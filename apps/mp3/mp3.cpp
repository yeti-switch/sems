...
static int MP3_2_Pcm16(unsigned char *out_buf, unsigned char *in_buf, unsigned int size, unsigned int channels,
                       unsigned int rate, long h_codec);

static int  MP3_ModuleLoad(void);
static void MP3_ModuleDestroy(void);

static int          Pcm16_2_MP3(unsigned char *out_buf, unsigned char *in_buf, unsigned int size, unsigned int channels,
                                unsigned int rate, long h_codec);
...
BEGIN_CODECS
CODEC(CODEC_MP3, Pcm16_2_MP3, MP3_2_Pcm16, (amci_plc_t)0, (amci_codec_init_t)MP3_create,
      (amci_codec_destroy_t)MP3_destroy, mp3_bytes2samples, mp3_samples2bytes)
END_CODECS
...
int MP3_ModuleLoad(void)
{
    ...
    if ((res = mpg123_init()) != MPG123_OK) {
        ERROR("initializing mpg123 failed: %d", res);
        return -1;
    }
#endif
...
long MP3_create(const char *format_parameters, amci_codec_fmt_info_t *format_description)
{
    ...
    DBG("MP3: creating lame %s", get_lame_version());
                       ...
                       unsigned int rate, long h_codec)
{
    ...
    res         = mpg123_decode(coder_state->mpg123_h, in_buf, size, out_buf, AUDIO_BUFFER_SIZE, &decoded_size);

    if (res == MPG123_NEW_FORMAT) {
        WARN("intermediate mp3 file format change!");
    }
    if (res == MPG123_ERR) {
        ERROR("decoding mp3: '%s'", mpg123_strerror(coder_state->mpg123_h));
        return -1;
    }
...
static int MP3_open(FILE *fp, struct amci_file_desc_t *fmt_desc, int options, long h_codec)
{
    ...
    if (options == AMCI_RDONLY) {
        ...
        if (MPG123_ERR == res) {
            ERROR("mpg123_open_feed returned mpg123 error '%s'", mpg123_strerror(coder_state->mpg123_h));
            return -1;
        }
        ...
        while (res != MPG123_NEW_FORMAT) {
            ...
            res = mpg123_decode(coder_state->mpg123_h, mp_rd_buf, 20, mp_rd_buf, 0, &decoded_size);
            if (res == MPG123_ERR) {
                ERROR("trying to determine MP3 file format: '%s'", mpg123_strerror(coder_state->mpg123_h));
                return -1;
            }
        }

        mpg123_getformat(coder_state->mpg123_h, &coder_state->rate, &coder_state->channels, &coder_state->enc);

        DBG("mpg123: New format: %li Hz, %i channels, encoding value %i", coder_state->rate, coder_state->channels,
            coder_state->enc);
...
static int MP3_close(FILE *fp, struct amci_file_desc_t *fmt_desc, int options, long h_codec, struct amci_codec_t *codec)
{
    int final_samples;
