...
TEST_F(RedisTest, RedisSimpleTest)
{
    ...
    redisContext *ctx     = redis::redisConnectWithTimeout(redis_test::instance()->settings.host.c_str(),
...
class TestRedisConnection : public RedisConnectionPool {
  ...
  public:
    ...
    int init(const string &host, int port)
    {
        ...
        conn    = addConnection("test", host, port);
        if (ret || !conn)
            return -1;
        return 0;
    }
...
TEST_F(RedisTest, RedisConnectionTest)
{
    ...
    conn.init(redis_test::instance()->settings.host.c_str(), redis_test::instance()->settings.port);
    conn.start();
