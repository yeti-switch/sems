...
void DILog::log(int level, pid_t pid, pid_t tid, [[maybe_unused]] const char *func, const char *file, int line,
                const char *msg, int msg_len)
{
    ...
    snprintf(ring_buf[pos], MAX_LINE_LEN, "%s [%u:%u/%s:%d] %s: %.*s\n", ts, /*ctime_r(&tv.tv_sec, ts),*/
             pid, tid, file, line, log_level2str[level], msg_len, msg);
...
{
    cfg_opt_t di_opt[]{ CFG_STR(PARAM_LOG_LEVEL_NAME, 0, CFGF_NODEFAULT), CFG_END() };
