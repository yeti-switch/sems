...
void SctpServerConnection::handle_notification(const sockaddr_storage &from)
{
    ...
    case SCTP_ASSOC_CHANGE:
    {
        ...
        case SCTP_COMM_UP:
            ...
            INFO("associated with %s:%u/%d (%d)", am_inet_ntop(&from).c_str(), am_get_port(&from), sac.sac_assoc_id,
                 fd);
            clients_mutex.lock();
            clients.emplace(sac.sac_assoc_id, client_info(am_inet_ntop(&from), am_get_port(&from)));
        ...
        case SCTP_SHUTDOWN_COMP:
            ...
            clients_mutex.unlock();
            break;
        case SCTP_CANT_STR_ASSOC: str = "CANT START ASSOC"; ERROR("SCTP_CANT_STR_ASSOC, assoc=%u", sac.sac_assoc_id);
        default:                  str = "UNKNOWN";
        } // switch(sac.sac_state)
    ...
    case SCTP_PEER_ADDR_CHANGE:
    {
        ...
        case SCTP_ADDR_UNREACHABLE: str = "ADDRESS UNAVAILABLE"; break;
    ...
    default: ERROR("Unknown notification event type=%xh", sn->sn_header.sn_type);
...
int SctpServerConnection::process(uint32_t events)
{
    ...
    length = sctp_recvmsg(fd, payload, sizeof(payload) - 1, (struct sockaddr *)&from, &fromlen, &sinfo, &flags);
    ...
    DBG("RECV sctp_bus event %d:%s -> %d:%s/%d seq: %ld", r.src_node_id(), r.src_session_id().c_str(), r.dst_node_id(),
        r.dst_session_id().c_str(), sinfo.sinfo_assoc_id, r.sequence());
    ...
    if (!AmSessionContainer::instance()->postEvent(r.dst_session_id(), ev)) {
        DBG("failed to post SctpBusEvent for session: %s", r.dst_session_id().c_str());
    }
