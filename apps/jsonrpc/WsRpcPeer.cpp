...
struct wslay_event_callbacks WsRpcPeer::callbacks = { WsRpcPeer::recv_callback,
...
WsRpcPeer::WsRpcPeer(const std::string &id)
    : JsonrpcNetstringsConnection(id)
    , ws_connected(false)
    , ctx_(0)
{
    Botan::SHA_1 sha;
...
int WsRpcPeer::send_data(char *data, int size)
{
    ...
    wslay_event_msg e_msg = { .opcode = WSLAY_TEXT_FRAME, .msg = (uint8_t *)data, .msg_length = (size_t)size };

    int ret = 0;
    if ((ret = wslay_event_queue_msg(ctx_, &e_msg)) != 0) {
        WARN("connection:%s. wslay_event_queue_msg: %d. restore queue", id.data(), ret);
        return 0;
    }
...
void WsRpcPeer::send_reply(sip_msg *req, int reply_code, const std::string &reason)
{
    ...
    *c++ = LF;
    ...
    if (netstringsBlockingWrite() == REMOVE)
        close();
}
...
int WsRpcPeer::read_data(char *data, int size)
{
    ...
    size_t read_size = size > static_cast<typeof(size)>(ws_resv_buffer.size()) ? ws_resv_buffer.size() : size;
...
void WsRpcPeer::on_msg_recv_callback(wslay_event_context_ptr ctx, const struct wslay_event_on_msg_recv_arg *arg,
                                     void *user_data)
{
...
ssize_t WsRpcPeer::send_callback(wslay_event_context_ptr ctx, const uint8_t *data, size_t len, int flags,
                                 void *user_data)
{
    WsRpcPeer *input = (WsRpcPeer *)user_data;
...
int WsRpcPeer::parse_input(sip_msg *s_msg)
{
    ...
    if (s_msg->upgrade->value.len != websocket_len || lower_cmp(s_msg->upgrade->value.s, websocket, websocket_len)) {
...
void WsRpcPeer::send_request()
{
    ...
    sip_header origin(sip_header::H_ORIGIN, cstring(HTTP_HDR_ORIGIN, SIP_HDR_LEN(HTTP_HDR_ORIGIN)),
                      cstring(hoststr.c_str(), hoststr.size()));
...
{
    if (conn_type == PEER_TCP || ws_connected)
        JsonrpcNetstringsConnection::addMessage(data, len);
    else {
