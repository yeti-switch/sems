...
int JsonRPCServerModule::configure(const std::string &config)
{
    ...
    static cfg_opt_t acl_sec[]{ CFG_STR_LIST(opt_whitelist, 0, CFGF_NODEFAULT), CFG_STR(opt_method, "", CFGF_NODEFAULT),
                                CFG_END() };

    cfg_opt_t listen_sec[] = { CFG_STR(opt_address, DEFAULT_JSONRPC_SERVER_HOST, CFGF_NONE),
                               CFG_INT(opt_port, DEFAULT_JSONRPC_SERVER_PORT, CFGF_NONE), CFG_END() };

    static cfg_opt_t tls_client[] = { CFG_STR_LIST(opt_tls_protocols, 0, CFGF_NODEFAULT),
    ...
    static cfg_opt_t tls_server[] = { CFG_STR_LIST(opt_tls_protocols, 0, CFGF_NODEFAULT),
                                      CFG_STR(opt_tls_certificate, "", CFGF_NONE),
                                      CFG_STR(opt_tls_certificate_key, "", CFGF_NONE),
    ...
    cfg_opt_t tls_sec[] = { CFG_SEC(sec_server_tls, tls_server, CFGF_NONE),
                            CFG_SEC(sec_client_tls, tls_client, CFGF_NONE), CFG_END() };

    cfg_opt_t opt[] = { CFG_SEC(sec_listen, listen_sec, CFGF_NONE),
    ...
    if (cfg_size(cfg, sec_tls)) {
        ...
        if (server_settings.verify_client_certificate && !server_settings.require_client_certificate) {
            ERROR("incorrect server tls configuration: verify client certificate cannot be set, if clients certificate "
                  "is not required");
            return -1;
        }
...
void JsonRPCServerModule::invoke(const string &method, const AmArg &args, AmArg &ret)
{
    if (method == "execRpc") {
...
void JsonRPCServerModule::execRpc(const AmArg &args, AmArg &ret)
{
    ...
    JsonRPCServerLoop::execRpc( // evq_link, notification_link, request_link
        ...
        args.get(4).asInt(),
        // host, port, method
        args.get(5).asCStr(), args.get(6).asInt(), args.get(7).asCStr(), params, udata, ret);
}
