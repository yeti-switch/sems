...
vector<Botan::Certificate_Store *> tls_rpc_conf::trusted_certificate_authorities(const string &type,
...
tls_rpc_conf::cert_chain(const std::vector<std::string>                &cert_key_types,
                         const std::vector<Botan::AlgorithmIdentifier> &cert_signature_schemes, const std::string &type,
...
std::shared_ptr<Botan::Private_Key> tls_rpc_conf::private_key_for(const Botan::X509_Certificate &cert,
                                                                  const string &type, const string &context)
{
    if (key) {
        return key;
    }
...
void SecureRpcPeer::initTls(bool server)
{
    try {
        if (server) {
            ...
            tls_channel = new Botan::TLS::Server(tls_callbacks, session_manager_tls::instance()->ssm, settings,
                                                 settings, rand_gen, false);
        } else {
            ...
            tls_channel = new Botan::TLS::Client(
                tls_callbacks, session_manager_tls::instance()->ssm, settings, settings, rand_gen,
                Botan::TLS::Server_Information(JsonRPCServerModule::instance()->host.c_str(),
                                               JsonRPCServerModule::instance()->port),
...
int SecureRpcPeer::ws_recv_data(uint8_t *data, size_t len)
{
    if (conn_type == PEER_WSS) {
        ...
        return WsRpcPeer::ws_recv_data(data, len);
};
...
int SecureRpcPeer::ws_send_data(const uint8_t *data, size_t len)
{
    if (conn_type == PEER_WSS) {
        ...
        return WsRpcPeer::ws_send_data(data, len);
}
...
int SecureRpcPeer::connect(const string &host, int port, string &res_str)
{
    if (conn_type == PEER_TCP)
        return JsonrpcNetstringsConnection::connect(host, port, res_str);
    else if (conn_type == PEER_WS)
        return WsRpcPeer::connect(host, port, res_str);
    ...
    if (!tls_connected) {
        ...
        if (tls_connected) {
            if (conn_type == PEER_WSS) {
                ...
                if (JsonrpcNetstringsConnection::netstringsBlockingWrite() == REMOVE)
                    return REMOVE;
            }
        }
        return CONTINUE;
...
int SecureRpcPeer::read_data(char *data, int size)
{
    ...
    } else if (conn_type == PEER_TCP || (conn_type == PEER_UNKNOWN && is_tls && !tls_connected) ||
    ...
    if (conn_type == PEER_WSS && ws_connected) {
        ...
        int ret_size = size > static_cast<typeof size>(tls_resv_buffer.size()) ? tls_resv_buffer.size() : size;
...
{
    if (conn_type == PEER_TCP || tls_connected)
