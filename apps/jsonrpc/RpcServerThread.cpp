    ...
    if (server_event->event_id == JsonServerEvent::SendMessage) {
        ...
        if (NULL == connection) {
            ...
            if ((NULL == js_connection) ||
                (NULL == (connection = dynamic_cast<JsonrpcNetstringsConnection*>(
                    js_connection))))
            {
        ...
        if (!snd_msg_ev->is_reply) {
            if(JsonRpcServer::createRequest(
                snd_msg_ev->reply_link, snd_msg_ev->method,
            ...
            int replyCreated = 0;
            replyCreated = JsonRpcServer::createReply(
                connection, snd_msg_ev->id,
                snd_msg_ev->params, snd_msg_ev->is_error);
            if(replyCreated) {
                // give back connection into server loop
                JsonRPCServerLoop::returnConnection(connection);
                return;
