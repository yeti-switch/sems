...
class SipRegistrarFactory : public AmConfigFactory, public AmSessionFactory, public AmDynInvokeFactory {
  private:
    ...
    ~SipRegistrarFactory() { SipRegistrar::dispose(); }

  public:
    DECLARE_FACTORY_INSTANCE(SipRegistrarFactory);

    AmDynInvoke *getInstance() { return registrar; }

    int onLoad() { return registrar->onLoad(); }

    void on_destroy() { registrar->stop(); }
...
struct aor_lookup_reply {
    ...
    bool parse(const RedisReply &e)
    {
        if (RedisReply::SuccessReply != e.result) {
            ERROR("error reply from redis %d %s", e.result, AmArg::print(e.data).c_str());
            return false;
        }
            ...
            for (int j = 0; j < m; j += 3) {
                ...
                if (it == aors.end()) {
                    it = aors.insert(aors.begin(),
                                     std::pair<RegistrationIdType, std::list<AorData>>(reg_id, std::list<AorData>()));
                }
...
struct RedisBlockingRequestCtx : public AmObject {
  public:
...
int SipRegistrar::configure(cfg_t *cfg)
{
    ...
    return RegistrarRedisClient::configure(cfg) | RegistrarClickhouse::configure(cfg);
}

void SipRegistrar::connect(const Connection &conn)
{
    session_container->postEvent(REDIS_APP_QUEUE, new RedisAddConnection(SIP_REGISTRAR_QUEUE, conn.id, conn.info));
}
...
void SipRegistrar::process_register_request_event(SipRegistrarRegisterRequestEvent &event)
{
    ...
    if (parse_nameaddr_list(contact_list, req->contact.c_str(), static_cast<int>(req->contact.length())) < 0) {
    ...
    for (const auto &c : contact_list) {
        ...
        if (!contact_uri.parse_contact(c2stlstr(c), 0, end)) {
            ...
            DBG("successfully parsed contact %s@%s", contact_uri.uri_user.c_str(), contact_uri.uri_host.c_str());
    ...
    if (!expires_found) {
        ...
        while (start_pos < req->hdrs.length()) {
            ...
            if (0 ==
                strncasecmp(req->hdrs.c_str() + start_pos, expires_param_header_name.c_str(), name_end - start_pos))
            {
    ...
    if (expires_found) {
        ...
        if (expires_min && expires_int && expires_int < 3600 && expires_int < expires_min) {
            DBG("expires %d is lower than allowed min: %d. reply with 423", expires_int, expires_min);
            static string min_expires_header = SIP_HDR_COL("Min-Expires") + int2str(expires_min) + CRLF;
        ...
        if (expires_max && expires_int > expires_max) {
            DBG("expires %d is greater than allowed max: %d. set it to max", expires_int, expires_max);
    ...
    while (start_pos < req->hdrs.length()) {
        ...
        if (0 == strncasecmp(hdr_name.c_str(), SIP_HDR_PATH, hdr_name.size())) {
            if (!path.empty())
                path += ",";
            path += req->hdrs.substr(val_begin, val_end - val_begin);
        } else if (0 == strncasecmp(hdr_name.c_str(), SIP_HDR_USER_AGENT, hdr_name.size())) {
...
void SipRegistrar::process_redis_reply_event(RedisReply &redis_reply)
{
    ...
    case UserTypeId::BlockingReqCtx:   process_redis_reply_blocking_req_ctx_event(redis_reply); break;
...
void SipRegistrar::process_redis_reply_register_event(RedisReply &event)
{
    ...
    if (!req || event.result != RedisReply::SuccessReply) {
        if (req) {
            ERROR("redis %s(%s) for request from %s:%hu. ci:%s", RedisReply::resultStr(event.result).data(),
                  event.data.print().c_str(), req->remote_ip.data(), req->remote_port, req->callid.data());
        } else {
            ERROR("redis %s(%s)", RedisReply::resultStr(event.result).data(), event.data.print().c_str());
        }
    ...
    if (!isArgArray(event.data)) {
        ERROR("error/unexpected reply from redis: %s for request from %s:%hu. Contact:'%s'",
              AmArg::print(event.data).c_str(), req->remote_ip.data(), req->remote_port, req->contact.data());
...
void SipRegistrar::process_redis_reply_resolve_aors_event(RedisReply &event)
{
    ...
    for (const auto &aor_entry : r.aors) {
        for (const auto &d : aor_entry.second) {
            DBG("aor_id: %s, contact: '%s', path: '%s'", aor_entry.first.c_str(), d.contact.data(), d.path.data());
        }
    }
...

        {
            ...
            if (it != keepalive_contexts.end()) {
                ...
                if (event.reply.local_reply)
                    ...
                    ctx.last_reply_rtt_ms =
                        std::chrono::duration_cast<milliseconds>(system_clock::now() - it->second.last_sent);
...
void SipRegistrar::post_register_response(const string &session_id, const AmSipRequest *req, int code,
...
void SipRegistrar::on_timer()
{
    ...
    for (auto &ctx_it : keepalive_contexts) {
        ...
        if (0 == dlg->sendRequest(SIP_METH_OPTIONS)) {
            ...
            ERROR("failed to send keep alive OPTIONS request for %s", ctx.aor.data());
        }
...
void SipRegistrar::keepalive_ctx_data::update(const string &_aor, const string &_path, const string &_interface_name,
...
void SipRegistrar::keepalive_ctx_data::dump(const string &key, const system_clock::time_point &now) const
{
    DBG("keepalive_context. key: '%s', "
        ...
        "next_send-now: %d",
        key.c_str(), aor.data(), path.data(), interface_name.c_str(),
...
void SipRegistrar::KeepAliveContexts::getSnapshot(AmArg &ret, std::function<void(AmArg &data)> f_enrich_entry)
{
    ret.assertArray();
    AmLock l(mutex);
    for (const auto &i : *this) {
...
void SipRegistrar::create_or_update_keep_alive_context(const string &key, std::function<void(AmArg &data)> f_load,
                                                       const seconds &keep_alive_interval_offset)
{
    ...
    if (!data.hasMember("path") || !data.hasMember("interface_name")) {
    ...
    if (it == keepalive_contexts.end()) {
        ctx = &keepalive_contexts
                   .try_emplace(key, contact, data["path"].asCStr(), data["interface_name"].asCStr(), next_time)
                   .first->second;
    } else {
        ctx = &it->second;
