...
TEST_F(RegistrarTest, RedisFormatTest)
{
    char *cmd, *cmd1;
...
TEST_F(RegistrarTest, RedisSimpleTest)
{
    timeval timeout = { DEFAULT_REDIS_TIMEOUT_MSEC, 0 };
    redisContext* ctx = redis::redisConnectWithTimeout(
        registrar_test::instance()->redis.host.c_str(),
        registrar_test::instance()->redis.port, timeout);
    ASSERT_TRUE(ctx);
...
TEST_F(RegistrarTest, RedisConnectionTest)
{
    TestRedisConnection conn;
    conn.init(registrar_test::instance()->redis.host.c_str(),
              registrar_test::instance()->redis.port);
    conn.start();
...
TEST_F(RegistrarTest, RedisMultiTest)
{
    timeval timeout = { DEFAULT_REDIS_TIMEOUT_MSEC, 0 };
    redisContext* ctx = redis::redisConnectWithTimeout(
        registrar_test::instance()->redis.host.c_str(),
