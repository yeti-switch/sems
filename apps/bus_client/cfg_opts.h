...
cfg_opt_t bus_node_param[] = { CFG_INT(PARAM_PRIORITY_NAME, 0, CFGF_NODEFAULT),
                               CFG_INT(PARAM_WEIGHT_NAME, 0, CFGF_NONE), CFG_END() };

cfg_opt_t method[] = { CFG_SEC(SECTION_BUS_NODE_NAME, bus_node_param, CFGF_MULTI | CFGF_TITLE),
                       CFG_BOOL(PARAM_BROADCAST_NAME, cfg_false, CFGF_NONE), CFG_END() };

cfg_opt_t routing[] = { CFG_SEC(SECTION_METHOD_NAME, method, CFGF_MULTI | CFGF_TITLE), CFG_END() };

cfg_opt_t bus_node[] = { CFG_STR(PARAM_ADDRESS_NAME, "", CFGF_NODEFAULT), CFG_INT(PARAM_PORT_NAME, 0, CFGF_NODEFAULT),
                         CFG_END() };

cfg_opt_t dyn_queue[] = { CFG_STR(PARAM_APP_NAME, "", CFGF_NODEFAULT), CFG_END() };

cfg_opt_t bus_client_opts[] = { CFG_INT(PARAM_RECONN_INT_NAME, RECONNECT_DEFAULT_INTERVAL, CFGF_NONE),
