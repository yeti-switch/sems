...
static int sctp_sendv(int s, struct iovec *iov, int iov_len, struct sctp_sndrcvinfo *sinfo, int flags)
{
...
BusConnection::BusConnection(BusClient *_bus, const sockaddr_storage &_saddr, int _slot, int _reconnect_interval,
                             int _node_id, int _so_rcvbuf, int _so_sndbuf)
    : slot(_slot)
    , node_id(_node_id)
...
{
    if (state != Connected && timer_val - last_activity > (uint64_t)reconnect_interval) {
        close();
        connect();
        ++reconn;
    }
...
void BusConnection::send_hello()
{
    ...
    size_t info_length = strlen(payload), pdu_length = sizeof(bus_pdu_hdr_t) + sizeof(bus_pdu_hello_t);
...
void BusConnection::handle_notification(const char *payload, int length)
{
    ...
    case SCTP_ASSOC_CHANGE:
        ...
        case SCTP_COMM_UP:
        case SCTP_RESTART:        send_hello(); break;
        case SCTP_COMM_LOST:
...
void BusConnection::hello_handler(bus_pdu_t *pdu, int info_length)
{
    ...
    INFO("HELLO: type=%d id=%u ver=0x%08x sign=0x%016lx info='%.*s'", hello->node_type, hello->node_id, hello->node_ver,
         hello->node_sign, info_length, hello + sizeof(struct bus_pdu_hello));
}
...
string BusConnection::inflatePacked(const char *data, uint32_t data_size)
{
    ...
    if (out_len_int <= 0 || out_len_int > MAX_DECOMPRESSED_SIZE) {
        ERROR("inflatePacked(%p, %u): got decompressed size: %d. return empty result", data, data_size, out_len_int);
        return ret;
    }
...
void BusConnection::pdu_handler(int status, const string &src, const string &dst, const char *body, uint32_t b_size,
                                const char *packed, uint32_t p_size)
{
    ...
    if (b_size)
        if (!json2arg(body, event_data)) {
            ERROR("failed deserialize json payload. body: '%s'", body);
    ...
    if (p_size) {
        ...
        if (!json2arg(conf, args)) {
            ERROR("failed deserialize json payload. packed: '%s'", conf.c_str());
            postError(dst, "incorrect json payload");
            return;
        }
        if (!isArgStruct(args)) {
    ...
    DBG("post reply event. src: '%s', dst: '%s', status: %d", src.c_str(), dst.c_str(), status);
...
void BusConnection::recv()
{
    ...
    length = sctp_recvmsg(fd, buffer, sizeof(buffer), (struct sockaddr *)&from, &fromlen, &sinfo, &flags);
    ...
    DBG("got PDU with type: %s (0x%04x). PDU length: %d, payload length: %d", pdu_type_to_str(type), hdr->type, length,
        ntohl(hdr->length));
#if 0
    string s;
    ...
    case BUS_PDU_QUERY_PACKED_RESP: packed = true;
    case BUS_PDU_QUERY_RESP:        bus->on_query_response(hdr->seq);
    case BUS_PDU_QUERY:
    ...
    case BUS_PDU_EVENT:
    {
        ...
        uint8_t          src_length = query.src_length, dst_length = query.dst_length;

        length -= (sizeof(bus_pdu_hdr_t) + (packed ? sizeof(bus_pdu_query_packet_t) : sizeof(bus_pdu_query_t)) +
        ...
        if (length < 0) {
            ERROR("Bad formated packet: length=%ld, src_len=%d, dst_len=%d, body_len=%d", length, src_length,
                  dst_length, body_length);
            break;
        }
        ...
        const char *addrs =
            reinterpret_cast<char *>(&query) + (packed ? sizeof(bus_pdu_query_packet_t) : sizeof(bus_pdu_query_t));
        if (src_length) {
...
void print_events(const char *func, uint32_t events)
{
    ...
    if (events & EPOLLIN)
        len += sprintf(&buf[len], "EPOLLIN ");
    if (events & EPOLLOUT)
        len += sprintf(&buf[len], "EPOLLOUT ");
    if (events & EPOLLPRI)
        len += sprintf(&buf[len], "EPOLLPRI ");
    if (events & EPOLLERR)
        len += sprintf(&buf[len], "EPOLLERR ");
    if (events & EPOLLHUP)
        len += sprintf(&buf[len], "EPOLLHUP ");
    if (events & EPOLLRDHUP)
        len += sprintf(&buf[len], "EPOLLRDHUP ");
...
bool BusConnection::sendMsg(BusMsg *msg, uint32_t &msg_seq)
{
    ...
    DBG("BUS_%s: %.*s: %.*s", msg->is_query ? "QUERY" : "EVENT", (int)dst.addr_length, (char *)dst.addr, (int)data_size,
        (char *)data);
...
void BusConnection::handler(uint32_t ev)
{
    if (ev & ~(EPOLLIN | EPOLLOUT)) {
        ...
        ERROR("NETWORK: fd=%d %s [%s]:%u", fd, err ? strerror(err) : "Peer shutdown", am_inet_ntop(&saddr).c_str(),
              am_get_port(&saddr));
