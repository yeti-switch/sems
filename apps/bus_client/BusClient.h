...
class BusDynamicQueue : public AmEventFdQueue, public AmEventHandler {
    string     queue_name;
    string     application;
    BusClient *bus;

  public:
    BusDynamicQueue(BusClient *bus, const string &queue_name, const string &application)
        : AmEventFdQueue(this)
...
class BusClient : public AmDynInvokeFactory,
    ...
    typedef enum { CONN_OK = 0, CONN_ERROR, CONN_WARNING } send_conn_result_t;
