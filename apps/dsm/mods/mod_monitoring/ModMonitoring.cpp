...
#define MOD_MONITORING_LOG(id, prop, val) \
	if(true_str==val) { MONITORING_LOG(id, prop, true) }\
	else if(false_str==val) { MONITORING_LOG(id, prop, false) }\
	else { MONITORING_LOG(id, prop, val) }

#define MOD_MONITORING_LOG_ADD(id, prop, val) \
	if(true_str==val) { MONITORING_LOG_ADD(id, prop, true) }\
	else if(false_str==val) { MONITORING_LOG_ADD(id, prop, false) }\
	else { MONITORING_LOG_ADD(id, prop, val) }
...
bool MonLogAction::execute(AmSession* sess, DSMSession* sc_sess,
  ...
  MOD_MONITORING_LOG(sess->getLocalTag().c_str(), prop.c_str(), val.c_str());
...
bool MonLogAddAction::execute(AmSession* sess,  DSMSession* sc_sess,
  ...
  MOD_MONITORING_LOG_ADD(sess->getLocalTag().c_str(), prop.c_str(), val.c_str());
...
EXEC_ACTION_START(MonLogGlobalAction) {
  ...
  MOD_MONITORING_LOG(id.c_str(), prop.c_str(), val.c_str());
...
EXEC_ACTION_START(MonLogAddGlobalAction) {
  ...
  MOD_MONITORING_LOG_ADD(id.c_str(), prop.c_str(), val.c_str());
