...
void DSMCall::onInvite(const AmSipRequest& req) {
  ...
  // pass AmSipRequest for use by mod_qwriter
...
void DSMCall::onDtmf(AmDtmfEvent* e) {
  ...
  params["source"] = int2str(e->event_id);
...
void DSMCall::process(AmEvent* event)
{
  ...
  if (conf_ev) {
    ...
    params["participants"] = int2str(conf_ev->participants);
    params["sess_id"] = conf_ev->sess_id;
  ...
  B2BSipReplyEvent* b2bevent = dynamic_cast<B2BSipReplyEvent *>(event);
  if(b2bevent)
  {
      map<string, string> params;
      params["b2b_sip_reason"] = b2bevent->reply.reason;
      params["b2b_sip_code"] = int2str(b2bevent->reply.code);
      engine.runEvent(this, this, DSMCondition::B2BEvent, &params);
  }
