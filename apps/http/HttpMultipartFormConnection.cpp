    ...
    for (const auto &part : event_->parts) {
        ...
        case HttpPostMultipartFormEvent::Part::FilePath:
            ...
            if (destination.min_file_size && file_size < destination.min_file_size) {
                INFO("file '%s' is too small (%u < %u). skip request. perform on_success actions", file_path.c_str(),
                     file_size, destination.min_file_size);
                curl_mime_free(form);
                form                     = 0;
...
static void dump_event(const HttpPostMultipartFormEvent &event)
{
    ...
    ERROR("%p parts: %ld, failover_idx: %d, attempt: %d", event_ptr, event.parts.size(), event.failover_idx,
          event.attempt);
    int i = 0;
    for (const auto &part : event.parts) {
        ERROR("%p parts[%d] name: %s, type: %d, content_type: %s, value: %s", event_ptr, i, part.name.c_str(),
              part.type, part.content_type.c_str(), part.value.c_str());
        i++;
    }
}
