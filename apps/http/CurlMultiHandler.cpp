...
void CurlMultiHandler::on_timer_event()
{
    ...
    CDBG("curl_multi_socket_action(CURL_SOCKET_TIMEOUT,0) = %d, running_handles = %d", rc, curl_running_handles);
...
void CurlMultiHandler::on_socket_event(int socket, uint32_t events)
{
    ...
    CDBG("on_socket_event(%d, %d (%s|%s|%s))", socket, events, events & EPOLLIN ? "EPOLLIN" : "",
         events & EPOLLOUT ? "EPOLLOUT" : "", events & EPOLLERR ? "EPOLLERR" : "");
    ...
    CDBG("curl_multi_socket_action(%d, %d) = %d, running_handles = %d", socket, action, rc, curl_running_handles);
...
int CurlMultiHandler::socket_callback(CURL *, curl_socket_t s, int what, void *sockp)
{
    ...
    if (sockp) {
        CDBG("modify socket %d watching for events %d (%s|%s|%s)", s, ev.events, ev.events & EPOLLIN ? "EPOLLIN" : "",
             ev.events & EPOLLOUT ? "EPOLLOUT" : "", ev.events & EPOLLERR ? "EPOLLERR" : "");
        ...
        CDBG("enable socket %d watching for events %d (%s|%s|%s)", s, ev.events, ev.events & EPOLLIN ? "EPOLLIN" : "",
             ev.events & EPOLLOUT ? "EPOLLOUT" : "", ev.events & EPOLLERR ? "EPOLLERR" : "");
