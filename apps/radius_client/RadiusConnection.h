...
class RadiusConnection {
  protected:
    ...
    long long int requests_sent, replies_got, requests_err, requests_timeouts, replies_err, replies_socket_err,
        replies_match_err, replies_validate_err;
    double min_response_time, max_response_time;

    enum field_format { str, integer, octets, date, ipv4, ipv6 };
    struct avp_info {
        uint8_t      type;
        field_format fmt;
        string       name;
  ...
  public:
    RadiusConnection(unsigned int connection_id, string &name, string &server, unsigned short port, string &secret,
                     unsigned int timeout_msec, unsigned int attempts);
