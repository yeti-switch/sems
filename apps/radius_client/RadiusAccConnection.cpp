...
RadiusAccConnection::RadiusAccConnection(unsigned int connection_id, string &name, string &server, unsigned short port,
                                         string &secret, unsigned int timeout_msec, unsigned int attempts,
                                         AmArg start_avps, AmArg interim_avps, AmArg stop_avps,
                                         bool enable_start_accounting, bool enable_interim_accounting,
                                         bool enable_stop_accounting, int interim_accounting_interval)
    ...
    , rules(enable_start_accounting, enable_interim_accounting, enable_stop_accounting, interim_accounting_interval)
{
}
...
{
    DBG("got reply for accounting request for session %s with code %d", request.session().c_str(), reply.code());
}
...
{
    switch (type) {
    case RadiusRequestEvent::Start:   return start_avps;
    case RadiusRequestEvent::Interim: return interim_avps;
    case RadiusRequestEvent::End:     return stop_avps;
    }
