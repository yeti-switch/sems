...
void RadiusClient::invoke(const string &method, const AmArg &args, AmArg &ret)
{
    if (method == "r") {
        ...
        ret = addAuthConnection(args.get(0).asInt(),  // connection_id
        ...
        ret = addAccConnection(args.get(0).asInt(),   // connection_id
                               args.get(1).asCStr(),  // name
                               args.get(2).asCStr(),  // server
                               args.get(3).asInt(),   // port
                               args.get(4).asCStr(),  // secret
...
void RadiusClient::process(AmEvent *ev)
{
    ...
    if (radius_request) {
        switch (ev->event_id) {
        case RadiusRequestEvent::Auth:       onRadiusAuthRequest(*radius_request); break;
        case RadiusRequestEvent::Accounting: onRadiusAccRequest(*radius_request); break;
        }
        return;
    }
...
void RadiusClient::onRadiusAuthRequest(const RadiusRequestEvent &req)
{
    ...
    if (it == auth_connections.end()) {
        ERROR("invalid server_id in request from %s", req.session_id.c_str());
        if (!AmSessionContainer::instance()->postEvent(
                req.session_id, new RadiusReplyEvent(RadiusReplyEvent::Error, RADIUS_INVALID_SERVER_ID, true)))
        {
...
int RadiusClient::addAuthConnection(unsigned int connection_id, string name, string server, unsigned short port,
                                    string secret, bool reject_on_error, unsigned int timeout_msec,
                                    unsigned int attempts, AmArg avps)
{
    struct epoll_event ev;
    AmLock             l(connections_mutex);
    (void)l;

    DBG("add auth connection %d to %s:%d. timeout = %ims, attempts = %i, reject_on_error = %d", connection_id,
        server.c_str(), port, timeout_msec, attempts, reject_on_error);
    ...
    RadiusAuthConnection *c = new RadiusAuthConnection(connection_id, name, server, port, secret, reject_on_error,
                                                       timeout_msec, attempts, avps);
...
void RadiusClient::clearAuthConnections()
{
    AmLock l(connections_mutex);
    (void)l;
    for (AuthConnections::iterator it = auth_connections.begin(); it != auth_connections.end(); it++) {
        const RadiusAuthConnection *c    = it->second;
        int                         sock = c->get_sock();
        if (epoll_ctl(epoll_fd, EPOLL_CTL_DEL, sock, NULL) == -1) {
            WARN("epoll_ctl error on delete operation for socket %d: %s", sock, strerror(errno));
        }
...
void RadiusClient::check_timeouts()
{
    ...
    for (AccConnections::iterator it = acc_connections.begin(); it != acc_connections.end(); it++) {
        it->second->check_timeouts();
    }
}
...
void RadiusClient::on_packet(int sock)
{
    ...
    if (it != sock2connection.end()) {
        DBG("process incoming packets for connection %d", it->first);
        it->second->process();
    }
}
...
void RadiusClient::getAccRules(const AmArg &args, AmArg &ret)
{
    ...
    if (it == acc_connections.end()) {
        ret.clear();
        ERROR("invalid radius acc profile id %d on rules request", connection_id);
        return;
    }
...
int RadiusClient::addAccConnection(unsigned int connection_id, string name, string server, unsigned short port,
                                   string secret, unsigned int timeout_msec, unsigned int attempts, AmArg start_avps,
                                   AmArg interim_avps, AmArg stop_avps, bool enable_start_accounting,
                                   bool enable_interim_accounting, bool enable_stop_accounting,
                                   int interim_accounting_interval)
{
    ...
    DBG("add acc connection %d to %s:%d. timeout = %ims, attempts = %i", connection_id, server.c_str(), port,
    ...
    RadiusAccConnection *c = new RadiusAccConnection(
        connection_id, name, server, port, secret, timeout_msec, attempts, start_avps, interim_avps, stop_avps,
        enable_start_accounting, enable_interim_accounting, enable_stop_accounting, interim_accounting_interval);
