...
int RadiusPacket::add_vendor_attr(uint8_t type, uint32_t vendor_id, uint8_t vendor_type, const char *buf,
                                  unsigned int len)
{
...
int RadiusPacket::add_attr_int32(uint8_t type, uint32_t i)
{
    ...
    *(uint32_t *)attrs_tail = htonl(i);
    attrs_tail += sizeof(uint32_t);
...
{
    attempt++;
    DBG("send radius packet with code %d for session %s", code(), session().c_str());
    gettimeofday(&netstamp, NULL);
