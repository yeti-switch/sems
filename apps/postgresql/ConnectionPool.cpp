...
ConnectionPool::ConnectionPool(const PGPool &pool, PoolWorker *worker, PGWorkerPoolCreate::PoolType type)
    : last_returned_conn_idx(0)
    , worker(worker)
    , pool(pool)
    , type(type)
    , connected(stat_group(Gauge, MOD_NAME, "pool_connected")
                    .addAtomicCounter()
                    .addLabel("worker", worker->get_name())
                    .addLabel("type", type == PGWorkerPoolCreate::Master ? pool_type_master : pool_type_slave))
    , poolsize(stat_group(Gauge, MOD_NAME, "pool_size")
                   .addAtomicCounter()
                   .addLabel("worker", worker->get_name())
                   .addLabel("type", type == PGWorkerPoolCreate::Master ? pool_type_master : pool_type_slave))
{
...
Connection *ConnectionPool::getFreeConnection()
{
    ...
    for (auto connections_to_check = connections_count, conn_idx = last_returned_conn_idx + 1; connections_to_check--;
         conn_idx++)
    {
...
void ConnectionPool::getStats(AmArg &stats, uint32_t conn_lifetime)
{
    ...
    AmArg &pool_stats = (type == PGWorkerPoolCreate::Master) ? stats[pool_type_master] : stats[pool_type_slave];
