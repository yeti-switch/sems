...
class Transaction {
  public:
    enum DbState { BEGIN = 0, BODY, END };

    enum Status { ACTIVE = 0, CANCELING, ERROR, FINISH };
  ...
  public:
    ...
    void   add_log(const char *func, const char *file, int line, const char *format, Types... args);
...
#ifdef TRANS_LOG_ENABLE
template <class... Types>
void Transaction::add_log(const char *func, const char *file, int line, const char *format, Types... args)
{
