...
int Transaction::check()
{
    ...
    do {
        ...
        case PQTRANS_INTRANS:
            ...
            ret = begin();
            if (ret < 0) {
                ERROR("begin(): %d. query: %s, last_error:%s", ret, tr_impl->query->get_query().data(),
            ...
            if (1 == ret) {
                do {
                    ...
                    ret = execute();
                    if (ret < 0) {
                        ERROR("execute(): %d. query: %s, last_error:%s", ret, tr_impl->query->get_query().data(),
                              tr_impl->query->get_last_error());
            ...
            if (0 == ret) {
                ...
                ret = end();
                if (ret < 0) {
                    ERROR("end(): %d. query: %s, last_error:%s", ret, tr_impl->query->get_query().data(),
                          tr_impl->query->get_last_error());
        ...
        case PQTRANS_INERROR:
            ...
            ret = rollback();
            if (ret < 0) {
                ERROR("rollback(): %d. query: %s, last_error:%s", ret, tr_impl->query->get_query().data(),
                      tr_impl->query->get_last_error());
            ...
            break;
        default: ERROR("unexpected tr_impl->status: %d", tr_impl->status);
        } // switch(tr_impl->status)
    } while (next);
...
{
    if (!tr_impl->query || !trans->tr_impl->query || !is_equal(trans))
        return false;
...
IQuery *Transaction::get_current_query(bool parent)
{
    ...
    if (num >= chain->get_size()) {
        if (num != chain->get_size()) {
            WARN("got result (%d) more than chain size (%d), return last query", num, chain->get_size());
        }
