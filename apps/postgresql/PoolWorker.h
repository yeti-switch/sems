...
class pending_reset_time_less {
  public:
    ...
    {
        if (a->getPendingResetTime() < b->getPendingResetTime())
            return true;
        return a < b;
    }
};

class PoolWorker : public ITransactionHandler, public IConnectionHandler {
    int    epoll_fd;
    string name;
    ...
    struct resetConnectionsContainer : public set<Connection *, pending_reset_time_less> {
        time_t getNearestResetTime() { return (*begin())->getPendingResetTime(); }
    } resetConnections;
    ...
    struct TransContainer {
        ...
        string                                sender_id;
        string                                trans_id;
        TransContainer(Transaction *trans, ConnectionPool *pool, const string &sender, const string &token,
                       const string &id = "")
            : trans(trans)
            , currentPool(pool)
            , createdTime(time(0))
    ...
    vector<Transaction *> erased;       // temp container for finished transactions(on the next iteration they will be
                                        // deleted)
