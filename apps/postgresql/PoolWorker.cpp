...
void PoolWorker::onConnect(Connection *conn)
{
    ...
    if (master && !master->checkConnection(conn, true) && slave)
        slave->checkConnection(conn, true);
    time_t now = time(0);
    if (conn_lifetime && (reconn_next_time > now + conn_lifetime || !reconn_next_time))
        reconn_next_time = now + conn_lifetime;
    if (use_pipeline)
        conn->startPipeline();
    if (!prepareds.empty() || !search_pathes.empty() || !init_queries.empty()) {
        ...
        if (!conn->runTransaction(trans)) {
            ERROR("connection %p/%s of worker \'%s\' transaction already exists ", conn, conn->getConnInfo().c_str(),
                  name.c_str());
            delete trans;
        }
        if (conn_lifetime)
...
void PoolWorker::onReset(Connection *conn, bool connected)
{
    ...
    if (connected && master && !master->checkConnection(conn, false) && slave)
        slave->checkConnection(conn, false);
}
...
{
    ERROR("pg connection %s:%p/%s stopped transaction %d %s", name.c_str(), trans->get_conn(),
          trans->get_conn()->getConnInfo().c_str(), trans->get_size(), trans->get_query()->get_query().c_str());
    for (auto tr_it = transactions.begin(); tr_it != transactions.end(); tr_it++) {
...
void PoolWorker::onDisconnect(Connection *conn)
{
    ...
    if (master && !master->checkConnection(conn, false) && slave)
        slave->checkConnection(conn, false);
...
void PoolWorker::onFinish(Transaction *trans, const AmArg &result)
{
    ...
    for (auto tr_it = transactions.begin(); tr_it != transactions.end(); tr_it++) {
        if (trans == tr_it->trans) {
            ...
            tr_size.dec((long long)tr_it->trans->get_size());
            finished_time.inc(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() -
                                                                                    tr_it->sendTime)
                                  .count());
            transactions.erase(tr_it);
            return;
...
bool PoolWorker::processEvent(void *p)
{
    ...
    if (master && master->processEvent(p))
        return true;
    if (slave && slave->processEvent(p))
        return true;
    return false;
}
...
void PoolWorker::createPool(PGWorkerPoolCreate::PoolType type, const PGPool &pool)
{
    if (type == PGWorkerPoolCreate::Master) {
        if (!master)
            master = new ConnectionPool(pool, this, PGWorkerPoolCreate::Master);
        else
            ERROR("master connection pool of worker %s already created", name.c_str());
    }
    if (type == PGWorkerPoolCreate::Slave) {
        if (!slave)
            slave = new ConnectionPool(pool, this, PGWorkerPoolCreate::Slave);
        else
            ERROR("slave connection pool of worker %s already created", name.c_str());
    }
}
...
void PoolWorker::getFreeConnection(Connection **conn, ConnectionPool **pool, std::function<void(const string &)> func)
{
    do {
        if (!*pool && master)
            *pool = master;
        else if (slave && (!*pool || *pool == master))
            *pool = slave;
        else {
...
int PoolWorker::retransmitTransaction(TransContainer &trans)
{
    ...
    if (!trans.currentPool) {
        ...
        if (failover_to_slave && slave && !conn)
            ...
            conn->runTransaction(trans.trans);
            return 0;
        } else if ((failover_to_slave && !slave) || !failover_to_slave) {
...
void PoolWorker::setWorkTimer(bool immediately)
{
    if (immediately) {
        ...
        auto update_timer_interval = [&interval, current](time_t next_time) {
            if (next_time && (!interval || next_time < current || next_time - current < interval)) {
    ...
    // DBG("worker \'%s\'\n\t\tcurrent_time - %lu, reset_next_time - %lu, retransmit_next_time - %lu, wait_next_time -
    // %lu, send_next_time - %lu, reconn_next_time - %lu",
    //     get_name().c_str(), time(0), reset_next_time, retransmit_next_time, wait_next_time, send_next_time,
    //     reconn_next_time);
}
...
{
    for (auto trans_it = retransmit_q.begin(); trans_it != retransmit_q.end();) {
    ...
    if (send_next_time > time(0) && queue.size() < batch_size)
        return;
...
void PoolWorker::runTransaction(Transaction *trans, const string &sender_id, const std::string &token)
{
    ...
    if (batch_size > 1 && !sender.empty()) {
        WARN(
            "batch size of worker \'%s\' is not null, sender_id \'%s\' is not null, will ignore sender_id and erase it",
...
void PoolWorker::runPrepared(const PGPrepareData &prepared)
{
    ...
    if (prepared.sql_types.empty()) {
        ...
        for (const auto &sql_type : prepared.sql_types) {
            ...
            if (oid == INVALIDOID) {
                ERROR("unsupported typname '%s' for prepared statement: %s. skip", sql_type.data(),
                      prepared.stmt.data());
                return;
            }
...
void PoolWorker::configure(const PGWorkerConfig &e)
{
    ...
    if (master)
        master->usePipeline(use_pipeline);
    if (slave)
        slave->usePipeline(use_pipeline);
...
{
    if (master && type == PGWorkerPoolCreate::Master)
        master->resetConnections();
    if (slave && type == PGWorkerPoolCreate::Slave)
...
{
    ERROR("pg connection %s/%s:%p/%s active transaction timeout. transaction size:%d, query:%s", name.c_str(),
          trans.currentPool == master ? pool_type_master.data() : pool_type_slave.data(), trans.trans->get_conn(),
          trans.trans->get_conn()->getConnInfo().c_str(), trans.trans->get_size(),
          trans.trans->get_query()->get_query().data());

    if (!retransmit_enable && (!failover_to_slave || trans.currentPool == slave) && !trans.sender_id.empty()) {
        auto *ev = new PGTimeout(trans.token);
...
{
    ERROR("pg connection %s/%s:%p/%s transaction error: '%s', transaction size:%d, query:%s", name.c_str(),
          trans.currentPool == master ? pool_type_master.data() : pool_type_slave.data(), trans.trans->get_conn(),
          trans.trans->get_conn()->getConnInfo().c_str(), error.data(), trans.trans->get_size(),
          trans.trans->get_query()->get_query().data());
    ...
    if (!retransmit_enable && (!failover_to_slave || trans.currentPool == slave) && !trans.sender_id.empty()) {
        ...
        } else if (trans.trans->get_type() == TR_POLICY || (trans.trans->get_type() == TR_NON && use_pipeline)) {
...
void PoolWorker::onTimer()
{
...
#ifdef TRANS_LOG_ENABLE
    for (auto trans_it = transactions.begin(); PostgreSQL::instance()->getLogTime() && trans_it != transactions.end();
         trans_it++)
    {
    ...
    resetConnections.clear();
    reset_next_time = 0;
    for (auto conn_it = conns.begin(); conn_it != conns.end();) {
    ...
    if (conn_lifetime && reconn_next_time <= now) {
        ...
        if (nextTime)
            reconn_next_time = nextTime + conn_lifetime;
    }
...
{
    return conn->getPendingResetTime() > 0 || resetConnections.find(conn) != resetConnections.end();
}
