...
class PostgreSQLFactory : public AmDynInvokeFactory, public AmConfigFactory {
    ...
    ~PostgreSQLFactory() { PostgreSQL::dispose(); }
  ...
  public:
    ...
    int reconfigure(const std::string &config) { return PostgreSQL::instance()->reconfigure(config); }
};
...
bool PostgreSQL::showConfiguration(const string &connection_id, const AmArg &request_id, const AmArg &params)
{
    postEvent(new JsonRpcRequestEvent(connection_id, request_id, false, MethodShowConfig, params));
    return true;
}

bool PostgreSQL::showRetransmits(const std::string &connection_id, const AmArg &request_id, const AmArg &params)
{
    if (!params.size() || !isArgCStr(params[0])) {
        throw AmSession::Exception(500, "usage: postgresql.show.retransmit <worker>");
    }

    postEvent(new JsonRpcRequestEvent(connection_id, request_id, false, MethodShowRetransmit, params));
    return true;
}

bool PostgreSQL::logPgEventsAsync(const string &connection_id, const AmArg &request_id, const AmArg &params)
{
...
{
    if (!args.size() || !isArgCStr(args[0]) || (args.size() > 1 && !isArgCStr(args[1])) ||
    ...
    if (args.size() > 1) {
        PGWorkerPoolCreate::PoolType type =
            (strcmp(args[1].asCStr(), "master") ? PGWorkerPoolCreate::Slave : PGWorkerPoolCreate::Master);
...
{
    if (args.size() != 2 || !isArgCStr(args[0])) {
...
#ifdef TRANS_LOG_ENABLE
bool PostgreSQL::transLog(const string &connection_id, const AmArg &request_id, const AmArg &params)
{
    if (params.size() != 3 || !isArgCStr(params[0]) || !isArgCStr(params[2])) {
        throw AmSession::Exception(
            500, "usage: postgresql.request.get_connection_log <worker name> <connection fd> <file path>");
    }
    ...
    postEvent(new JsonRpcRequestEvent(connection_id, request_id, false, MethodTransLog, params));
    return true;
}
#endif
...
void PostgreSQL::init_rpc_tree()
{
    ...
    auto &remove = reg_leaf(request, "remove");
...
void PostgreSQL::process(AmEvent *ev)
{
    ...
    default: process_postgres_event(ev);
    }
}
