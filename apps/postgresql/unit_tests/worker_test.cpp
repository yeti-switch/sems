...
TEST_F(PostgresqlTest, WorkerTransactionTest)
{
    ...
    AmArg arg;
    PostgreSQL::instance()->showStats(arg, arg);
    AmArg arg1 = arg["workers"][WORKER_POOL_NAME];
    for(size_t i = 0; i < arg1["master"].size(); i++) {
...
TEST_F(PostgresqlTest, WorkerQueueErrorTest)
{
    ...
    while(true){
        ...
        AmArg arg;
        worker.getStats(arg);
        if(arg["finished"].asInt() == 2 && arg["retransmit"].asInt() == 1) break;
        usleep(500);
    }
    ...
    while(true){
        ...
        AmArg arg;
        worker.getStats(arg);
        if(arg["finished"].asInt() == 3 && arg["retransmit"].asInt() == 1) break;
        usleep(500);
    }
}
...
TEST_F(PostgresqlTest, WorkerSearchPathTest)
{
    ...
    while(true) {
        ...
        AmArg arg;
        worker.getStats(arg);
        if(arg["master"][0]["status"].asInt() == CONNECTION_OK ||
