...
TEST_F(PostgresqlTest, QueryParamTest)
{
    ...
    QueryParams   *q_insert =
        new QueryParams("INSERT INTO test(id, value, data, str) VALUES($1, $2, $3, $4);", false, false);
...
TEST_F(PostgresqlTest, QueryPreparedTest)
{
    ...
    PreparedTransaction prt("test_insert", "INSERT INTO test(id, value, data, str) VALUES($1, $2, $3, $4);", oids,
                            &handler);
...
TEST_F(PostgresqlTest, ChainQueryTest)
{
    ...
    QueryChain *q_chain = new QueryChain(new QueryParams(
...
TEST_F(PostgresqlTest, DbTransactionMergeTest)
{
    PGHandler    handler;
    Transaction *trans =
        createDbTransaction(&handler, PGTransactionData::read_committed, PGTransactionData::write_policy::read_only);
    Transaction *trans1 =
        createDbTransaction(&handler, PGTransactionData::read_committed, PGTransactionData::write_policy::read_only);
    ...
    Transaction *trans2 =
        createDbTransaction(&handler, PGTransactionData::read_committed, PGTransactionData::write_policy::read_write);
...
TEST_F(PostgresqlTest, DbPipelineAbortedTest)
{
    ...
    QueryChain    *query = new QueryChain(new QueryParams(
        "CREATE TABLE IF NOT EXISTS test(id int, value float8, data varchar(50), str json);", false, false));
    query->addQuery(new QueryParams("SELECT TTT", false, false));
    pg3.exec(query);
    server->addError("SELECT TTT", false);
...
TEST_F(PostgresqlTest, DbPipelineTransErrorTest)
{
    ...
    query = new QueryChain(new QueryParams(
        "CREATE TABLE IF NOT EXISTS test(id int, value float8, data varchar(50), str json);", false, false));
    pg1->exec(query);
