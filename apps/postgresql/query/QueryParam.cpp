...
[[maybe_unused]] static inline bool pq_get_bool(const char *value)
{
    return (bool)*value;
}
static inline int16_t pg_get_int2(const char *value)
{
    return (int16_t)ntohs(*(int16_t *)value);
}
static inline int32_t pq_get_int4(const char *value)
{
    return (int32_t)ntohl(*(int32_t *)value);
}
static inline int64_t pq_get_int8(const char *value)
{
    return ntohll(*(int64_t *)value);
}
[[maybe_unused]] static inline const char *pq_get_text(const char *value)
{
    return value;
}
    ...
    case INT2OID: // smallint
        if (isArgInt(val)) {
            ...
            *((int16_t *)binvalue) = 0;
            ...
            ERROR("AmArg Int expected for smallint/int2. got:%s. save as null", AmArg::print(val).data());
            oid = INVALIDOID;
        }
        break;
    case INT8OID: // bigint
        if (isArgInt(val)) {
            ...
            *((int64_t *)binvalue) = 0;
            ...
            ERROR("AmArg Int/LongLong expected for bigint/int8. got:%s. save as null", AmArg::print(val).data());
            oid = INVALIDOID;
        }
        break;
    case JSONOID:
        if (isArgCStr(val)) {
            ...
            ERROR("AmArg Json expected for string. got:%s. save as null", AmArg::print(val).data());
            oid = INVALIDOID;
        }
        break;
    default: ERROR("unsupported typed param with oid: %d. save as null", param_oid); oid = INVALIDOID;
    }
}
...
{
    switch (oid) {
    case INVALIDOID: return nullptr;
    case BOOLOID:
    case INT2OID:
    case INT4OID:
    case INT8OID:
...
{
    switch (oid) {
    case INVALIDOID: return 0;
    case BOOLOID:    return sizeof(uint8_t);
    case INT2OID:    return sizeof(uint16_t);
    case INT4OID:    return sizeof(uint32_t);
    case INT8OID:    return sizeof(uint64_t);
    case FLOAT4OID:  return sizeof(float);
    case FLOAT8OID:  return sizeof(double);
    case TEXTOID:
    case JSONOID:    return strvalue.size();
    }
    return 0;
}
...
bool QueryParam::is_binary_format()
{
    ...
    case TEXTOID:
    case JSONOID:   return false;
    }
    return true;
}

template <typename T> struct free_deleter {
    void operator()(T const *p) { free(p); }
};

static bool iterate_pg_array(const char *value, AmArg &ret, std::function<AmArg(const char *item_value)> parse_cb)
{
    ...
    for (; *s; s++) {
        ...
        default:
            ...
            case ST_ITEM_START:
                ...
                token = s;
                break;
            case ST_ESCAPED_SYMBOL: state = last_state; break;
            default:                break;
            }
        }
    }
...
AmArg get_result(unsigned int oid, bool is_binary, const char *value, bool is_null)
{
    ...
    case BOOLOID:
        if (is_binary) {
            ...
            if (value[0] == 't' || value[0] == 'y' || strcmp(value, "on") == 0 || value[0] == '1') {
                ret = AmArg(true);
            } else {
                ret = AmArg(false);
            }
    ...
    case INT2OID:
        if (is_binary)
            ret = AmArg(pg_get_int2(value));
        else
            ret = AmArg(atoi(value));
        break;
    case INT4OID:
        if (is_binary)
            ret = AmArg(pq_get_int4(value));
        else
            ret = AmArg(atol(value));
        break;
    case INT8OID:
        if (is_binary)
            ret = AmArg(pq_get_int8(value));
        else
            ret = AmArg(atoll(value));
        break;
    case FLOAT4OID:
        if (is_binary)
            ret = AmArg((double)pq_get_float4(value));
        else
            ret = AmArg(atof(value));
        break;
    case FLOAT8OID:
        if (is_binary)
            ret = AmArg(pq_get_float8(value));
        else
            ret = AmArg(atof(value));
        break;
    case JSONOID:
        ...
        if (isArgUndef(param))
            ...
            if (param.hasMember("pg")) {
                ...
                if (isArgArray(a) && a.size() == 2 && isArgCStr(a[0])) {
                    qparams.emplace_back(pg_typname2oid(a[0].asCStr()), a[1]);
                } else {
