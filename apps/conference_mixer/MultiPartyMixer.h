...
class MultiPartyMixer
{
    ...
    struct ChannelData {
        ...
        ChannelData(int samplerate) : samplerate(samplerate), samples() {}
    };
...
public:
    ...
    const ext_resampling_state_ptr& get_ext_resampler(int neighbor_idx)
        { return ext_resampling[backlog_id];  }
    ...
    void PutExtChannelPacket(int neighbor_idx, unsigned long long system_ts,
                         unsigned char* buffer,
                         unsigned int   size);

    int GetExtChannelPacket(unsigned long long system_ts,
                         unsigned char* buffer,
                         unsigned int&  output_sample_rate);
