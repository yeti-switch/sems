...
TxRing::TxRing(int sd, const vector<sockaddr_storage> &neighbor_saddr)
    ...
    , pending(0)
{
    buffer = (unsigned char *)mmap(NULL, TX_RING_MMAP_SIZE, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
...
void TxRing::put(unsigned long long ts, uint64_t id, unsigned output_sample_rate, unsigned char *data, unsigned size)
{
...
{
    buffer = (unsigned char *)mmap(NULL, RX_RING_MMAP_SIZE, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
    if (buffer == MAP_FAILED)
        throw string("RxRing: mmap() call failed"); // throw std::bad_alloc();
...
void RxRing::handler(uint32_t ev, int fd)
{
    ...
    do {
        ...
        length = ::recvfrom(fd, next_rx_frame->data, UDP_MESSAGE_MAX, 0, reinterpret_cast<sockaddr *>(&saddr),
                            &recv_addr_len);
