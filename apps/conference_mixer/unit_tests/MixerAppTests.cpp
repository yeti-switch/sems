...
vector<int> make_sequence(size_t size) {
    vector<int> tmp(size);
    int count {};
    for(auto it = tmp.begin(); it != tmp.end(); ++it) *it = count++;
    return tmp;
}
...
struct ThreadBase
{
    ...
    virtual ~ThreadBase() {}
    virtual void thread_func() { for(int i {}; i < repeat_count; ++i) do_run(); }
...
TEST_F(MixerTest, DISABLED_MultipleConfChStressTest)
{
    ...
    for(int i {}; i < PERSISTENT_EXT_CHANNELS_COUNT; ++i) {
        for(auto & ch_id : ch_ids) {
            auto ch_ptr = Mixer::instance()->getConferenceChannel(ch_id, NON_PERSISTENT_EXT_CHANNELS_COUNT+i, "local_tag", 8000);
