...
{
    cfg_opt_t opt[] = { CFG_BOOL("run_garbage_collector", cfg_true, CFGF_NONE),
                        CFG_INT("garbage_collector_interval", 10, CFGF_NONE),
                        CFG_INT("retain_samples_s", 10, CFGF_NONE), CFG_END() };
...
int Monitor::onLoad()
{
    ...
    if (gcRun) {
        DBG("Running garbage collection for monitoring every %u seconds", gcInterval);
...
void Monitor::invoke(const string &method, const AmArg &args, AmArg &ret)
{
    if ((method == "log") || (method == "set")) {
        ...
        get(args, ret);
    } else if (method == "getByFilter") {
        listByFilter(args, ret, false, frtype_info);
...
void Monitor::addSample(const AmArg &args, AmArg &ret)
{
    ...
    LogBucket &bucket = getLogBucket(args[0].asCStr());
    bucket.log_lock.lock();
    list<SampleInfo::time_cnt> &sample_list = bucket.samples[args[0].asCStr()].sample[args[1].asCStr()];
...
void Monitor::truncate_samples(list<SampleInfo::time_cnt> &v, struct timeval now)
{
...
void Monitor::getCount(const AmArg &args, AmArg &ret)
{
    ...
    if (it != bucket.samples.end()) {
        ...
        map<string, list<SampleInfo::time_cnt>>::iterator s_it = it->second.sample.find(args[1].asCStr());
...
void Monitor::getAllCounts(const AmArg &args, AmArg &ret)
{
    ...
    if (it != bucket.samples.end()) {
        ...
        for (map<string, list<SampleInfo::time_cnt>>::iterator s_it = it->second.sample.begin();
...
void Monitor::getAttribute(const AmArg &args, AmArg &ret)
{
    ...
    for (int i = 0; i < NUM_LOG_BUCKETS; i++) {
        logs[i].log_lock.lock();
        for (std::map<string, LogInfo>::iterator it = logs[i].log.begin(); it != logs[i].log.end(); it++) {
            ret.push(AmArg());
...
DEF_GET_ATTRIB_FUNC(getAttributeActive, (!(it->second.finished && it->second.finished <= now)))
DEF_GET_ATTRIB_FUNC(getAttributeFinished, (it->second.finished && it->second.finished <= now))
...
void Monitor::listByFilter(const AmArg &args, AmArg &ret, bool erase, filter_respone_type rtype)
{
    ret.assertArray();
    for (int i = 0; i < NUM_LOG_BUCKETS; i++) {
        logs[i].log_lock.lock();
        try {
            ...
            while (it != logs[i].log.end()) {
                ...
                if (match) {
                    ...
                    case frtype_info:
                        ret.push(it->second.info);
                        ret.back()["id"] = it->first.c_str();
                        break;
                    case frtype_id:
                    default:        ret.push(AmArg(it->first.c_str()));
                    }
                    if (erase) {
                        std::map<string, LogInfo>::iterator d_it = it;
                        it++;
...
void Monitor::listByRegex(const AmArg &args, AmArg &ret)
{
    ...
    for (int i = 0; i < NUM_LOG_BUCKETS; i++) {
        ...
        try {
            for (std::map<string, LogInfo>::iterator it = logs[i].log.begin(); it != logs[i].log.end(); it++) {
                if (!it->second.info.hasMember(args[0].asCStr()) || !isArgCStr(it->second.info[args[0].asCStr()]) ||
