...
{
    putEvent(new AudioRecorderCtlEvent(recorder_id, AudioRecorderEvent::addRecorder,
                                       AmAudioFileRecorder::RecorderMonoAmAudioFile, file_path, sync_ctx_id));
}
...
void _AmAudioFileRecorderProcessor::processRecorderEvent(AudioRecorderEvent &ev)
{
    ...
    RecordersMap &r =
        ev.getRecorderClassByEventId() == AudioRecorderCtlEvent::RecorderClassMono ? mono_recorders : stereo_recorders;

    RecordersMap::iterator recorder_it = r.find(ev.recorder_id);
    if (recorder_it == r.end()) {
        if (ev.event_id == AudioRecorderEvent::addRecorder || ev.event_id == AudioRecorderEvent::addStereoRecorder) {
            ...
            if (0 != recorder->init(ctl_event->file_path, ctl_event->sync_ctx_id)) {
                ERROR("can't init recorder %s with path '%s'", ev.recorder_id.c_str(), ctl_event->file_path.c_str());
                delete recorder;
                return;
            }

            DBG("recorder %s inited with file: %s", ev.recorder_id.c_str(), ctl_event->file_path.c_str());
    ...
    case AudioRecorderEvent::putSamples:
    {
        ...
        recorder->writeSamples(samples_event.data, samples_event.data_size, samples_event.sample_rate);
    } break;
    ...
    case AudioRecorderEvent::addStereoRecorder:
        ...
        if (0 != recorder->add_file(ctl_event->file_path)) {
            ERROR("failed to add file to the recorder %s with path '%s'", ev.recorder_id.c_str(),
                  ctl_event->file_path.c_str());
        } else {
            DBG("recorder %s updated with file: %s", ev.recorder_id.c_str(), ctl_event->file_path.c_str());
        }
        break;
    case AudioRecorderEvent::delRecorder:
    case AudioRecorderEvent::delStereoRecorder:
        DBG("delete recorder %p with id: %s", recorder, recorder_it->first.c_str());
        delete recorder;
        r.erase(recorder_it);
        recorders_closed++;
        break;
    case AudioRecorderEvent::setTag:
    {
        ...
        DBG("tag recorder %p(%s) channel_id %u, tag %u", recorder, recorder_it->first.c_str(), tag_event.channel_id,
            tag_event.tag);
    ...
    case AudioRecorderEvent::markRecordStopped:
        ...
        DBG("mark recorder %p(%s) as stopped. file_path:'%s'", recorder, recorder_it->first.c_str(),
