...
{
    DBG("AmB2ABSession::relayEvent: id=%s", other_id.c_str());
...
void AmB2ABCallerSession::onB2ABEvent(B2ABEvent *ev)
{
    ...
    case B2ABConnectEarlyAudio:
    {
        ...
        B2ABConnectEarlyAudioEvent *ca = dynamic_cast<B2ABConnectEarlyAudioEvent *>(ev);
        if (!ca)
            return;
...
void AmB2ABCallerSession::connectCallee(const string &remote_party, const string &remote_uri, const string &local_party,
                                        const string &local_uri, const string &headers)
{
    if (callee_status != None)
        terminateOtherLeg();

    B2ABConnectLegEvent *ev = new B2ABConnectLegEvent(remote_party, remote_uri, local_party, local_uri,
...
void AmB2ABCallerSession::setupCalleeSession(AmB2ABCalleeSession *callee_session, B2ABConnectLegEvent *ev)
{
...
void AmB2ABCalleeSession::onB2ABEvent(B2ABEvent *ev)
{
    if (ev->event_id == B2ABConnectLeg) {
        ...
        try {
            ...
            MONITORING_LOG4(getLocalTag().c_str(), "b2b_leg", other_id.c_str(), "from", co_ev->local_party.c_str(),
                            "to", co_ev->remote_party.c_str(), "ruri", co_ev->remote_uri.c_str());
                                     ...
                                     AmBasicSipDialog::Status old_dlg_status)
{
    ...
    if ((old_dlg_status == AmSipDialog::Trying) || (old_dlg_status == AmSipDialog::Proceeding) ||
