...
void AmPlayoutBuffer::write(u_int32_t ref_ts, u_int32_t rtp_ts, int16_t *buf, u_int32_t len, bool begin_talk)
{
    unsigned int mapped_ts;
    if (!recv_offset_i) {
        recv_offset   = rtp_ts - ref_ts;
        recv_offset_i = true;
        DBG("initialized recv_offset with %u (%u - %u)", recv_offset, ref_ts, rtp_ts);
        mapped_ts = r_ts = w_ts = ref_ts;
    } else {
        mapped_ts = rtp_ts - recv_offset;

        // resync
        if (ts_less()(mapped_ts, ref_ts - MAX_DELAY / 2) || !ts_less()(mapped_ts, ref_ts + MAX_DELAY)) {

            DBG("resync needed: reference ts = %u; write ts = %u", ref_ts, mapped_ts);
            recv_offset = rtp_ts - ref_ts;
            mapped_ts = r_ts = w_ts = ref_ts;
    ...
    if (ts_less()(last_ts, mapped_ts) && !begin_talk && (mapped_ts - last_ts <= PLC_MAX_SAMPLES)) {
...
u_int32_t AmAdaptivePlayout::next_delay(u_int32_t ref_ts, u_int32_t ts)
{
    ...
    int32_t D = int32_t(D_r + double(D_r1 - D_r) * ((double(o_stat.size()) + 1.0) * (1.0 - loss_rate) - double(r)));
...
void AmAdaptivePlayout::write_buffer(u_int32_t ref_ts, u_int32_t ts, int16_t *buf, u_int32_t len)
{
    // predict next delay
    ...
    if (ts_less()(wsola_off + EXP_THRESHOLD, p_delay) || // expand packet
        ts_less()(p_delay + shr_threshold, wsola_off))
    { // shrink packet
...
u_int32_t AmAdaptivePlayout::time_scale(u_int32_t ts, float factor, u_int32_t packet_len)
{
    ...
    while (true) {
        ...
        buffer_put(cur_ts + TEMPLATE_SEG, srch + TEMPLATE_SEG, p_buf_end - srch - TEMPLATE_SEG);
        // size s has changed
        s += tmpl - srch;
...
#ifdef DEBUG_PLAYOUTBUF
        DBG("at ts %u: new size = %u, ratio = %f, requested = %f (wsola_off = %ld)", ts, s, act_fact, factor,
            (long)wsola_off);
#endif
