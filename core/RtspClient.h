...
{
    string          uri;
    RtspNoFileEvent(string &_uri) : AmEvent(RTSP_NO_FILE_ID), uri(_uri) {}
};

struct RtspTimeoutEvent
    : public AmEvent
{
    string          uri;
    RtspTimeoutEvent(const string &_uri) : AmEvent(RTSP_TIMEOUT_ID), uri(_uri) {}
};
