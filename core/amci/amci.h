 ...
 */
typedef int (*amci_converter_t)(unsigned char *out, unsigned char *in, unsigned int size, unsigned int channels,
 ...
 */
typedef int (*amci_plc_t)(unsigned char *out, unsigned int size, unsigned int channels, unsigned int rate,
                          long h_codec);
 ...
 */
typedef int (*amci_file_close_t)(FILE *fptr, struct amci_file_desc_t *fmt_desc, int options, long h_codec,
 ...
 * @see amci_inoutfmt_t::open
 */
typedef int (*amci_file_mem_open_t)(unsigned char *mptr, unsigned long size, unsigned long *pos,
 ...
 * @see amci_inoutfmt_t::on_close
 */
typedef int (*amci_file_mem_close_t)(unsigned char *mptr, unsigned long *pos, struct amci_file_desc_t *fmt_desc,
 ...
 * @param format_description [out] pointer to describing block, with amci_codec_fmt_info_t array; zero-terminated. 0 if
 * none <table><tr><td><b>key</b></td><td><b>value</b></td></tr> <tr><td>AMCI_FMT_FRAME_LENGTH (1)</td><td>  frame
 * length in ms (for framed codecs; must be multiple of 10)</td></tr> <tr><td>AMCI_FMT_FRAME_SIZE (2)</td><td>  frame
 * size in samples</td></tr> <tr><td>AMCI_FMT_ENCODED_FRAME_SIZE (3)</td><td>  encoded frame size</td></tr></table>
 ...
 * @hideinitializer
 */
#define CODEC_VARIABLE_FRAMES(id, intern2type, type2intern, plc, init, destroy, bytes2samples, samples2bytes,          \
                              frames2bytes)                                                                            \
 ...
 * @hideinitializer
 */
#define SUBTYPE(type, name, rate, channels, codec_id) { type, name, rate, channels, codec_id },
