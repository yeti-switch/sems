...
#define cmp_cond(trsp) 0 == strncasecmp(s.s, trsp.s, trsp.len <= s.len ? trsp.len : s.len)
    ...
    if (cmp_cond(udp))
        return SIP_info::UDP;
    else if (cmp_cond(tcp))
        return SIP_info::TCP;
    else if (cmp_cond(tls))
        return SIP_info::TLS;
    else if (cmp_cond(ws))
        return SIP_info::WS;
    else if (cmp_cond(wss))
        return SIP_info::WSS;
    return SIP_info::UNDEFINED;
#undef cmp_cond
}

const char *AmBasicSipDialog::status2str[AmBasicSipDialog::__max_Status] = { "Disconnected", "Trying", "Proceeding",
                                                                             "Cancelling",   "Early",  "Connected",
                                                                             "Disconnecting" };
...
{
    DBG3("setting SIP dialog status: %s->%s", getStatusStr(), getStatusStr(new_status));
...
int AmBasicSipDialog::getOutboundIf()
{
    ...
    if (!next_hop.empty() && !parse_next_hop(stl2cstr(next_hop), ip_list) && !ip_list.empty()) {
        d_uri.host     = ip_list.front().host;
        transport_type = str2transport(ip_list.front().trsp);
    } else if (!outbound_proxy.empty() && (remote_tag.empty() || force_outbound_proxy)) {
        ...
        if (!route_uri) {
            DBG("Could not parse route (local_tag='%s';route='%s')", local_tag.c_str(), route.c_str());
            goto error;
        }
    ...
    if (!dest_uri.empty()) {
        if (parse_uri(&d_uri, dest_uri.c_str(), static_cast<int>(dest_uri.length()), true) < 0) {
            DBG("Could not parse destination URI (local_tag='%s';dest_uri='%s')", local_tag.c_str(), dest_uri.c_str());
            goto error;
        }
    ...
    if (resolve_sip_uri(d_uri, remote_ip, static_cast<dns_priority>(resolve_priority)) < 0) {
        ERROR("Could not resolve URI host '%.*s' (local_tag='%s')", d_uri.host.len, d_uri.host.s, local_tag.c_str());
        goto error;
    }
    ...
    if (AT_NONE == address_type) {
        address_type = AT_V4;
        ERROR(
            "Could not resolve address type for resolved remote IP (local_tag='%s'; remote_ip='%s'). Use default: IPv4",
    ...
    if (force_outbound_interface_by_name.empty()) {
        if (get_local_addr_for_dest(remote_ip, local_ip, static_cast<dns_priority>(resolve_priority)) < 0) {
            DBG("No local address for dest '%.*s' (local_tag='%s')", d_uri.host.len, d_uri.host.s, local_tag.c_str());
            goto error;
        }
        ...
        if (if_it == AmConfig.sip_if_names.end()) {
            ERROR("Could not find a local interface for force interface name "
                  "(local_tag='%s';remote_ip='%s';iface_name='%s')",
    ...
    if (proto_id < 0) {
        ERROR("Could not find a transport in interface %s for resolved IP (local_tag='%s'; local_ip='%s'; "
              "remote_ip='%s').",
              AmConfig.sip_ifs[if_it->second].name.data(), local_tag.c_str(), local_ip.c_str(), remote_ip.c_str());
        goto error;
    }
...
AddressType AmBasicSipDialog::getOutboundAddrType()
{
    ...
    if (addrType) {
        ...
        ERROR("Could not parse local URI (local_tag='%s';local_ip='%s')", local_tag.c_str(), local_ip.c_str());
    }
...
{
    // Sanity checks
    if (!remote_tag.empty() && !req.from_tag.empty() && (req.from_tag != remote_tag)) {
        DBG("remote_tag = '%s'; req.from_tag = '%s'", remote_tag.c_str(), req.from_tag.c_str());
...
void AmBasicSipDialog::onRxRequest(const AmSipRequest &req)
{
    ...
    if (req.from_uri.length() &&
        (remote_uri.empty() || (req.method == SIP_METH_INVITE || req.method == SIP_METH_UPDATE ||
                                req.method == SIP_METH_SUBSCRIBE || req.method == SIP_METH_NOTIFY)))
    {
        ...
        if (remote_uri != req.from_uri) {
            ...
            if (nat_handling && req.first_hop) {
                ...
                ensure_ipv6_reference(nh);
                nh += ":" + int2str(req.remote_port) + "/" + req.trsp;
                setNextHop(nh);
...
void AmBasicSipDialog::onRxReply(const AmSipReply &reply)
{
    if (!onRxReplySanity(reply)) {
        DBG("reply %d:%s for %s is dropped by onRxReplySanity", reply.code, reply.reason.c_str(),
    ...
    if (t_it == uac_trans.end()) {
        _LOG(reply.code < 200 ? L_DBG : L_ERR, "could not find any transaction matching reply: %s\n",
             ((AmSipReply)reply).print().c_str());
        return;
    }

    DBG("onRxReply(rep = %u %s): transaction found!", reply.code, reply.reason.c_str());
...
{
    if ((reply.code > 100) && (reply.code < 300) && !reply.to_uri.empty() && !reply.to_tag.empty() &&
        (remote_uri.empty() ||
         ...
         (reply.cseq_method == SIP_METH_SUBSCRIBE)))
    {
        ...
        if (!getNextHop().empty()) {
            string nh = reply.remote_ip + ":" + int2str(reply.remote_port) + "/" + reply.trsp;
            setNextHop(nh);
        }
...
int AmBasicSipDialog::onTxReply(const AmSipRequest &req, AmSipReply &reply, int &flags)
{
    if (hdl)
        hdl->onSendReply(req, reply, flags);

    return 0;
}

void AmBasicSipDialog::onReplyTxed(const AmSipRequest &req, const AmSipReply &reply)
{
    if (hdl)
        hdl->onReplySent(req, reply);
...
{
    DBG("Could not send request: method=%s; ruri=%s; call-id=%s; cseq=%i", req.method.c_str(), req.r_uri.c_str(),
        req.callid.c_str(), req.cseq);
    if (hdl)
        hdl->onRequestSendFailed(req);
}

int AmBasicSipDialog::reply(const AmSipRequest &req, unsigned int code, const string &reason, const AmMimeBody *body,
                            const string &hdrs, int flags)
{
    ...
    if (t_it == uas_trans.end()) {
        DBG("could not find any transaction matching request "
            ...
            "remote_tag=%s",
            req.cseq, code, callid.c_str(), local_tag.c_str(), remote_tag.c_str());
        return -1;
    }
    ...
    if (onTxReply(req, reply, flags)) {
        WARN("onTxReply failed, callid - %s", callid.c_str());
        return -1;
    }
...
/* static */
int AmBasicSipDialog::reply_error(const AmSipRequest &req, unsigned int code, const string &reason, const string &hdrs,
...
int AmBasicSipDialog::sendRequest(const string &method, const AmMimeBody *body, const string &hdrs, int flags,
                                  sip_timers_override *timers_override, sip_target_set *target_set_override,
                                  unsigned int redirects_allowed)
{
    unique_ptr<sip_target_set> targets(target_set_override ? target_set_override
                                                           : new sip_target_set((dns_priority)getResolvePriority()));
