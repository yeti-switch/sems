...
class CallStatusUpdateEvent : public AmEvent {
  ...
  public:
    enum UpdateType { Initialize = 0, Update, Obsolete };
 ...
 */
class AmCallWatcher : public AmThread, public AmEventQueue, public AmEventHandler {
  public:
...
class AmCallWatcherGarbageCollector : public AmThread {
  ...
  public:
    AmCallWatcherGarbageCollector(AmMutex &mut, AmCallWatcher::CallStatusTimedMap &garbage)
        : mut(mut)
        , garbage(garbage)
    {
    }
