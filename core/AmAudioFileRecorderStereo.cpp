...
(\
    ((lts==rts) || (SCALED_DIFF(file_sp, lts,rts) <= MAX_TS_DIFF)) \
    ...
    int input_sample_rate)
{
    ...
    return state->resample(
        samples, size,
        ((double)file_sp) / ((double)input_sample_rate));
}
...
void AmAudioFileRecorderStereo::writeStereoSamples(unsigned long long ts,
