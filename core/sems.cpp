...
{
    printf("  Supported SRTP profiles:\n");
...
static void print_supported_zrtp_attributes()
{
    ...
    printf("  Supported ZRTP attributes:\n"
           "    hash algorithms:\n"
           "       S256, S384, N256, N384\n"
           "    ciphers algorithms:\n"
...
static void print_usage(bool short_ = false)
{
    if (short_) {
        ...
        printf("Usage: %s [STARTUP_OPTIONS...|INFORMATIONAL_OPTION]\n\n"
               " Startup options:\n"
...
static bool process_args(int argc, char *argv[], std::map<char, string> &args)
{
    ...
    while (-1 != (opt = getopt_long(argc, argv, opts, long_options, &option_index))) {
        ...
        default: args[static_cast<char>(opt)] = (optarg ? optarg : "yes");
        }
    }
    return true;
}
...
{
    for (std::map<char, string>::iterator it = args.begin(); it != args.end(); ++it) {
        switch (it->first) {
        /*case 'd':
        ...
        case 'E':
            ...
            break;
        case 'f': AmLcConfig::instance().config_path = it->second; break;
        case 'x': AmConfig.modules_path.emplace_back(it->second); break;
#ifndef DISABLE_DAEMON_MODE
        case 'P': AmConfig.deamon_pid_file = it->second; break;
        case 'u': AmConfig.deamon_uid = it->second; break;
        case 'g': AmConfig.deamon_gid = it->second; break;
#endif
        case 'h':
        case 'v':
        default:
...
class ConfigReloadTimer : public timer {
  ...
  public:
    ConfigReloadTimer()
        : timer(wheeltimer::instance()->wall_clock)
    {
    }
    void fire() override
    {
...
static void signal_handler(int sig)
{
    ...
    if (sig == SIGUSR2) {
        DBG("brodcasting User event to %u sessions...", AmSession::getSessionNum());
...
{
    static int sigs[] = { SIGHUP, SIGPIPE, SIGINT, SIGTERM, SIGQUIT, SIGCHLD, SIGUSR1, SIGUSR2, 0 };
...
static int write_pid_file()
{
    ...
    if (fpid) {
        ...
        ERROR("Cannot write PID file '%s': %s.", AmConfig.deamon_pid_file.c_str(), strerror(errno));
    }
...
int set_fd_limit()
{
    ...
    DBG3("Open FDs limit has been raised to %u", (unsigned int)rlim.rlim_cur);
...
{
    switch (level) {
    case srtp_log_level_error:   ERROR("SRTP-LOG: %s", msg); break;
    case srtp_log_level_warning: WARN("SRTP-LOG: %s", msg); break;
    case srtp_log_level_info:    INFO("SRTP-LOG: %s", msg); break;
...
class GroupsContainerSelfStatm : public StatsCountersGroupsContainerInterface {
    long long sys_page_size;

    struct Group : public StatCountersGroupsInterface {
        ...
        void iterate_counters(iterate_counters_callback_type callback) override { callback(value, /*0,*/ labels); }
    };
    Group vmsize_group;
    Group vmrss_group;
...
int main(int argc, char *argv[])
{
...
#endif
           "\n",
           AmLcConfig::instance().config_path.c_str(), AmLcConfig::instance().getModulesPathList().c_str()
#ifndef DISABLE_DAEMON_MODE
                                                           ,
           AmConfig.deamon_mode ? "yes" : "no", AmConfig.deamon_uid.empty() ? "<not set>" : AmConfig.deamon_uid.c_str(),
    ...
    if (AmConfig.deamon_mode) {
        ...
        if (!AmConfig.deamon_gid.empty()) {
            ...
            if (str2i(AmConfig.deamon_gid, gid)) {
                ...
                if (grnam != NULL) {
                    ...
                    ERROR("Cannot find group '%s' in the group database.", AmConfig.deamon_gid.c_str());
                    goto error;
                }
            }
        ...
        if (!AmConfig.deamon_uid.empty()) {
            ...
            if (str2i(AmConfig.deamon_uid, uid)) {
                ...
                if (pwnam != NULL) {
                    ...
                    ERROR("Cannot find user '%s' in the user database.", AmConfig.deamon_uid.c_str());
                    goto error;
                }
            }
        ...
        if (freopen("/dev/null", "r", stdin) == 0) {
            ERROR("Cannot replace stdin with /dev/null: %s.", strerror(errno));
            /* continue, leave it open */
        }

        if (freopen("/dev/null", "w", stdout) == 0) {
            ERROR("Cannot replace stdout with /dev/null: %s.", strerror(errno));
            /* continue, leave it open */
        }

        /* close stderr only if log_stderr=0 */
        if ((!AmConfig.log_stderr) && (freopen("/dev/null", "w", stderr) == 0)) {
            ERROR("Cannot replace stderr with /dev/null: %s.", strerror(errno));
            /* continue, leave it open */
        }
    } // if(AmConfig.deamon_mode)
#endif /* DISABLE_DAEMON_MODE */
    ...
    stat_group(Counter, "core", "localtime").addFunctionCounter([]() -> unsigned long long {
        return static_cast<unsigned long long>(time(nullptr));
    });

    stat_group(Counter, "core", "uptime").addFunctionCounter([]() -> unsigned long long {
        return static_cast<unsigned long long>(time(nullptr) - start_time);
    });

    stat_group(Gauge, "core", "version")
        .addFunctionCounter([]() -> unsigned long long { return 1; })
        .addLabel("core", SEMS_VERSION);

    stat_group(Gauge, "core", "shutdown_mode").addFunctionCounter([]() -> unsigned long long {
        return AmConfig.shutdown_mode == true ? 1 : 0;
    });

    if (AmConfig.session_limit) {
        stat_group(Gauge, "core", "sessions_limit").addFunctionCounter([]() -> unsigned long long {
            return AmConfig.session_limit;
        });
    }

    stat_group(Counter, "core", "start_time").addAtomicCounter().set(static_cast<unsigned long long>(start_time));
