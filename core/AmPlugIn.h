...
class AmPlugIn : public AmPayloadProvider {
  ...
  public:
     ...
     * @return -1 if failed, else 0.
     */
    int load(const vector<string> &directories, const std::vector<std::string> &plugins);
    ...
    /** register a SIP Event Handler module
        ...
        @return true on success
     */
    static bool registerSIPEventHandler(const string &seh_name, AmSessionEventHandlerFactory *f);
