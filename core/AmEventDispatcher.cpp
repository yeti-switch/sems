...
bool AmEventDispatcher::addEventQueue(const string &local_tag, AmEventQueueInterface *q)
{
...
bool AmEventDispatcher::addEventQueue(const string &local_tag, AmEventQueueInterface *q, const string &callid,
...
bool AmEventDispatcher::post(const string &callid, const string &remote_tag, const string &via_branch, AmEvent *ev)
{
    string id = callid + remote_tag;
    if (AmConfig.accept_forked_dialogs) {
        id += via_branch;
