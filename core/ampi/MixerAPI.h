...
class ConferenceChannel : public AmAudio {
    ...
    void put_external(int neighbor_idx, unsigned long long ts, unsigned char *buffer, int sample_rate, int size);
...
class Mixer : public AmDynInvokeFactory,
  ...
  public:
    ...
    channel_ptr getConferenceChannel(const string &channel_id, uint64_t channel_ext_id, const string &local_tag,
                                     int sample_rate);
...
{
    return nr < MAX_CHANNEL_CTX ? &Mixer::backlog_data[nr] : nullptr;
}
