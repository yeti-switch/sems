...
struct PGPool {
    ...
    PGPool(const string &host_, uint16_t port_, const string &name_, const string &user_, const string &pass_)
        : host(host_)
        , port(port_)
        , name(name_)
        , user(user_)
...
class PGWorkerDestroy : public PGEvent {
  public:
    ...
    PGWorkerDestroy(string name_)
        : PGEvent(WorkerDestroy)
        , worker_name(name_)
    {
    }
};
...
class PGSetSearchPath : public PGEvent {
  public:
    ...
    PGSetSearchPath(string name)
        : PGEvent(SetSearchPath)
        , worker_name(name)
    {
    }
...
class QueryInfo {
  public:
    ...
    template <typename T> QueryInfo &addParam(const T &param)
    {
        params.emplace_back(param);
        return *this;
    }

    template <typename T> QueryInfo &addTypedParam(const char *sql_type, const T &param)
    {
        params.emplace_back();
...
class PGQueryData {
  public:
    ...
    PGQueryData(const string &name_, const string &query_, bool single, const string &session_id = string(),
    ...
    PGQueryData(const string &name_, const string &session_id = string(), const string &token_ = string())
        : worker_name(name_)
        , sender_id(session_id)
        , token(token_)
    {
    ...
    void addQuery(const string &query_, bool single_) { info.emplace_back(query_, single_); }
};
...
class PGTransactionData {
  public:
    ...
    enum class write_policy { read_only, read_write } wp;
...
class PGWorkerConfig : public PGEvent {
  public:
    ...
    PGWorkerConfig(const string &name, bool failover_to_slave, bool retransmit_enable, bool use_pipeline,
                   uint32_t trans_wait_time     = PG_DEFAULT_WAIT_TIME,
                   uint32_t retransmit_interval = PG_DEFAULT_RET_INTERVAL,
                   uint32_t reconnect_interval = PG_DEFAULT_REC_INTERVAL, uint32_t batch_size = PG_DEFAULT_BATCH_SIZE,
                   uint32_t batch_timeout = PG_DEFAULT_BATCH_TIMEOUT, uint32_t max_queue_length = PG_DEFAULT_MAX_Q_LEN,
    ...
    void addSearchPath(const string &search_path) { search_pathes.push_back(search_path); }

    void addReconnectError(const string &error) { reconnect_errors.push_back(error); }

    template <typename T> void addInitialQuery(T &&event) { initial_queries.emplace_back(event); }
};
...
class PGPrepareExec : public PGEvent {
  public:
    ...
    PGPrepareExec(const string &name_, const string &stmt_, const QueryInfo &info_, const string &session_id = string(),
                  const string &token_ = string())
        : PGEvent(PrepareExec)
        , worker_name(name_)
        , stmt(stmt_)
    ...
    template <typename T> QueryInfo &addParam(const T &param) { return info.addParam(param); }
};
