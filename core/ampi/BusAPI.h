...
struct BusMsg : public AmEvent {
    ...
    BusMsg(bool _is_query, string _local_tag, string _application_method, string _body, uint64_t _timeout = 0,
           int _status = 0)
        : AmEvent(0)
        , state(New)
        , status(_status)
...
struct BusReplyEvent : public AmEvent {
    enum BusResult { Success, Error } result;
