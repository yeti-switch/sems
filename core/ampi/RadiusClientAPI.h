...
struct RadiusAccountingRules {
    bool enable_start_accounting, enable_interim_accounting, enable_stop_accounting;
    ...
    RadiusAccountingRules(bool start, bool interim, bool stop, int interval)
...
struct RadiusRequestEvent : public AmEvent {
    string                   session_id;
    unsigned int             server_id;
    struct timeval           created_at;
    std::map<string, string> values_hash;

    enum RadiusRequestType { Auth = 0, Accounting };

    enum RadiusAccountingType { Start = 0, Interim, End } accounting_type;

    RadiusRequestEvent(unsigned int server_id, string session_id, std::map<string, string> values)
        : AmEvent(Auth)
    ...
    RadiusRequestEvent(RadiusAccountingType type, unsigned int server_id, string session_id,
...
struct RadiusReplyEvent : public AmEvent {
    enum RadiusResult { Accepted, Rejected, Error } result;
