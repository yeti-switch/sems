...
class srtp_master_keys {
  ...
  public:
    struct strp_mater_key_container : srtp_master_key_t {
        string   key_data;
...
class AmSrtpConnection : public AmStreamConnection {
    ...
    int  is_valid_keys(srtp_profile_t profile, const string &tx_key, const srtp_master_keys &rx_keys);

  public:
    AmSrtpConnection(AmMediaTransport *_transport, const string &remote_addr, int remote_port,
                     AmStreamConnection::ConnectionType conn_type);
    virtual ~AmSrtpConnection();

    void setKeysExpired() { keys_expired = true; }
    void use_keys(srtp_profile_t profile, const string &tx_key, const srtp_master_keys &rx_keys);

    void update_keys(srtp_profile_t profile, const string &tx_key, const srtp_master_keys &rx_keys);
