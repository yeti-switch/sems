...
void AmMediaUdptlState::addConnections(const AmMediaStateArgs &args)
{
    ...
    CLASS_DBG("add udptl connection, state:%s, type:%s, raddr:%s, rport:%d", state2str(), transport->type2str(),
    ...
    transport->addConnection(new_udptl_conn, [&]() { transport->setCurUdptlConn(new_udptl_conn); });
}
