...
static int iLBC_2_Pcm16_Ext(unsigned char *out_buf, unsigned char *in_buf, unsigned int size, unsigned int channels,
                            unsigned int rate, long h_codec, int mode);

static int iLBC_2_Pcm16(unsigned char *out_buf, unsigned char *in_buf, unsigned int size, unsigned int channels,
                        unsigned int rate, long h_codec);

static int iLBC_PLC(unsigned char *out_buf, unsigned int size, unsigned int channels, unsigned int rate, long h_codec);

static int  Pcm16_2_iLBC(unsigned char *out_buf, unsigned char *in_buf, unsigned int size, unsigned int channels,
                         unsigned int rate, long h_codec);
...
BEGIN_CODECS
CODEC(CODEC_ILBC, Pcm16_2_iLBC, iLBC_2_Pcm16, iLBC_PLC, iLBC_create, iLBC_destroy, ilbc_bytes2samples,
      ilbc_samples2bytes)
END_CODECS
...
int Pcm16_2_iLBC(unsigned char *out_buf, unsigned char *in_buf, unsigned int size, unsigned int channels,
                 unsigned int rate, long h_codec)
{
...
static int iLBC_PLC(unsigned char *out_buf, unsigned int size, unsigned int channels, unsigned int rate, long h_codec)
{
    ...
    in_size = (codec_inst->iLBC_Dec_Inst.no_of_bytes * size) / (2 * codec_inst->iLBC_Dec_Inst.blockl);
                            ...
                            unsigned int rate, long h_codec, int mode)
{
    ...
    if (noframes * codec_inst->iLBC_Dec_Inst.no_of_bytes != size) {
        WARN("Dropping extra %d bytes from iLBC packet.", size - noframes * codec_inst->iLBC_Dec_Inst.no_of_bytes);
    }

    for (i = 0; i < noframes; i++) {
        /* do actual decoding of block */

        iLBC_decode(decblock, in_buf + i * codec_inst->iLBC_Dec_Inst.no_of_bytes, &codec_inst->iLBC_Dec_Inst,
