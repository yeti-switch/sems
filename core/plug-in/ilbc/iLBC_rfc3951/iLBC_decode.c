...
)
{
    ...
    if (state_first) { /* put adaptive part in the end */
        ...
        memcpy(mem + CB_MEML - iLBCdec_inst->state_short_len, decresidual + start_pos,
               iLBCdec_inst->state_short_len * sizeof(float));

        /* construct decoded vector */

        iCBConstruct(&decresidual[start_pos + iLBCdec_inst->state_short_len], extra_cb_index, extra_gain_index,
...
)
{
    ...
    if (mode > 0) { /* the data are good */
        ...
        if (mode == 1) { /* No bit errors was detected,
            ...
            Decode(iLBCdec_inst, decresidual, start, idxForMax, idxVec, syntdenum, cb_index, gain_index, extra_cb_index,
                   extra_gain_index, state_first);

            /* preparing the plc for a future loss! */

            doThePLC(PLCresidual, PLClpc, 0, decresidual, syntdenum + (LPC_FILTERORDER + 1) * (iLBCdec_inst->nsub - 1),
    ...
    if (iLBCdec_inst->use_enhancer == 1) {
        ...
        if (iLBCdec_inst->mode == 20) {
            ...
            syntFilter(data + i * SUBL,
                       iLBCdec_inst->old_syntdenum + (i + iLBCdec_inst->nsub - 1) * (LPC_FILTERORDER + 1), SUBL,
            ...
            for (i = 0; i < 2; i++) {
                syntFilter(data + i * SUBL,
                           iLBCdec_inst->old_syntdenum + (i + iLBCdec_inst->nsub - 2) * (LPC_FILTERORDER + 1), SUBL,
                           iLBCdec_inst->syntMem);
            }
        ...
        lag = 20;
        maxcc =
            xCorrCoef(&decresidual[BLOCKL_MAX - ENH_BLOCKL], &decresidual[BLOCKL_MAX - ENH_BLOCKL - lag], ENH_BLOCKL);

        for (ilag = 21; ilag < 120; ilag++) {
            cc = xCorrCoef(&decresidual[BLOCKL_MAX - ENH_BLOCKL], &decresidual[BLOCKL_MAX - ENH_BLOCKL - ilag],
                           ENH_BLOCKL);
