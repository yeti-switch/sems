...
)
{
    ...
    for (q = hl - 1; q >= 0; q--) {
        ...
        if (blockStartPos[q] - ENH_OVERHANG >= 0) {
            refiner(sseq + q * ENH_BLOCKL, blockStartPos + q, idata, idatal, centerStartPos, blockStartPos[q],
                    period[lagBlock[q + 1]]);
        } else {
    ...
    for (q = hl + 1; q <= 2 * hl; q++) {
        ...
        if (blockStartPos[q] + ENH_BLOCKL + ENH_OVERHANG < idatal) {
            refiner(sseq + ENH_BLOCKL * q, blockStartPos + q, idata, idatal, centerStartPos, blockStartPos[q],
                    period[lagBlock[q]]);
        } else {
            psseq = sseq + q * ENH_BLOCKL;
            memset(psseq, 0, ENH_BLOCKL * sizeof(float));
...
)
{
    ...
    DownSample(enh_buf + (ENH_NBLOCKS_EXTRA + ioffset) * ENH_BLOCKL - 120, lpFilt_coefsTbl,
               inLen - ioffset * ENH_BLOCKL, lpState, downsampled);
    ...
    for (iblock = 0; iblock < ENH_NBLOCKS - ioffset; iblock++) {
        ...
        maxcc = xCorrCoef(downsampled + 60 + iblock * ENH_BLOCKL_HALF,
                          downsampled + 60 + iblock * ENH_BLOCKL_HALF - lag, ENH_BLOCKL_HALF);
        for (ilag = 11; ilag < 60; ilag++) {
            cc = xCorrCoef(downsampled + 60 + iblock * ENH_BLOCKL_HALF,
                           downsampled + 60 + iblock * ENH_BLOCKL_HALF - ilag, ENH_BLOCKL_HALF);
    ...
    if (iLBCdec_inst->prev_enh_pl == 1) {
        ...
        for (i = 0; i < plc_blockl; i++) {
            ftmp2 +=
                enh_buf[ENH_BUFL - 1 - iLBCdec_inst->blockl - i] * enh_buf[ENH_BUFL - 1 - iLBCdec_inst->blockl - i];
        ...
        if (ftmp1 > (float)2.0 * ftmp2 && ftmp1 > 0.0) {
            ...
            for (i = plc_blockl - 10; i < plc_blockl; i++) {
                plc_pred[i] *= (float)(i - plc_blockl + 10) * ((float)1.0 - (float)2.0 * ftmp2 / ftmp1) / (float)(10) +
        ...
        for (i = 0; i < plc_blockl; i++) {
            ...
            *enh_bufPtr1 *= ftmp1;
            *enh_bufPtr1 += ((float)1.0 - ftmp1) * plc_pred[plc_blockl - 1 - i];
            enh_bufPtr1--;
        }
    }
    ...
    if (iLBCdec_inst->mode == 20) {
        ...
        for (iblock = 0; iblock < 2; iblock++) {
            enhancer(out + iblock * ENH_BLOCKL, enh_buf, ENH_BUFL, (5 + iblock) * ENH_BLOCKL + 40, ENH_ALPHA0,
                     enh_period, enh_plocsTbl, ENH_NBLOCKS_TOT);
        }
    } else if (iLBCdec_inst->mode == 30) {
        /* Enhancer with 80 samples delay */
        for (iblock = 0; iblock < 3; iblock++) {
            enhancer(out + iblock * ENH_BLOCKL, enh_buf, ENH_BUFL, (4 + iblock) * ENH_BLOCKL, ENH_ALPHA0, enh_period,
                     enh_plocsTbl, ENH_NBLOCKS_TOT);
        }
    }
