...
)
{
    ...
    for (stage = 0; stage < nStages; stage++) {
        ...
        if (lTarget == SUBL) {
            ...
            searchAugmentedCB(20, 39, stage, base_size - lTarget / 2, target, buf + LPC_FILTERORDER + lMem,
        ...
        if ((lTarget == SUBL) && (sIndAug != 0)) {
            searchAugmentedCB(sIndAug, eIndAug, stage, 2 * base_size - 20, target, cbvectors + lMem, &max_measure,
                              &best_index, &gain, energy, invenergy);
        }
        ...
        if (lTarget == (STATE_LEN - iLBCenc_inst->state_short_len)) {
            ...
            if (index[stage] < base_size) {
                ...
                if (position < (base_size - 20)) {
                    ...
                    createAugmentedVec(index[stage] - (filterno + 1) * base_size + 40, cbvectors + filterno * lMem,
                                       aug_vec);
                    pp = aug_vec;
                }
            }
