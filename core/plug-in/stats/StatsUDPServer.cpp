...
int StatsUDPServer::execute(char *msg_buf, string &reply, struct sockaddr_in &addr)
{
    ...
    if (cmd_str == "calls")
        ...
        reply = "calls                              -  number of active calls (Session Container size)\n"
        ...
        if (cmd_str.substr(4, 8) == "loglevel") {
            if (!AmLcConfig::instance().setLogLevel(&cmd_str.c_str()[13]))
        ...
        try {
            ...
            AmArg args;
            for (vector<string>::iterator it = s_args.begin(); it != s_args.end(); it++) {
                args.push(it->c_str());
            ...
            reply = "Exception occured: AmDynInvoke::NotImplemented '" + e.what + "'\n";
...
int StatsUDPServer::send_reply(const string &reply, const struct sockaddr_in &reply_addr)
{
    int err =
        sendto(sd, reply.c_str(), reply.length() + 1, 0, (const sockaddr *)&reply_addr, sizeof(struct sockaddr_in));
