...
int Pcm16_2_Speex(unsigned char *out_buf, unsigned char *in_buf, unsigned int size, unsigned int channels,
                  unsigned int rate, long h_codec);
int Speex_2_Pcm16(unsigned char *out_buf, unsigned char *in_buf, unsigned int size, unsigned int channels,
                  unsigned int rate, long h_codec);
...
CODEC(CODEC_SPEEX_UB, Pcm16_2_Speex, Speex_2_Pcm16, AMCI_NO_CODEC_PLC, speexUB_create, speex_destroy, NULL, NULL)
#endif
#if SYSTEM_SAMPLECLOCK_RATE >= 16000
CODEC(CODEC_SPEEX_WB, Pcm16_2_Speex, Speex_2_Pcm16, AMCI_NO_CODEC_PLC, speexWB_create, speex_destroy, NULL, NULL)
#endif
CODEC(CODEC_SPEEX_NB, Pcm16_2_Speex, Speex_2_Pcm16, AMCI_NO_CODEC_PLC, speexNB_create, speex_destroy, NULL, NULL)
END_CODECS
...
long speex_create(unsigned int sample_rate, const char *format_parameters, amci_codec_fmt_info_t *format_description)
{
    ...
    case 32000:
        ...
        break;
#endif
    default: ERROR("Unsupported sample rate for Speex codec (%u)", sample_rate); return 0;
    }
    ...
    */
    ...
    DBG("SpeexState %p inserted with %d frames per packet,", ss, ss->frames_per_packet);
                  ...
                  unsigned int rate, long h_codec)
{
    ...
    if (blocks.rem) {
        ERROR("Pcm16_2_Speex: not integral number of blocks %d.%d", blocks.quot, blocks.rem);
        return -1;
    }
                  ...
                  unsigned int rate, long h_codec)
{
    ...
    while (speex_bits_remaining(&ss->decoder.bits) > 40) {
        ...
        if (((frames_out + 1) * ss->frame_size * sizeof(short)) > AUDIO_BUFFER_SIZE) {
            ERROR("speex decoder is going to overflow output buffer. "
                  "skip subframe decoding and return actually decoded samples count");
