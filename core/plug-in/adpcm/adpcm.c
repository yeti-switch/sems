...
static int Pcm16_2_G726_16(unsigned char *out_buf, unsigned char *in_buf, unsigned int size, unsigned int channels,
                           unsigned int rate, long h_codec);
static int G726_16_2_Pcm16(unsigned char *out_buf, unsigned char *in_buf, unsigned int size, unsigned int channels,
                           unsigned int rate, long h_codec);

static int Pcm16_2_G726_24(unsigned char *out_buf, unsigned char *in_buf, unsigned int size, unsigned int channels,
                           unsigned int rate, long h_codec);
static int G726_24_2_Pcm16(unsigned char *out_buf, unsigned char *in_buf, unsigned int size, unsigned int channels,
                           unsigned int rate, long h_codec);

static int Pcm16_2_G726_32(unsigned char *out_buf, unsigned char *in_buf, unsigned int size, unsigned int channels,
                           unsigned int rate, long h_codec);
static int G726_32_2_Pcm16(unsigned char *out_buf, unsigned char *in_buf, unsigned int size, unsigned int channels,
                           unsigned int rate, long h_codec);

static int Pcm16_2_G726_40(unsigned char *out_buf, unsigned char *in_buf, unsigned int size, unsigned int channels,
                           unsigned int rate, long h_codec);
static int G726_40_2_Pcm16(unsigned char *out_buf, unsigned char *in_buf, unsigned int size, unsigned int channels,
                           unsigned int rate, long h_codec);
...
BEGIN_CODECS
CODEC(CODEC_G726_16, Pcm16_2_G726_16, G726_16_2_Pcm16, AMCI_NO_CODEC_PLC, G726_create, G726_destroy,
      g726_16_bytes2samples, g726_16_samples2bytes)
CODEC(CODEC_G726_24, Pcm16_2_G726_24, G726_24_2_Pcm16, AMCI_NO_CODEC_PLC, G726_create, G726_destroy,
      g726_24_bytes2samples, g726_24_samples2bytes)
CODEC(CODEC_G726_32, Pcm16_2_G726_32, G726_32_2_Pcm16, AMCI_NO_CODEC_PLC, G726_create, G726_destroy,
      g726_32_bytes2samples, g726_32_samples2bytes)
CODEC(CODEC_G726_40, Pcm16_2_G726_40, G726_40_2_Pcm16, AMCI_NO_CODEC_PLC, G726_create, G726_destroy,
      g726_40_bytes2samples, g726_40_samples2bytes)
END_CODECS
                           ...
                           unsigned int rate, long h_codec)
{
    ...
    for (i = 0; i < PCM16_B2S(size); i += 4) {
        out_buf[i >> 2] = 0;
        for (j = 0; j < 4; j++) {
            char v = g723_16_encoder(sample_buf[i + j], AUDIO_ENCODING_LINEAR, &cs->to_g726);
...
#endif
            pcm_buf[4 * i + j] = g723_16_decoder(w, AUDIO_ENCODING_LINEAR, &cs->from_g726);
        }
    }
                           ...
                           unsigned int rate, long h_codec)
{
    ...
    for (i = 0; i < PCM16_B2S(size); i += 8) {
        ...
        for (j = 0; j < 8; j++) {
#ifdef G726_PACK_RFC3551
            v |= (g723_24_encoder(sample_buf[i + j], AUDIO_ENCODING_LINEAR, &cs->to_g726)) << (j * 3);
#else
            v |= (g723_24_encoder(sample_buf[i + j], AUDIO_ENCODING_LINEAR, &cs->to_g726)) << ((7 - j) * 3);
#endif
        }
                           ...
                           unsigned int rate, long h_codec)
{
    ...
    for (i = 0; i < size; i += 3) {
        u_int32_t v = ((in_buf[i + 2]) << 16) | ((in_buf[i + 1]) << 8) | (in_buf[i]);
        ...
        for (j = 0; j < 8; j++) {
            ...
            w = (v >> ((7 - j) * 3)) & 0x7;
#endif
            pcm_buf[8 * (i / 3) + j] = g723_24_decoder(w, AUDIO_ENCODING_LINEAR, &cs->from_g726);
        }
    }
                           ...
                           unsigned int rate, long h_codec)
{
    ...
    for (i = 0; i < PCM16_B2S(size); i += 2) {
        out_buf[i >> 1] = 0;
        for (j = 0; j < 2; j++) {
            char v = g721_encoder(sample_buf[i + j], AUDIO_ENCODING_LINEAR, &cs->to_g726);
...
#endif
            pcm_buf[2 * i + j] = g721_decoder(w, AUDIO_ENCODING_LINEAR, &cs->from_g726);
        }
    }
                           ...
                           unsigned int rate, long h_codec)
{
    ...
    for (i = 0; i < PCM16_B2S(size); i += 8) {
        ...
        for (j = 0; j < 8; j++) {
#ifdef G726_PACK_RFC3551
            v |= ((u_int64_t)g723_40_encoder(sample_buf[i + j], AUDIO_ENCODING_LINEAR, &cs->to_g726)) << (j * 5);
#else
            v |= ((u_int64_t)g723_40_encoder(sample_buf[i + j], AUDIO_ENCODING_LINEAR, &cs->to_g726)) << ((7 - j) * 5);
#endif
        }
                           ...
                           unsigned int rate, long h_codec)
{
    ...
    for (i = 0; i < size; i += 5) {
        v = ((u_int64_t)in_buf[i + 4]) << 32 | (u_int64_t)(in_buf[i + 3]) << 24 | (u_int64_t)(in_buf[i + 2]) << 16 |
            (u_int64_t)(in_buf[i + 1]) << 8 | (u_int64_t)(in_buf[i]);
        ...
        for (j = 0; j < 8; j++) {
            ...
            w = (v >> ((7 - j) * 5)) & 0x1f;
#endif
            pcm_buf[8 * (i / 5) + j] = g723_40_decoder(w, AUDIO_ENCODING_LINEAR, &cs->from_g726);
        }
    }
