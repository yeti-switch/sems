...
int Pcm16_2_iSAC(unsigned char *out_buf, unsigned char *in_buf, unsigned int size, unsigned int channels,
                 unsigned int rate, long h_codec);
int iSAC_2_Pcm16(unsigned char *out_buf, unsigned char *in_buf, unsigned int size, unsigned int channels,
                 unsigned int rate, long h_codec);

static long iSAC_create(const char *format_parameters, amci_codec_fmt_info_t *format_description);
...
CODEC(CODEC_iSAC_WB, Pcm16_2_iSAC, iSAC_2_Pcm16, AMCI_NO_CODEC_PLC, iSAC_create, iSAC_destroy, iSAC_bytes2samples,
      iSAC_samples2bytes)
#endif
END_CODECS
...
BEGIN_PAYLOADS
#if SYSTEM_SAMPLECLOCK_RATE >= 16000
PAYLOAD(-1, "isac", iSAC_SAMPLE_RATE, iSAC_SAMPLE_RATE, 1, CODEC_iSAC_WB, AMCI_PT_AUDIO_FRAME)
#endif
END_PAYLOADS
