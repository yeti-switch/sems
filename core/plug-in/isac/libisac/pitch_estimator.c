                                ...
                                PitchAnalysisStruct *State, double *lags)
{
    ...
    memcpy(buf_dec, State->dec_buffer,
           sizeof(double) * (PITCH_CORR_LEN2 + PITCH_CORR_STEP2 + PITCH_MAX_LAG / 2 - PITCH_FRAME_LEN / 2 + 2));
    ...
    /* low-pass filtering */
    for (k = PITCH_CORR_LEN2 + PITCH_CORR_STEP2 + PITCH_MAX_LAG / 2 - PITCH_FRAME_LEN / 2 + 2;
