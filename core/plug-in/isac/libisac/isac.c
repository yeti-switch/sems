...
static void UpdatePayloadSizeLimit(ISACMainStruct *instISAC)
{
    ...
    lim30MsPayloadBytes = WEBRTC_SPL_MIN((instISAC->maxPayloadSizeBytes), (instISAC->maxRateBytesPer30Ms));

    lim60MsPayloadBytes = WEBRTC_SPL_MIN((instISAC->maxPayloadSizeBytes), (instISAC->maxRateBytesPer30Ms << 1));
    ...
    if (instISAC->bandwidthKHz == isac8kHz) {
        ...
        // limit.
        instISAC->instLB.ISACencLB_obj.payloadLimitBytes60 = lim60MsPayloadBytes;
        instISAC->instLB.ISACencLB_obj.payloadLimitBytes30 = lim30MsPayloadBytes;
    } else {
        ...
        if (lim30MsPayloadBytes > 250) {
            ...
            instISAC->instLB.ISACencLB_obj.payloadLimitBytes30 = (lim30MsPayloadBytes << 2) / 5;
        } else if (lim30MsPayloadBytes > 200) {
            // for the interval of 200 to 250 the share of
            // upper-band linearly grows from 20 to 50;
            instISAC->instLB.ISACencLB_obj.payloadLimitBytes30 = (lim30MsPayloadBytes << 1) / 5 + 100;
        } else {
            // allocate only 20 for upper-band
            instISAC->instLB.ISACencLB_obj.payloadLimitBytes30 = lim30MsPayloadBytes - 20;
        }
        instISAC->instUB.ISACencUB_obj.maxPayloadSizeBytes = lim30MsPayloadBytes;
    }
}
...
static void UpdateBottleneck(ISACMainStruct *instISAC)
{
    ...
    if ((instISAC->codingMode == 0) && (instISAC->instLB.ISACencLB_obj.buffer_index == 0) &&
        (instISAC->instLB.ISACencLB_obj.frame_nb == 0))
    {
        ...
        if ((instISAC->bandwidthKHz == isac8kHz) && (bottleneck > 37000) && (bottleneck < 41000)) {
            bottleneck = 37000;
        }
        ...
        // the last LPC vector.
        if ((instISAC->bandwidthKHz != isac16kHz) && (bottleneck > 46000)) {
            bottleneck = 46000;
        }
        ...
        if (instISAC->encoderSamplingRateKHz == kIsacWideband) {
            ...
            instISAC->instLB.ISACencLB_obj.bottleneck = (bottleneck > 32000) ? 32000 : bottleneck;
            instISAC->bandwidthKHz                    = isac8kHz;
        } else {
            // do the rate-allosation and get the new bandwidth.
            enum ISACBandwidth bandwidth;
            WebRtcIsac_RateAllocation(bottleneck, &(instISAC->instLB.ISACencLB_obj.bottleneck),
                                      &(instISAC->instUB.ISACencUB_obj.bottleneck), &bandwidth);
 ...
 *
 */
static void GetSendBandwidthInfo(ISACMainStruct *instISAC, WebRtc_Word16 *bandwidthIndex, WebRtc_Word16 *jitterInfo)
{
    if ((instISAC->instLB.ISACencLB_obj.buffer_index == (FRAMESAMPLES_10ms << 1)) &&
        (instISAC->instLB.ISACencLB_obj.frame_nb == 0))
    {
        /* bandwidth estimation and coding */
        WebRtcIsac_GetDownlinkBwJitIndexImpl(&(instISAC->bwestimator_obj), bandwidthIndex, jitterInfo,
 ...
 *                             -1 - Error
 */
WebRtc_Word16 WebRtcIsac_Assign(ISACStruct **ISAC_main_inst, void *instISAC_Addr)
{
 ...
 *                              -1 - Error
 */
static WebRtc_Word16 EncoderInitLb(ISACLBStruct *instLB, WebRtc_Word16 codingMode, enum IsacSamplingRate sampRate)
{
...
static WebRtc_Word16 EncoderInitUb(ISACUBStruct *instUB, WebRtc_Word16 bandwidth)
{
    WebRtc_Word16 statusInit = 0;
    int           k;
    ...
    memset(instUB->ISACencUB_obj.data_buffer_float, 0, (MAX_FRAMESAMPLES + LB_TOTAL_DELAY_SAMPLES) * sizeof(float));

    memcpy(&(instUB->ISACencUB_obj.lastLPCVec), WebRtcIsac_kMeanLarUb16, sizeof(double) * UB_LPC_ORDER);

    return statusInit;
}


WebRtc_Word16 WebRtcIsac_EncoderInit(ISACStruct *ISAC_main_inst, WebRtc_Word16 codingMode)
{
    ISACMainStruct *instISAC;
    ...
    WebRtcIsac_InitBandwidthEstimator(&instISAC->bwestimator_obj, instISAC->encoderSamplingRateKHz,
                                      instISAC->decoderSamplingRateKHz);
    ...
    status = EncoderInitLb(&instISAC->instLB, codingMode, instISAC->encoderSamplingRateKHz);
    if (status < 0) {
    ...
    if (instISAC->encoderSamplingRateKHz == kIsacSuperWideband) {
        ...
        memset(instISAC->analysisFBState1, 0, FB_STATE_SIZE_WORD32 * sizeof(WebRtc_Word32));
        memset(instISAC->analysisFBState2, 0, FB_STATE_SIZE_WORD32 * sizeof(WebRtc_Word32));

        status = EncoderInitUb(&(instISAC->instUB), instISAC->bandwidthKHz);
        if (status < 0) {
            instISAC->errorCode = -status;
            return -1;
        }
 ...
 *                            : -1 - Error
 */
WebRtc_Word16 WebRtcIsac_Encode(ISACStruct *ISAC_main_inst, const WebRtc_Word16 *speechIn, WebRtc_Word16 *encoded)
{
    ISACMainStruct *instISAC;
    ISACLBStruct   *instLB;
    ISACUBStruct   *instUB;
    ...
    if (instISAC->encoderSamplingRateKHz == kIsacSuperWideband) {
        WebRtcSpl_AnalysisQMF(speechIn, speechInLB, speechInUB, instISAC->analysisFBState1, instISAC->analysisFBState2);
    ...
    // ENCODE LOWER-BAND
    //
    streamLenLB = WebRtcIsac_EncodeLb(inFrame, &instLB->ISACencLB_obj, instISAC->codingMode, bottleneckIdx);
    ...
    if (instISAC->encoderSamplingRateKHz == kIsacSuperWideband) {
        ...
        instUB->ISACencUB_obj.numBytesUsed = streamLenLB + 1 + LEN_CHECK_SUM_WORD8;
        ...
        case isac12kHz:
        {
            streamLenUB = WebRtcIsac_EncodeUb12(inFrame, &instUB->ISACencUB_obj, jitterInfo);
            break;
        }
        case isac16kHz:
        {
            streamLenUB = WebRtcIsac_EncodeUb16(inFrame, &instUB->ISACencUB_obj, jitterInfo);
            break;
        }
        case isac8kHz:
        {
        ...
        // to 255.
        if ((streamLenUB > (255 - (LEN_CHECK_SUM_WORD8 + 1))) || (streamLenUB == -ISAC_PAYLOAD_LARGER_THAN_LIMIT)) {
        ...
        if (streamLenUB > 0) {
            ptrEncodedUW8[streamLenLB] = (WebRtc_UWord8)(streamLenUB + 1 + LEN_CHECK_SUM_WORD8);
    ...
    if (instISAC->codingMode == 0) {
        ...
        instISAC->MaxDelay = (double)WebRtcIsac_GetUplinkMaxDelay(&instISAC->bwestimator_obj);

        /* update rate model and get minimum number of bytes in this packet */
        minBytes = WebRtcIsac_GetMinBytes(&(instISAC->rate_data_obj), streamLen,
    ...
    if ((instISAC->bandwidthKHz != isac8kHz) && (streamLenUB > 0)) {
        ...
        WebRtcIsac_GetCrc((WebRtc_Word16 *)(&(ptrEncodedUW8[streamLenLB + 1])), streamLenUB + garbageLen, &crc);
#ifndef WEBRTC_BIG_ENDIAN
        for (k = 0; k < LEN_CHECK_SUM_WORD8; k++) {
            ptrEncodedUW8[streamLen - LEN_CHECK_SUM_WORD8 + k] = (WebRtc_UWord8)((crc >> (24 - k * 8)) & 0xFF);
        }
#else
 ...
 *                                 the struct.
 */
WebRtc_Word16 WebRtcIsac_GetNewBitStream(ISACStruct *ISAC_main_inst, WebRtc_Word16 bweIndex, WebRtc_Word16 jitterInfo,
                                         WebRtc_Word32 rate, WebRtc_Word16 *encoded, WebRtc_Word16 isRCU)
{
    ...
    if ((bandwidthKHz == isac12kHz) && (instISAC->bandwidthKHz == isac16kHz)) {
        return -1;
    }

    // These gains are in dB
    // gain for the given rate.
    gain1 = WebRtcIsac_GetSnr(rateLB, instISAC->instLB.ISACencLB_obj.current_framesamples);
    // gain of this iSAC
    gain2 = WebRtcIsac_GetSnr(instISAC->instLB.ISACencLB_obj.bottleneck,
    ...
    streamLenLB =
        WebRtcIsac_EncodeStoredDataLb(&instISAC->instLB.ISACencLB_obj.SaveEnc_obj, &iSACBitStreamInst, bweIndex, scale);
    ...
    // gain of this iSAC
    gain2 = WebRtcIsac_GetSnr(instISAC->instUB.ISACencUB_obj.bottleneck, FRAMESAMPLES);
    ...
    case isac12kHz:
    {
        streamLenUB = WebRtcIsac_EncodeStoredDataUb12(&(instISAC->instUB.ISACencUB_obj.SaveEnc_obj), &iSACBitStreamInst,
                                                      jitterInfo, scale);
        break;
    }
    case isac16kHz:
    {
        streamLenUB = WebRtcIsac_EncodeStoredDataUb16(&(instISAC->instUB.ISACencUB_obj.SaveEnc_obj), &iSACBitStreamInst,
                                                      jitterInfo, scale);
        break;
    }
    default: return -1;
    ...
    WebRtcIsac_GetCrc((WebRtc_Word16 *)(&(encodedPtrUW8[streamLenLB + 1])), streamLenUB, &crc);
#ifndef WEBRTC_BIG_ENDIAN
    for (k = 0; k < LEN_CHECK_SUM_WORD8; k++) {
        encodedPtrUW8[totalStreamLen - LEN_CHECK_SUM_WORD8 + k] = (WebRtc_UWord8)((crc >> (24 - k * 8)) & 0xFF);
    }
#else
    memcpy(&encodedPtrUW8[streamLenLB + streamLenUB + 1], &crc, LEN_CHECK_SUM_WORD8);
#endif
...
static WebRtc_Word16 DecoderInitLb(ISACLBStruct *instISAC)
{
    ...
    WebRtcIsac_InitPostFilterbank(&instISAC->ISACdecLB_obj.postfiltbankstr_obj);
...
static WebRtc_Word16 DecoderInitUb(ISACUBStruct *instISAC)
{
    ...
    WebRtcIsac_InitPostFilterbank(&instISAC->ISACdecUB_obj.postfiltbankstr_obj);
    return (0);
}
...
WebRtc_Word16 WebRtcIsac_DecoderInit(ISACStruct *ISAC_main_inst)
{
    ...
    if (instISAC->decoderSamplingRateKHz == kIsacSuperWideband) {
        memset(instISAC->synthesisFBState1, 0, FB_STATE_SIZE_WORD32 * sizeof(WebRtc_Word32));
        memset(instISAC->synthesisFBState2, 0, FB_STATE_SIZE_WORD32 * sizeof(WebRtc_Word32));
 ...
 *                              -1 - Error
 */
WebRtc_Word16 WebRtcIsac_UpdateBwEstimate(ISACStruct *ISAC_main_inst, const WebRtc_UWord16 *encoded,
                                          WebRtc_Word32 packet_size, WebRtc_UWord16 rtp_seq_number,
...
#ifndef WEBRTC_BIG_ENDIAN
    for (k = 0; k < 10; k++) {
        streamdata.stream[k] = (WebRtc_UWord8)((encoded[k >> 1] >> ((k & 1) << 3)) & 0xFF);
    }
#else
    memcpy(streamdata.stream, encoded, 10);
#endif

    err = WebRtcIsac_EstimateBandwidth(&instISAC->bwestimator_obj, &streamdata, packet_size, rtp_seq_number, send_ts,
                                       arr_ts, instISAC->encoderSamplingRateKHz, instISAC->decoderSamplingRateKHz);
...
static WebRtc_Word16 Decode(ISACStruct *ISAC_main_inst, const WebRtc_UWord16 *encoded, WebRtc_Word16 lenEncodedBytes,
                            WebRtc_Word16 *decoded, WebRtc_Word16 *speechType, WebRtc_Word16 isRCUPayload)
{
    ...
    lenEncodedLBBytes = (lenEncodedBytes > STREAM_SIZE_MAX) ? STREAM_SIZE_MAX : lenEncodedBytes;

    // Copy to lower-band bit-stream structure
    memcpy(instISAC->instLB.ISACdecLB_obj.bitstr_obj.stream, ptrEncodedUW8, lenEncodedLBBytes);

    // Regardless of that the current codec is setup to work in
    // wideband or super-wideband, the decoding of the lower-band
    // has to be performed.
    numDecodedBytesLB = WebRtcIsac_DecodeLb(outFrame, decInstLB, &numSamplesLB, isRCUPayload);

    // Check for error
    if ((numDecodedBytesLB < 0) || (numDecodedBytesLB > lenEncodedLBBytes) || (numSamplesLB > MAX_FRAMESAMPLES)) {
    ...
    while (totSizeBytes != lenEncodedBytes) {
        if ((totSizeBytes > lenEncodedBytes) || (ptrEncodedUW8[totSizeBytes] == 0) || (numLayer > MAX_NUM_LAYERS)) {
            instISAC->errorCode = ISAC_LENGTH_MISMATCH;
            return -1;
        }
    ...
    if (instISAC->decoderSamplingRateKHz == kIsacWideband) {
        ...
        if (numDecodedBytesLB == lenEncodedBytes) {
            ...
            if (lenNextStream <= (LEN_CHECK_SUM_WORD8 + 1)) {
                ...
                WebRtcIsac_GetCrc((WebRtc_Word16 *)(&ptrEncodedUW8[numDecodedBytesLB + 1]),
                                  lenNextStream - LEN_CHECK_SUM_WORD8 - 1, &crc);

                validChecksum = 1;
                for (k = 0; k < LEN_CHECK_SUM_WORD8; k++) {
                    validChecksum &= (((crc >> (24 - k * 8)) & 0xFF) ==
                                      ptrEncodedUW8[numDecodedBytesLB + lenNextStream - LEN_CHECK_SUM_WORD8 + k]);
                }
            }
            ...
            if (!validChecksum) {
                ...
                memcpy(decInstUB->bitstr_obj.stream, &ptrEncodedUW8[numDecodedBytesLB + 1], lenNextStream);
                ...
                if (instISAC->encoderSamplingRateKHz == kIsacSuperWideband) {
                    err = WebRtcIsac_UpdateUplinkJitter(&(instISAC->bwestimator_obj), maxDelayBit);
                    if (err < 0) {
                        instISAC->errorCode = -err;
                        return -1;
                    }
                ...
                err = WebRtcIsac_DecodeBandwidth(&decInstUB->bitstr_obj, &bandwidthKHz);
                if (err < 0) {
                    instISAC->errorCode = -err;
                    return -1;
                }
                ...
                case isac12kHz:
                {
                    numDecodedBytesUB = WebRtcIsac_DecodeUb12(outFrame, decInstUB, isRCUPayload);
                    ...
                    if (instISAC->resetFlag_8kHz > 0) {
                        if (instISAC->resetFlag_8kHz == 2) {
                            ...
                            memset(outFrame, 0, MAX_FRAMESAMPLES * sizeof(float));
                        } else {
                            const float rampStep = 2.0f / MAX_FRAMESAMPLES;
                            float       rampVal  = 0;
                            memset(outFrame, 0, (MAX_FRAMESAMPLES >> 1) * sizeof(float));
                ...
                case isac16kHz:
                {
                    numDecodedBytesUB = WebRtcIsac_DecodeUb16(outFrame, decInstUB, isRCUPayload);
                    break;
                }
                default: return -1;
                }
                ...
                if ((numDecodedBytesUB != lenNextStream) &&
                    (numDecodedBytesUB != (lenNextStream - ptrEncodedUW8[numDecodedBytesLB + 1 + numDecodedBytesUB])))
                {
                    instISAC->errorCode = ISAC_LENGTH_MISMATCH;
                    return -1;
                }
                ...
                for (k = 0; k < numSamplesUB; k++) {
                    if (outFrame[k] > 32767) {
                        ...
                        outFrameUB[k] = (WebRtc_Word16)WebRtcIsac_lrint(outFrame[k]);
                    }
                }
            }
        }
        ...
        while (speechIdx < numSamplesLB) {
            WebRtcSpl_SynthesisQMF(&outFrameLB[speechIdx], &outFrameUB[speechIdx], &decoded[(speechIdx << 1)],
...
WebRtc_Word16 WebRtcIsac_Decode(ISACStruct *ISAC_main_inst, const WebRtc_UWord16 *encoded,
                                WebRtc_Word16 lenEncodedBytes, WebRtc_Word16 *decoded, WebRtc_Word16 *speechType)
{
    WebRtc_Word16 isRCUPayload = 0;
    return Decode(ISAC_main_inst, encoded, lenEncodedBytes, decoded, speechType, isRCUPayload);
}
...
WebRtc_Word16 WebRtcIsac_DecodeRcu(ISACStruct *ISAC_main_inst, const WebRtc_UWord16 *encoded,
                                   WebRtc_Word16 lenEncodedBytes, WebRtc_Word16 *decoded, WebRtc_Word16 *speechType)
{
 ...
 *                              -1 - Error
 */
WebRtc_Word16 WebRtcIsac_DecodePlc(ISACStruct *ISAC_main_inst, WebRtc_Word16 *decoded, WebRtc_Word16 noOfLostFrames)
{
 ...
 *                             -1 - Error
 */
static WebRtc_Word16 ControlLb(ISACLBStruct *instISAC, double rate, WebRtc_Word16 frameSize)
{
...
WebRtc_Word16 WebRtcIsac_Control(ISACStruct *ISAC_main_inst, WebRtc_Word32 bottleneckBPS, WebRtc_Word16 frameSize)
{
    ISACMainStruct    *instISAC;
    WebRtc_Word16      status;
    double             rateLB;
    ...
    if (instISAC->encoderSamplingRateKHz == kIsacWideband) {
        ...
        if (WebRtcIsac_RateAllocation(bottleneckBPS, &rateLB, &rateUB, &bandwidthKHz) < 0) {
            return -1;
        }
    }

    if ((instISAC->encoderSamplingRateKHz == kIsacSuperWideband) && (frameSize != 30) && (bandwidthKHz != isac8kHz)) {
    ...
    if ((instISAC->bandwidthKHz == isac8kHz) && (bandwidthKHz != isac8kHz)) {
        ...
        if (bandwidthKHz == isac12kHz) {
            instISAC->instUB.ISACencUB_obj.buffer_index = instISAC->instLB.ISACencLB_obj.buffer_index;
        } else {
            instISAC->instUB.ISACencUB_obj.buffer_index =
                LB_TOTAL_DELAY_SAMPLES + instISAC->instLB.ISACencLB_obj.buffer_index;

            memcpy(&(instISAC->instUB.ISACencUB_obj.lastLPCVec), WebRtcIsac_kMeanLarUb16,
 ...
 *                             -1 - Error
 */
WebRtc_Word16 WebRtcIsac_ControlBwe(ISACStruct *ISAC_main_inst, WebRtc_Word32 bottleneckBPS, WebRtc_Word16 frameSizeMs,
                                    WebRtc_Word16 enforceFrameSize)
{
    ...
    if ((frameSizeMs != 30) && (instISAC->encoderSamplingRateKHz == kIsacSuperWideband)) {
        return -1;
    }
    ...
    if (frameSizeMs != 0) {
        if ((frameSizeMs == 30) || (frameSizeMs == 60)) {
            instISAC->instLB.ISACencLB_obj.new_framelength = (FS / 1000) * frameSizeMs;
 ...
 *
 */
WebRtc_Word16 WebRtcIsac_GetDownLinkBwIndex(ISACStruct *ISAC_main_inst, WebRtc_Word16 *bweIndex,
                                            WebRtc_Word16 *jitterInfo)
{
    ...
    WebRtcIsac_GetDownlinkBwJitIndexImpl(&(instISAC->bwestimator_obj), bweIndex, jitterInfo,
                                         instISAC->decoderSamplingRateKHz);
    return 0;
}
 ...
 */
WebRtc_Word16 WebRtcIsac_UpdateUplinkBw(ISACStruct *ISAC_main_inst, WebRtc_Word16 bweIndex)
{
    ISACMainStruct *instISAC;
    ...
    /* Call function to get Bandwidth Estimate */
    returnVal = WebRtcIsac_UpdateUplinkBwImpl(&(instISAC->bwestimator_obj), bweIndex, instISAC->encoderSamplingRateKHz);
 ...
 *
 */
WebRtc_Word16 WebRtcIsac_ReadBwIndex(const WebRtc_Word16 *encoded, WebRtc_Word16 *bweIndex)
{
    Bitstr streamdata;
#ifndef WEBRTC_BIG_ENDIAN
    int k;
 ...
 *
 */
WebRtc_Word16 WebRtcIsac_ReadFrameLen(ISACStruct *ISAC_main_inst, const WebRtc_Word16 *encoded,
 ...
 */
WebRtc_Word16 WebRtcIsac_GetUplinkBw(ISACStruct *ISAC_main_inst, WebRtc_Word32 *bottleneck)
{
 ...
 *                             -1 if error happens
 */
WebRtc_Word16 WebRtcIsac_SetMaxPayloadSize(ISACStruct *ISAC_main_inst, WebRtc_Word16 maxPayloadBytes)
{
    ISACMainStruct *instISAC;
 ...
 *                             -1 if error happens
 */
WebRtc_Word16 WebRtcIsac_SetMaxRate(ISACStruct *ISAC_main_inst, WebRtc_Word32 maxRate)
{
    ISACMainStruct *instISAC;
 ...
 *
 */
WebRtc_Word16 WebRtcIsac_GetRedPayload(ISACStruct *ISAC_main_inst, WebRtc_Word16 *encoded)
{
    ISACMainStruct *instISAC;
    ...
    streamLenLB = WebRtcIsac_EncodeStoredDataLb(&instISAC->instLB.ISACencLB_obj.SaveEnc_obj, &iSACBitStreamInst,
                                                instISAC->instLB.ISACencLB_obj.lastBWIdx, RCU_TRANSCODING_SCALE);
    ...
    streamLenUB = WebRtcIsac_GetRedPayloadUb(&instISAC->instUB.ISACencUB_obj.SaveEnc_obj, &iSACBitStreamInst,
    ...
    if ((instISAC->bandwidthKHz != isac8kHz) && (streamLenUB > 0)) {
        ...
        WebRtcIsac_GetCrc((WebRtc_Word16 *)(&(ptrEncodedUW8[streamLenLB + 1])), streamLenUB, &crc);
#ifndef WEBRTC_BIG_ENDIAN
        for (k = 0; k < LEN_CHECK_SUM_WORD8; k++) {
            ptrEncodedUW8[streamLen - LEN_CHECK_SUM_WORD8 + k] = (WebRtc_UWord8)((crc >> (24 - k * 8)) & 0xFF);
        }
 ...
 *                             -1 if failed.
 */
WebRtc_Word16 WebRtcIsac_SetEncSampRate(ISACStruct *ISAC_main_inst, enum IsacSamplingRate sampRate)
{
    ISACMainStruct *instISAC;

    instISAC = (ISACMainStruct *)ISAC_main_inst;

    if ((sampRate != kIsacWideband) && (sampRate != kIsacSuperWideband)) {
        // Sampling Frequency is not supported
        instISAC->errorCode = ISAC_UNSUPPORTED_SAMPLING_FREQUENCY;
        return -1;
    } else if ((instISAC->initFlag & BIT_MASK_ENC_INIT) != BIT_MASK_ENC_INIT) {
        ...
        if ((sampRate == kIsacWideband) && (instISAC->encoderSamplingRateKHz == kIsacSuperWideband)) {
            // changing from super-wideband to wideband.
            // we don't need to re-initialize the encoder of the
            // lower-band.
            instISAC->bandwidthKHz = isac8kHz;
            if (codingMode == 1) {
                ControlLb(instLB, (bottleneck > 32000) ? 32000 : bottleneck, FRAMESIZE);
            }
            instISAC->maxPayloadSizeBytes = STREAM_SIZE_MAX_60;
            instISAC->maxRateBytesPer30Ms = STREAM_SIZE_MAX_30;
        } else if ((sampRate == kIsacSuperWideband) && (instISAC->encoderSamplingRateKHz == kIsacWideband)) {
            if (codingMode == 1) {
                WebRtcIsac_RateAllocation(bottleneck, &bottleneckLB, &bottleneckUB, &(instISAC->bandwidthKHz));
            }
            ...
            if (codingMode == 1) {
                ...
                ControlLb(instLB, bottleneckLB, (instISAC->bandwidthKHz == isac8kHz) ? frameSizeMs : FRAMESIZE);
                if (instISAC->bandwidthKHz > isac8kHz) {
 ...
 *                             -1 if failed.
 */
WebRtc_Word16 WebRtcIsac_SetDecSampRate(ISACStruct *ISAC_main_inst, enum IsacSamplingRate sampRate)
{
    ISACMainStruct *instISAC;
    ...
    if ((sampRate != kIsacWideband) && (sampRate != kIsacSuperWideband)) {
        ...
        if ((instISAC->decoderSamplingRateKHz == kIsacWideband) && (sampRate == kIsacSuperWideband)) {
