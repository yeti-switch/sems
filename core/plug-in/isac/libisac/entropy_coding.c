...
static const WebRtc_Word32 kRPointsQ10[100] = {
    14495, 14295, 14112, 13944, 13788, 13643, 13459, 13276, 13195, 13239, 13243, 13191, 13133, 13216, 13263,
    13330, 13316, 13242, 13191, 13106, 12942, 12669, 12291, 11840, 11361, 10795, 10192, 9561,  8934,  8335,
    7750,  7161,  6589,  6062,  5570,  5048,  4548,  4069,  3587,  3143,  2717,  2305,  1915,  1557,  1235,
    963,   720,   541,   423,   366,   369,   435,   561,   750,   1001,  1304,  1626,  1989,  2381,  2793,
    3219,  3656,  4134,  4612,  5106,  5629,  6122,  6644,  7216,  7801,  8386,  8987,  9630,  10255, 10897,
    11490, 11950, 12397, 12752, 12999, 13175, 13258, 13323, 13290, 13296, 13335, 13113, 13255, 13347, 13355,
    13298, 13247, 13313, 13155, 13267, 13313, 13374, 13446, 13525, 13609
};


/* cdf array for encoder bandwidth (12 vs 16 kHz) indicator */
static const WebRtc_UWord16 kOneBitEqualProbCdf[3] = { 0, 32768, 65535 };

/* pointer to cdf array for encoder bandwidth (12 vs 16 kHz) indicator */
static const WebRtc_UWord16 *kOneBitEqualProbCdf_ptr[1] = { kOneBitEqualProbCdf };
...
static void WebRtcIsac_FindInvArSpec(const WebRtc_Word16 *ARCoefQ12, const WebRtc_Word32 gainQ10,
                                     WebRtc_Word32 *CurveQ16)
{
    ...
    for (k = 1; k < AR_ORDER; k += 2) {
        // CS_ptrQ9 = WebRtcIsac_kCos[k];
        for (n = 0; n < FRAMESAMPLES / 8; n++)
            CurveQ16[n] += WEBRTC_SPL_RSHIFT_W32(WEBRTC_SPL_MUL(WebRtcIsac_kCos[k][n], CorrQ11[k + 1]) + 2, 2);
    }
    ...
    for (n = 0; n < FRAMESAMPLES / 8; n++)
        diffQ16[n] =
            WEBRTC_SPL_RSHIFT_W32(WEBRTC_SPL_MUL(CS_ptrQ9[n], WEBRTC_SPL_RSHIFT_W32(CorrQ11[1], shftVal)) + 2, 2);
    for (k = 2; k < AR_ORDER; k += 2) {
        CS_ptrQ9 = WebRtcIsac_kCos[k];
        for (n = 0; n < FRAMESAMPLES / 8; n++)
            diffQ16[n] += WEBRTC_SPL_RSHIFT_W32(
                WEBRTC_SPL_MUL(CS_ptrQ9[n], WEBRTC_SPL_RSHIFT_W32(CorrQ11[k + 1], shftVal)) + 2, 2);
    }
...
static void GenerateDitherQ7Lb(WebRtc_Word16 *bufQ7, WebRtc_UWord32 seed, int length, WebRtc_Word16 AvgPitchGain_Q12)
{
    int           k, shft;
 ...
 */
static void GenerateDitherQ7LbUB(WebRtc_Word16 *bufQ7, WebRtc_UWord32 seed, int length)
{
    int k;
    for (k = 0; k < length; k++) {
        /* new random unsigned int */
 ...
 */
int WebRtcIsac_DecodeSpecLb(Bitstr *streamdata, double *fr, double *fi, WebRtc_Word16 AvgPitchGain_Q12)
{
    ...
    if ((len = WebRtcIsac_DecLogisticMulti2(data, streamdata, invARSpecQ8, DitherQ7, FRAMESAMPLES, !IS_SWB_12KHZ)) < 1)
        return -ISAC_RANGE_ERROR_DECODE_SPECTRUM;
 ...
 *                              0 if succeeded.
 */
int WebRtcIsac_DecodeSpecUB16(Bitstr *streamdata, double *fr, double *fi)
{
    WebRtc_Word16  DitherQ7[FRAMESAMPLES];
    WebRtc_Word16  data[FRAMESAMPLES];
    WebRtc_Word32  invARSpec2_Q16[FRAMESAMPLES_QUARTER];
 ...
 *                              0 if succeeded.
 */
int WebRtcIsac_DecodeSpecUB12(Bitstr *streamdata, double *fr, double *fi)
{
    WebRtc_Word16  DitherQ7[FRAMESAMPLES];
    WebRtc_Word16  data[FRAMESAMPLES];
    WebRtc_Word32  invARSpec2_Q16[FRAMESAMPLES_QUARTER];
    ...
    /* arithmetic decoding of spectrum */
    if ((len = WebRtcIsac_DecLogisticMulti2(data, streamdata, invARSpecQ8, DitherQ7, (FRAMESAMPLES_HALF),
                                            IS_SWB_12KHZ)) < 1)
    {
        return -ISAC_RANGE_ERROR_DECODE_SPECTRUM;
    }
...
int WebRtcIsac_EncodeSpecLb(const WebRtc_Word16 *fr, const WebRtc_Word16 *fi, Bitstr *streamdata,
                            WebRtc_Word16 AvgPitchGain_Q12)
{
    ...
    err = WebRtcIsac_EncLogisticMulti2(streamdata, dataQ7, invARSpecQ8, FRAMESAMPLES, !IS_SWB_12KHZ);
    if (err < 0) {
        return (err);
    }
 ...
 *                              0 if succeeded.
 */
int WebRtcIsac_EncodeSpecUB16(const WebRtc_Word16 *fr, const WebRtc_Word16 *fi, Bitstr *streamdata)
{
    WebRtc_Word16  ditherQ7[FRAMESAMPLES];
    WebRtc_Word16  dataQ7[FRAMESAMPLES];
    WebRtc_Word32  PSpec[FRAMESAMPLES_QUARTER];
    ...
    for (j = 0, k = 0; k < FRAMESAMPLES; k += 4, j++) {
        ...
        val           = ((fr[(FRAMESAMPLES_HALF)-1 - j] + ditherQ7[k + 2] + 64) & 0xFF80) - ditherQ7[k + 2];
        dataQ7[k + 2] = val;
        sum += val * val;

        val           = ((fi[(FRAMESAMPLES_HALF)-1 - j] + ditherQ7[k + 3] + 64) & 0xFF80) - ditherQ7[k + 3];
        dataQ7[k + 3] = val;
        sum += val * val;
...
int WebRtcIsac_EncodeSpecUB12(const WebRtc_Word16 *fr, const WebRtc_Word16 *fi, Bitstr *streamdata)
{
    WebRtc_Word16  ditherQ7[FRAMESAMPLES];
    WebRtc_Word16  dataQ7[FRAMESAMPLES];
    WebRtc_Word32  PSpec[FRAMESAMPLES_QUARTER];
    ...
    /* arithmetic coding of spectrum */
    err = WebRtcIsac_EncLogisticMulti2(streamdata, dataQ7, invARSpecQ8, (FRAMESAMPLES_HALF), IS_SWB_12KHZ);
    if (err < 0) {
        return (err);
    }
                                   ...
                                   int numPolyVecs) // includes the first and the last point of the interval
{
    ...
    for (coeffCntr = 0; coeffCntr < UB_LPC_ORDER; coeffCntr++) {
        delta[coeffCntr] = (larVecs[UB_LPC_ORDER + coeffCntr] - larVecs[coeffCntr]) / (numPolyVecs - 1);
    }

    for (polyCntr = 0; polyCntr < numPolyVecs; polyCntr++) {
        for (coeffCntr = 0; coeffCntr < UB_LPC_ORDER; coeffCntr++) {
            larInterpol[coeffCntr] = larVecs[coeffCntr] + delta[coeffCntr] * polyCntr;
        }
...
WebRtc_Word16 WebRtcIsac_DecodeInterpolLpcUb(Bitstr *streamdata, double *percepFilterParams, WebRtc_Word16 bandwidth)
{
    ...
    err = WebRtcIsac_DecodeLpcCoefUB(streamdata, lpcCoeff, percepFilterGains, bandwidth);
    ...
    for (interpolCntr = 0; interpolCntr < numSegments; interpolCntr++) {
        WebRtcIsac_Lar2PolyInterpolUB(&lpcCoeff[interpolCntr * UB_LPC_ORDER], ptrOutParam, numVecPerSegment + 1);
...
int WebRtcIsac_DecodeLpcCoef(Bitstr *streamdata, double *LPCCoef, int *outmodel)
{
    ...
    err = WebRtcIsac_DecHistOneStepMulti(&model, streamdata, WebRtcIsac_kQKltModelCdfPtr,
                                         WebRtcIsac_kQKltModelInitIndex, 1);
    if (err < 0) // error check
        return err;

    /* entropy decoding of quantization indices */
    err = WebRtcIsac_DecHistOneStepMulti(index_s, streamdata, WebRtcIsac_kQKltCdfPtrShape[model],
                                         WebRtcIsac_kQKltInitIndexShape[model], KLT_ORDER_SHAPE);
    if (err < 0) // error check
        return err;
    err = WebRtcIsac_DecHistOneStepMulti(index_g, streamdata, WebRtcIsac_kQKltCdfPtrGain[model],
                                         WebRtcIsac_kQKltInitIndexGain[model], KLT_ORDER_GAIN);
    if (err < 0) // error check
        return err;
    ...
    for (k = 0; k < KLT_ORDER_SHAPE; k++) {
        tmpcoeffs_s[WebRtcIsac_kQKltSelIndShape[k]] =
            WebRtcIsac_kQKltLevelsShape[WebRtcIsac_kQKltOfLevelsShape[model] + WebRtcIsac_kQKltOffsetShape[model][k] +
                                        index_s[k]];
    }
    for (k = 0; k < KLT_ORDER_GAIN; k++) {
        tmpcoeffs_g[WebRtcIsac_kQKltSelIndGain[k]] =
            WebRtcIsac_kQKltLevelsGain[WebRtcIsac_kQKltOfLevelsGain[model] + WebRtcIsac_kQKltOffsetGain[model][k] +
                                       index_g[k]];
    }
...
void WebRtcIsac_EncodeLar(double *LPCCoef, int *model, double *size, Bitstr *streamdata, ISAC_SaveEncData_t *encData)
{
    ...
    {
        index_s[k] = (WebRtcIsac_lrint(tmpcoeffs_s[WebRtcIsac_kQKltSelIndShape[k]] / KLT_STEPSIZE)) +
                     WebRtcIsac_kQKltQuantMinShape[k]; // ATTN: ok?
        if (index_s[k] < 0)
            index_s[k] = 0;
    ...
    *size  = Bits;
    ...
    for (k = 0; k < KLT_ORDER_SHAPE; k++) {
        tmpcoeffs_s[WebRtcIsac_kQKltSelIndShape[k]] =
            WebRtcIsac_kQKltLevelsShape[WebRtcIsac_kQKltOfLevelsShape[bmodel] + index_ovr_s[k]];
    }
    /* inverse KLT  */
    /* left transform */ // Transpose matrix!
    offss = 0;
...
void WebRtcIsac_EncodeLpcLb(double *LPCCoef_lo, double *LPCCoef_hi, int *model, double *size, Bitstr *streamdata,
...
WebRtc_Word16 WebRtcIsac_EncodeLpcUB(double *lpcVecs, Bitstr *streamdata, double *interpolLPCCoeff,
                                     WebRtc_Word16 bandwidth, ISACUBSaveEncDataStruct *encData)
{
    ...
    case isac12kHz:
    {
        ...
        memcpy(encData->indexLPCShape, idx, UB_LPC_ORDER * UB_LPC_VEC_PER_FRAME * sizeof(int));
        WebRtcIsac_EncHistMulti(streamdata, idx, WebRtcIsac_kLpcShapeCdfMatUb12, UB_LPC_ORDER * UB_LPC_VEC_PER_FRAME);
        for (interpolCntr = 0; interpolCntr < UB_INTERPOL_SEGMENTS; interpolCntr++) {
            WebRtcIsac_Lar2PolyInterpolUB(lpcVecs, interpolLPCCoeff, kLpcVecPerSegmentUb12 + 1);
    ...
    {
        // Stor the indices to be used for multiple encoding.
        memcpy(encData->indexLPCShape, idx, UB_LPC_ORDER * UB16_LPC_VEC_PER_FRAME * sizeof(int));
        WebRtcIsac_EncHistMulti(streamdata, idx, WebRtcIsac_kLpcShapeCdfMatUb16, UB_LPC_ORDER * UB16_LPC_VEC_PER_FRAME);
        for (interpolCntr = 0; interpolCntr < UB16_INTERPOL_SEGMENTS; interpolCntr++) {
            WebRtcIsac_Lar2PolyInterpolUB(lpcVecs, interpolLPCCoeff, kLpcVecPerSegmentUb16 + 1);
            lpcVecs += UB_LPC_ORDER;
...
void WebRtcIsac_EncodeLpcGainLb(double *LPCCoef_lo, double *LPCCoef_hi, int model, Bitstr *streamdata,
                                ISAC_SaveEncData_t *encData)
{
    ...
    for (k = 0; k < KLT_ORDER_GAIN; k++) {
        ...
        /* find quantization levels for coefficients */
        tmpcoeffs_g[WebRtcIsac_kQKltSelIndGain[k]] =
            WebRtcIsac_kQKltLevelsGain[WebRtcIsac_kQKltOfLevelsGain[model] + index_ovr_g[k]];
...
WebRtc_Word16 WebRtcIsac_DecodeLpcGainUb(double *lpGains, Bitstr *streamdata)
{
    ...
    int    idx[UB_LPC_GAIN_DIM];
    int    err;
    err = WebRtcIsac_DecHistOneStepMulti(idx, streamdata, WebRtcIsac_kLpcGainCdfMat, WebRtcIsac_kLpcGainEntropySearch,
                                         UB_LPC_GAIN_DIM);
    if (err < 0) {
        return -1;
    }
...
int WebRtcIsac_DecodeRc(Bitstr *streamdata, WebRtc_Word16 *RCQ15)
{
    ...
    /* entropy decoding of quantization indices */
    err = WebRtcIsac_DecHistOneStepMulti(index, streamdata, WebRtcIsac_kQArRcCdfPtr, WebRtcIsac_kQArRcInitIndex,
                                         AR_ORDER);
    if (err < 0) // error check
        return err;
...
int WebRtcIsac_DecodeGain2(Bitstr *streamdata, WebRtc_Word32 *gainQ10)
{
    ...
    err = WebRtcIsac_DecHistOneStepMulti(&index, streamdata, WebRtcIsac_kQGainCdf_ptr, WebRtcIsac_kQGainInitIndex, 1);
    if (err < 0) // error check
        return err;
...
int WebRtcIsac_DecodePitchGain(Bitstr *streamdata, WebRtc_Word16 *PitchGains_Q12)
{
    ...
    *WebRtcIsac_kQPitchGainCdf_ptr = WebRtcIsac_kQPitchGainCdf;
    err = WebRtcIsac_DecHistBisectMulti(&index_comb, streamdata, WebRtcIsac_kQPitchGainCdf_ptr,
...
void WebRtcIsac_EncodePitchLag(double *PitchLags, WebRtc_Word16 *PitchGain_Q12, Bitstr *streamdata,
                               ISAC_SaveEncData_t *encData)
{
    int                    k, j;
    double                 StepSize;
    ...
    for (k = 0; k < 4; k++) {
        ...
        else if (index[k] > upper_limit[k])
            index[k] = upper_limit[k];
        index[k] -= lower_limit[k];
...
const WebRtc_UWord16 WebRtcIsac_kFrameLengthCdf[4] = { 0, 21845, 43690, 65535 };
...
int WebRtcIsac_DecodeFrameLen(Bitstr *streamdata, WebRtc_Word16 *framesamples)
{
    ...
    err = WebRtcIsac_DecHistOneStepMulti(&frame_mode, streamdata, WebRtcIsac_kFrameLengthCdf_ptr,
    ...
    case 2:
        *framesamples = 960; /* 60ms */
        break;
    default: err = -ISAC_DISALLOWED_FRAME_MODE_DECODER;
    }
...
int WebRtcIsac_EncodeFrameLen(WebRtc_Word16 framesamples, Bitstr *streamdata)
{
    ...
    switch (framesamples) {
    case 480: frame_mode = 1; break;
    case 960: frame_mode = 2; break;
    default:  status = -ISAC_DISALLOWED_FRAME_MODE_ENCODER;
    }
...
static const WebRtc_UWord16 kBwCdf[25] = { 0,     2731,  5461,  8192,  10923, 13653, 16384, 19114, 21845,
                                           24576, 27306, 30037, 32768, 35498, 38229, 40959, 43690, 46421,
                                           49151, 51882, 54613, 57343, 60074, 62804, 65535 };
...
void WebRtcIsac_TranscodeLPCCoef(double *LPCCoef_lo, double *LPCCoef_hi, int model, int *index_g)
{
...
/* decode & dequantize LPC Coef */
int WebRtcIsac_DecodeLpcCoefUB(Bitstr *streamdata, double *lpcVecs, double *percepFilterGains, WebRtc_Word16 bandwidth)
{
    ...
    {
        err =
            WebRtcIsac_DecHistOneStepMulti(index_s, streamdata, WebRtcIsac_kLpcShapeCdfMatUb12,
                                           WebRtcIsac_kLpcShapeEntropySearchUb12, UB_LPC_ORDER * UB_LPC_VEC_PER_FRAME);
        break;
    }
    case isac16kHz:
    {
        err = WebRtcIsac_DecHistOneStepMulti(index_s, streamdata, WebRtcIsac_kLpcShapeCdfMatUb16,
...
WebRtc_Word16 WebRtcIsac_EncodeBandwidth(enum ISACBandwidth bandwidth, Bitstr *streamData)
{
    int bandwidthMode;
    switch (bandwidth) {
    case isac12kHz:
    ...
    default: return -ISAC_DISALLOWED_ENCODER_BANDWIDTH;
    }

    WebRtcIsac_EncHistMulti(streamData, &bandwidthMode, kOneBitEqualProbCdf_ptr, 1);
    return 0;
}

WebRtc_Word16 WebRtcIsac_DecodeBandwidth(Bitstr *streamData, enum ISACBandwidth *bandwidth)
{
    int bandwidthMode;

    if (WebRtcIsac_DecHistOneStepMulti(&bandwidthMode, streamData, kOneBitEqualProbCdf_ptr, kOneBitEqualProbInitIndex,
                                       1) < 0)
    {
        // error check
    ...
    default: return -ISAC_DISALLOWED_BANDWIDTH_MODE_DECODER;
    }
    return 0;
}

WebRtc_Word16 WebRtcIsac_EncodeJitterInfo(WebRtc_Word32 jitterIndex, Bitstr *streamData)
{
...
WebRtc_Word16 WebRtcIsac_DecodeJitterInfo(Bitstr *streamData, WebRtc_Word32 *jitterInfo)
{
    int intVar;

    // Use the same CDF table as for bandwidth
    // both take two values with equal probability
    if (WebRtcIsac_DecHistOneStepMulti(&intVar, streamData, kOneBitEqualProbCdf_ptr, kOneBitEqualProbInitIndex, 1) < 0)
    {
        // error check
        return -ISAC_RANGE_ERROR_DECODE_BANDWITH;
    }
