...
static const float kQRateTableWb[12] = { 10000.0f, 11115.3f, 12355.1f, 13733.1f, 15264.8f, 16967.3f,
                                         18859.8f, 20963.3f, 23301.4f, 25900.3f, 28789.0f, 32000.0f };


static const float kQRateTableSwb[24] = {
    10000.0f, 11115.3f, 12355.1f, 13733.1f, 15264.8f, 16967.3f, 18859.8f, 20963.3f,
    23153.1f, 25342.9f, 27532.7f, 29722.5f, 31912.3f, 34102.1f, 36291.9f, 38481.7f,
    40671.4f, 42861.2f, 45051.0f, 47240.8f, 49430.6f, 51620.4f, 53810.2f, 56000.0f,
};


WebRtc_Word32 WebRtcIsac_InitBandwidthEstimator(BwEstimatorstr *bwest_str, enum IsacSamplingRate encoderSampRate,
                                                enum IsacSamplingRate decoderSampRate)
{
    ...
    case kIsacWideband:
    {
        ...
        bwest_str->rec_bw_inv        = 1.0f / (INIT_BN_EST_WB + INIT_HDR_RATE_WB);
    ...
    case kIsacSuperWideband:
    {
        ...
        bwest_str->rec_bw_inv        = 1.0f / (INIT_BN_EST_SWB + INIT_HDR_RATE_SWB);
...
WebRtc_Word16 WebRtcIsac_UpdateBandwidthEstimator(BwEstimatorstr *bwest_str, const WebRtc_UWord16 rtp_number,
                                                  const WebRtc_Word32 frame_length, const WebRtc_UWord32 send_ts,
                                                  const WebRtc_UWord32 arr_ts, const WebRtc_Word32 pksize
                                                  /*,    const WebRtc_UWord16 Index*/)
{
    ...
    if (frame_length != bwest_str->prev_frame_length) {
        bwest_str->rec_header_rate = (float)HEADER_SIZE * 8.0f * 1000.0f / (float)frame_length; /* bits/s */
    }

    /* UPDATE ESTIMATES ON THIS SIDE */
    /* compute far-side transmission rate */
    rec_rtp_rate = ((float)pksize * 8.0f * 1000.0f / (float)frame_length) + bwest_str->rec_header_rate;
    ...
    if (bwest_str->count_tot_updates_rec > 0) {
        ...
        //  that strict -DH
        {
            ...
            if ((WebRtc_UWord32)(arr_ts - bwest_str->last_update_ts) * 1000.0f / FS > 3000) {
                ...
                num_pkts_expected =
                    (int)(((float)(arr_ts - bwest_str->last_update_ts) * 1000.0f / (float)FS) / (float)frame_length);

                if (((float)bwest_str->num_pkts_rec / (float)num_pkts_expected) > 0.9) {
                    float inv_bitrate = (float)pow(
                        0.99995, (double)((WebRtc_UWord32)(arr_ts - bwest_str->last_reduction_ts) * 1000.0f / FS));
        ...
        if (frame_length != bwest_str->prev_frame_length) {
            ...
            bwest_str->rec_bw_inv = 1.0f / ((float)bwest_str->rec_bw + bwest_str->rec_header_rate);
        }
        ...
        if (rtp_number == bwest_str->prev_rec_rtp_number + 1) {
            ...
            if ((bwest_str->prev_rec_rtp_rate > bwest_str->rec_bw_avg) && (rec_rtp_rate > bwest_str->rec_bw_avg) &&
                !bwest_str->in_wait_period)
            {
                ...
                curr_bw_inv =
                    arr_ts_diff / ((float)(pksize + HEADER_SIZE) * 8.0f * FS); // (180+35)*8*16000 = 27.5 Mbit....


                if (curr_bw_inv < (1.0f / (MAX_ISAC_BW + bwest_str->rec_header_rate))) {
                    // don't allow inv rate to be larger than MAX
                    curr_bw_inv = (1.0f / (MAX_ISAC_BW + bwest_str->rec_header_rate));
                }

                /* update bottle neck rate estimate */
                bwest_str->rec_bw_inv = weight * curr_bw_inv + (1.0f - weight) * bwest_str->rec_bw_inv;
                ...
                t_diff_proj = ((float)(pksize + HEADER_SIZE) * 8.0f * 1000.0f) / bwest_str->rec_bw_avg;


                // difference between projected and actual
                //   arrival time differences
                arr_time_noise     = (float)(arr_ts_diff * 1000.0f / FS) - t_diff_proj;
                arr_time_noise_abs = (float)fabs(arr_time_noise);

                /* long term averaged absolute jitter */
                bwest_str->rec_jitter = weight * arr_time_noise_abs + (1.0f - weight) * bwest_str->rec_jitter;
                ...
                bwest_str->rec_jitter_short_term = 0.05f * arr_time_noise + 0.95f * bwest_str->rec_jitter_short_term;
            }
        }
    } else {
    ...
    if (bwest_str->rec_bw_inv > 1.0f / ((float)MIN_ISAC_BW + bwest_str->rec_header_rate)) {
        bwest_str->rec_bw_inv = 1.0f / ((float)MIN_ISAC_BW + bwest_str->rec_header_rate);
    }

    // limit maximum bitrate
    if (bwest_str->rec_bw_inv < 1.0f / ((float)MAX_ISAC_BW + bwest_str->rec_header_rate)) {
        bwest_str->rec_bw_inv = 1.0f / ((float)MAX_ISAC_BW + bwest_str->rec_header_rate);
    }
    ...
    bwest_str->rec_bw = (WebRtc_Word32)(1.0f / bwest_str->rec_bw_inv - bwest_str->rec_header_rate);

    if (immediate_set) {
        bwest_str->rec_bw = (WebRtc_Word32)(delay_correction_factor * (float)bwest_str->rec_bw);

        if (bwest_str->rec_bw < (WebRtc_Word32)MIN_ISAC_BW) {
            bwest_str->rec_bw = (WebRtc_Word32)MIN_ISAC_BW;
        }

        bwest_str->rec_bw_avg = bwest_str->rec_bw + bwest_str->rec_header_rate;

        bwest_str->rec_bw_avg_Q = (float)bwest_str->rec_bw;

        bwest_str->rec_jitter_short_term = 0.0f;

        bwest_str->rec_bw_inv = 1.0f / (bwest_str->rec_bw + bwest_str->rec_header_rate);
...
/* Index         - integer (range 0...23) indicating bottle neck & jitter as estimated by other side */
/* returns 0 if everything went fine, -1 otherwise                                                   */
WebRtc_Word16 WebRtcIsac_UpdateUplinkBwImpl(BwEstimatorstr *bwest_str, WebRtc_Word16 index,
                                            enum IsacSamplingRate encoderSamplingFreq)
{
    ...
    if (encoderSamplingFreq == kIsacWideband) {
        if (index > 11) {
            ...
            bwest_str->send_max_delay_avg = 0.9f * bwest_str->send_max_delay_avg + 0.1f * (float)MAX_ISAC_MD;
        } else {
            /* compute the jitter estimate as decoded on the other side */
            bwest_str->send_max_delay_avg = 0.9f * bwest_str->send_max_delay_avg + 0.1f * (float)MIN_ISAC_MD;
        }

        /* compute the BN estimate as decoded on the other side */
        bwest_str->send_bw_avg = 0.9f * bwest_str->send_bw_avg + 0.1f * kQRateTableWb[index];
    } else {
        /* compute the BN estimate as decoded on the other side */
        bwest_str->send_bw_avg = 0.9f * bwest_str->send_bw_avg + 0.1f * kQRateTableSwb[index];
    }
...
// statistics.
WebRtc_Word16 WebRtcIsac_UpdateUplinkJitter(BwEstimatorstr *bwest_str, WebRtc_Word32 index)
{
    if ((index < 0) || (index > 23)) {
        return -ISAC_RANGE_ERROR_BW_ESTIMATOR;
    }
...
WebRtc_UWord16 WebRtcIsac_GetDownlinkBwJitIndexImpl(BwEstimatorstr *bwest_str, WebRtc_Word16 *bottleneckIndex,
                                                    ...
                                                    enum IsacSamplingRate decoderSamplingFreq)
{
    ...
    if (((1.f - weight) * bwest_str->rec_max_delay_avg_Q + weight * MAX_ISAC_MD - MaxDelay) >
        (MaxDelay - (1.f - weight) * bwest_str->rec_max_delay_avg_Q - weight * MIN_ISAC_MD))
    {
        jitterInfo[0] = 0;
        /* update quantized average */
        bwest_str->rec_max_delay_avg_Q = (1.f - weight) * bwest_str->rec_max_delay_avg_Q + weight * (float)MIN_ISAC_MD;
    } else {
        jitterInfo[0] = 1;
        /* update quantized average */
        bwest_str->rec_max_delay_avg_Q = (1.f - weight) * bwest_str->rec_max_delay_avg_Q + weight * (float)MAX_ISAC_MD;
    }
    ...
    bwest_str->rec_bw_avg_Q =
        (1 - weight) * bwest_str->rec_bw_avg_Q + weight * ptrQuantizationTable[bottleneckIndex[0]];
    bottleneckIndex[0] += jitterInfo[0] * 12 * addJitterInfo;

    bwest_str->rec_bw_avg = (1 - weight) * bwest_str->rec_bw_avg + weight * (rate + bwest_str->rec_header_rate);
...
WebRtc_Word32 WebRtcIsac_GetDownlinkBandwidth(const BwEstimatorstr *bwest_str)
{
    ...
    jitter_sign = bwest_str->rec_jitter_short_term / bwest_str->rec_jitter_short_term_abs;
...
WebRtc_Word32 WebRtcIsac_GetDownlinkMaxDelay(const BwEstimatorstr *bwest_str)
{
...
void WebRtcIsac_GetUplinkBandwidth(const BwEstimatorstr *bwest_str, WebRtc_Word32 *bitRate)
{
    /* limit range of bottle neck rate */
...
WebRtc_Word32 WebRtcIsac_GetUplinkMaxDelay(const BwEstimatorstr *bwest_str)
{
 ...
 */
int WebRtcIsac_GetMinBytes(RateModel *State, int StreamSize, /* bytes in bitstream */
                           ...
                           /*,WebRtc_Word16        frequentLargePackets*/)
{
    ...
    if (State->InitCounter > 0) {
        ...
        if (State->BurstCounter) {
            if (State->StillBuffered < (1.0 - 1.0 / BURST_LEN) * DelayBuildUp) {
                ...
                MinRate = (1.0 + (FS / 1000) * DelayBuildUp / (double)(BURST_LEN * FrameSamples)) * BottleNeck;
            } else {
                // max bps derived from StillBuffered and DelayBuildUp
                // values
                MinRate =
                    (1.0 + (FS / 1000) * (DelayBuildUp - State->StillBuffered) / (double)FrameSamples) * BottleNeck;
    ...
    if ((State->ExceedAgo > burstInterval) && (State->BurstCounter == 0)) {
        if (State->PrevExceed) {
 ...
 * update long-term average bitrate and amount of data in buffer
 */
void WebRtcIsac_UpdateRateModel(RateModel *State, int StreamSize, /* bytes in bitstream */
                                const int    FrameSamples,        /* samples per frame */
                                const double BottleNeck)          /* bottle neck rate; excl headers (bps) */
{
    double TransmissionTime;
...
double WebRtcIsac_GetSnr(double bottle_neck, int framesamples)
{
    ...
    /* find new SNR value */
    switch (framesamples) {
    case 320: s2nr = a_20 + b_20 * bottle_neck * 0.001 + c_20 * bottle_neck * bottle_neck * 0.000001; break;
    case 480: s2nr = a_30 + b_30 * bottle_neck * 0.001 + c_30 * bottle_neck * bottle_neck * 0.000001; break;
    case 960: s2nr = a_60 + b_60 * bottle_neck * 0.001 + c_60 * bottle_neck * bottle_neck * 0.000001; break;
    default:  s2nr = 0;
    }
