...
static int WebRtcIsac_Fftradix(double Re[], double Im[], size_t nTotal, size_t nPass, size_t nSpan, int isign,
                               int max_factors, unsigned int max_perm, FFTstr *fftstate);
...
int WebRtcIsac_Fftns(unsigned int ndim, const int dims[], double Re[], double Im[], int iSign, double scaling,
                     FFTstr *fftstate)
{
    ...
    for (i = 0; i < ndim; i++) {
        nSpan = dims[i];
        nPass *= nSpan;
        ret = FFTRADIX(Re, Im, nTotal, nSpan, nPass, iSign, max_factors, max_perm, fftstate);
...
static int FFTRADIX(REAL Re[], REAL Im[], size_t nTotal, size_t nPass, size_t nSpan, int iSign, int max_factors,
                    unsigned int max_perm, FFTstr *fftstate)
{
