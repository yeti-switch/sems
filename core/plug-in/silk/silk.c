...
int Pcm16_2_SILK(unsigned char *out_buf, unsigned char *in_buf, unsigned int size, unsigned int channels,
                 unsigned int rate, long h_codec);
int SILK_2_Pcm16(unsigned char *out_buf, unsigned char *in_buf, unsigned int size, unsigned int channels,
                 unsigned int rate, long h_codec);
...
CODEC(CODEC_SILK_UB, Pcm16_2_SILK, SILK_2_Pcm16, AMCI_NO_CODEC_PLC, SILK_UB_create, SILK_destroy, SILK_bytes2samples,
      SILK_samples2bytes)
#endif
#if SYSTEM_SAMPLECLOCK_RATE >= 16000
CODEC(CODEC_SILK_WB, Pcm16_2_SILK, SILK_2_Pcm16, AMCI_NO_CODEC_PLC, SILK_WB_create, SILK_destroy, SILK_bytes2samples,
      SILK_samples2bytes)
#endif
#if SYSTEM_SAMPLECLOCK_RATE >= 12000
CODEC(CODEC_SILK_MB, Pcm16_2_SILK, SILK_2_Pcm16, AMCI_NO_CODEC_PLC, SILK_MB_create, SILK_destroy, SILK_bytes2samples,
      SILK_samples2bytes)
#endif
CODEC(CODEC_SILK_NB, Pcm16_2_SILK, SILK_2_Pcm16, AMCI_NO_CODEC_PLC, SILK_NB_create, SILK_destroy, SILK_bytes2samples,
      SILK_samples2bytes)
END_CODECS
...
static int create_SILK_encoder(SILK_state *st, unsigned int rtp_Hz, unsigned int avg_bit_rate)
{
...
static long SILK_create(unsigned int rtp_Hz, unsigned int avg_bit_rate, const char *format_parameters,
                 ...
                 unsigned int rate, long h_codec)
{
    ...
    /* Silk Encoder */
    ret = SKP_Silk_SDK_Encode(st->psEnc, &st->encControl, (SKP_int16 *)in_buf, (SKP_int16)size / 2, out_buf, &nBytes);
    if (ret) {
                 ...
                 unsigned int rate, long h_codec)
{
    ...
    do {
        /* Decode 20 ms */
        ret = SKP_Silk_SDK_Decode(st->psDec, &st->decControl, 0 /*lost*/, in_buf, size, outPtr, &len);
        if (ret) {
        ...
        if ((unsigned char *)outPtr - out_buf > AUDIO_BUFFER_SIZE) {
            ...
            ERROR("Buffer overflow (size=%li)", (unsigned char *)outPtr - out_buf);
            return -1; // TODO
        }
