...
struct fmtdesc *alldescs[] = { &f_audio, &f_alaw, &f_ulaw, &f_linear, (struct fmtdesc *)NULL };
        ...
        if (cc != sizeof(s)) {
            if (cc >= 0)
                fprintf(stderr, "%s: incomplete frame (%d byte%s missing) from %s\n", progname, sizeof(s) - cc,
                        "s" + (sizeof(s) - cc == 1), inname ? inname : "stdin");
            gsm_destroy(r);
            errno = 0;
            return -1;
        }
...
static int process P1((name), char *name)
{
    ...
    if ((*(f_decode ? init_output : init_input))()) {
        fprintf(stderr, "%s: error %s %s\n", progname, f_decode ? "writing header to" : "reading header from",
...
{
    printf("%s 1.0, version %s\n", progname, "$Id: toast.c,v 1.1 2003/02/03 15:04:56 rco Exp $");
}
