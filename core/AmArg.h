...
class AmArg : public AmObject {
  ...
  private:
    ...
    struct ValueRef : public atomic_ref_cnt {
        AmObject *arg_ptr;
  ...
  public:
    ...
    template <typename... Args> AmArg &assign_array(Args &&...args)
    {
        invalidate();
        assertArray();
    ...
    template <typename T> AmArg &assign_array(std::initializer_list<T> l)
    {
        invalidate();
        assertArray();
        for (const auto &it : l) {
    ...
    template <Type _type_id> bool is() const { return type == _type_id; }
    bool                          isNumber() const;
    ...
    template <typename T> T asNumber()
    {
        ...
        case LongLong: return static_cast<T>(v_long);
        case Double:   return static_cast<T>(v_double);
        default:       throw TypeMismatchException();
        }
    }
...
#define define_isArg_assertArg(TYPE)                                                                                   \
    inline bool isArg##TYPE(const AmArg &a)                                                                            \
    {                                                                                                                  \
        return a.is<AmArg::TYPE>();                                                                                    \
    }                                                                                                                  \
    inline void assertArg##TYPE(const AmArg &a)                                                                        \
    {                                                                                                                  \
        a.assertType(AmArg::TYPE);                                                                                     \
    }

define_isArg_assertArg(Undef) define_isArg_assertArg(Int) define_isArg_assertArg(LongLong) define_isArg_assertArg(Bool)
    define_isArg_assertArg(Double) define_isArg_assertArg(CStr) define_isArg_assertArg(AObject)
        define_isArg_assertArg(ADynInv) define_isArg_assertArg(Blob) define_isArg_assertArg(Array)
            define_isArg_assertArg(Struct) define_isArg_assertArg(Reference)
