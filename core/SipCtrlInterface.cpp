...
int _SipCtrlInterface::load()
{
    ...
    auto &queue_size_group = stat_group(Gauge, "core", "transport_send_queue_size")
        .setHelp("Connections sending queues size sum");

    if(nr_tcp_sockets || nr_tls_sockets || nr_ws_sockets || nr_wss_sockets) {
        queue_size_group.addFunctionCounter([]()->unsigned long long {
...
{
    return trans_layer::instance()->cancel(
        tt,stl2cstr(dialog_id),
        inv_cseq,maxf,stl2cstr(hdrs),stl2cstr(dlg_route_set));
}
				...
				int redirects_allowed)
{
    ...
    if(req.method == "CANCEL") {
        return cancel(
            &req.tt, dialog_id, req.cseq, req.max_forwards,
            req.hdrs, string());
    }
...
bool _SipCtrlInterface::sip_msg2am_request(const sip_msg *msg,
    ...
    sip_trans *tr, trans_bucket *buk)
{
  ...
  default:
    ERROR("BUG: unexpected timout event type '%d'.", evt);
    return;
  }
