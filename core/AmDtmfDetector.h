...
class AmDtmfEvent : public AmEvent {
  ...
  public:
    AmDtmfEvent(int event, int duration, int volume, Dtmf::EventSource source)
        : AmEvent(source)
        , m_event(event)
...
class AmKeyPressSink {
  public:
     ...
     */
    virtual void registerKeyReleased(int event, int volume, Dtmf::EventSource source, const struct timeval &start,
     ...
     * @param event_id id of the event
     */
    virtual void registerKeyPressed(int event, int volume, Dtmf::EventSource source, bool has_eventid = false,
                                    unsigned int event_id = 0) = 0;
    /**
...
class AmInbandDtmfDetector {
  ...
  public:
     ...
     */
    virtual int streamPut(bool &dtmf_detected, const unsigned char *samples, unsigned int size,
 ...
 */
class AmDtmfDetector : public AmEventHandler, public AmKeyPressSink {
  private:
    ...
    bool non_initiallized_usage; // to supress repeated warnings in putDtmfAudio
     ...
     * @param stop time when key was released
     */
    void registerKeyReleased(int event, int volume, Dtmf::EventSource source, const struct timeval &start,
                             const struct timeval &stop, bool has_eventid = false, unsigned int event_id = 0);
    /**
     * Through this method the AmDtmfDetector receives events that was
     * detected by specific detectors.
     * @param event code of key released
     ...
     */
    void registerKeyPressed(int event, int volume, Dtmf::EventSource source, bool has_eventid = false,
