...
unsigned int precoded_bytes2samples(long h_codec, unsigned int num_bytes)
{
    ...
    return ((precoded_payload_t *)h_codec)->frame_ms * ((precoded_payload_t *)h_codec)->sample_rate / 1000;
}
...
amci_codec_t _codec_precoded = { PRECODED_CODEC_ID,     NULL, NULL, NULL, NULL, NULL, precoded_bytes2samples,
...
int precoded_file_close(FILE *fp, struct amci_file_desc_t *fmt_desc, int options, long h_codec,
...
int AmPrecodedFile::open(const std::string &filename)
{
    ...
    while (ifs.good() && !ifs.eof()) {
        ...
        if (codec_def.size() != 8) {
            ERROR("unable to decipher codec line '%s'", codec_line.c_str());
            continue;
        }
...
#define get_uint_item(name, index, description)                                                                        \
    ...
    if (str2i(codec_def[index], name)) {                                                                               \
        ERROR("decoding " description " in codec line '%s'", codec_line.c_str());                                      \
        continue;                                                                                                      \
    }                                                                                                                  \
    pl.name = name;
...
#undef get_uint_item
        ...
        DBG("inserting codec '%s' file '%s' and id %d", pl.name, pl.filename.c_str(), pl.payload_id);
...
{
    std::map<int, precoded_payload_t>::const_iterator it = payloads.find(payload_id);
...
int AmPrecodedFile::getDynPayload(const string &name, int rate, int encoding_param) const
{
    ...
    for (std::map<int, precoded_payload_t>::const_iterator pl_it = payloads.begin(); pl_it != payloads.end(); ++pl_it)
