...
bool AmSipDialog::onRxReqSanity(const AmSipRequest &req)
{
    ...
    if (invite || (req.method == SIP_METH_UPDATE)) {
        ...
        DBG3("AmSipDialog::onRxReqSanity: %s, pending %d", invite ? "INVITE" : "UPDATE", pending);

        // check for pending UAS transactions
        if (pending) {
            reply_error(req, 491, SIP_REPLY_PENDING,
                        SIP_HDR_COLSP(SIP_HDR_RETRY_AFTER) + int2str(get_random() % 10) + CRLF, logger);
            return false;
        }

        // check for OA state
        bool has_sdp = (req.body.hasContentType(SIP_APPLICATION_SDP) != nullptr);
        if (offeranswer_enabled && (invite || has_sdp)) // skip OA checking for UPDATE without SDP
        {
...
int AmSipDialog::onSdpCompleted(bool sdp_offer_owner)
{
    ...
    int ret = ((AmSipDialogEventHandler *)hdl)->onSdpCompleted(oa.getLocalSdp(), oa.getRemoteSdp(), sdp_offer_owner);
...
{
    DBG3("%sabling offer_answer on SIP dialog '%s'", oa_enabled ? "en" : "dis", local_tag.c_str());
...
bool AmSipDialog::onRxReplySanity(const AmSipReply &reply)
{
    if (!getRemoteTag().empty() && reply.to_tag != getRemoteTag()) {
        ...
        } else if (reply.cseq_method == SIP_METH_BYE || reply.cseq_method == SIP_METH_CANCEL) {
            DBG("[%s] reply for %s '%d %s' is not matched with dialog. but matched with transaction. process it",
                local_tag.c_str(), reply.cseq_method.c_str(), reply.code, reply.reason.c_str());
            // PASS
        } else {
            // DROP
            return false;
...
bool AmSipDialog::onRxReplyStatus(const AmSipReply &reply)
{
    ...
    if (status == Disconnecting) {
        DBG("?Disconnecting?: cseq_method = %s; code = %i", reply.cseq_method.c_str(), reply.code);
    ...
    bool cont = true;
    if ((reply.code >= 200) && (reply.code < 300) && (reply.cseq_method == SIP_METH_INVITE)) {
        if (hdl)
...
void AmSipDialog::uasTimeout(AmSipTimeoutEvent *to_ev)
{
    ...
    case AmSipTimeoutEvent::noACK:
        ...
        if (hdl)
            ((AmSipDialogEventHandler *)hdl)->onNoAck(to_ev->cseq);
        break;
...
int AmSipDialog::bye(const string &hdrs, int flags, bool final)
{
    ...
    case Connected:
    {
        ...
        if (final) {
            ...
            while (!uas_trans.empty()) {
                ...
                CLASS_DBG("bye(): terminate UAS %s/%d with %d %s", req.method.data(), req_cseq, code, reason);
    ...
    case Cancelling:
        ...
        // missing AmSipRequest to be able
        // to send the reply on behalf of the app.
        DBG("[%s] ignoring bye() in %s state: no UAS transaction to reply", local_tag.c_str(), getStatusStr());
        drop();
...
int AmSipDialog::refer(const string &refer_to, int expires, const string &referred_by)
{
    if (getStatus() == Connected) {
...
int AmSipDialog::prack(const AmSipReply &reply1xx, const AmMimeBody *body, const string &hdrs)
{
    switch (getStatus()) {
    case Trying:
    case Proceeding:
    ...
    case Disconnected:
    case Disconnecting: ERROR("can not send PRACK while dialog is in state '%d'.", status); return -1;
    default:            ERROR("BUG: unexpected dialog state '%d'.", status); return -1;
    }

    string h = hdrs + SIP_HDR_COLSP(SIP_HDR_RACK) + int2str(reply1xx.rseq) + " " + int2str(reply1xx.cseq) + " " +
...
{
    for (TransMap::reverse_iterator t = uac_trans.rbegin(); t != uac_trans.rend(); t++) {
        if (t->second.method == SIP_METH_INVITE) {
            cancel_final |= final;
            switch (getStatus()) {
            case Trying:
                ...
                if (hdrs.length())
                    t->second.hdrs += hdrs;
                return 0;
            case Cancelling: ERROR("INVITE transaction has already been cancelled"); return -1;
            default:
                setStatus(Cancelling);
                return SipCtrlInterface::cancel(&t->second.tt, local_tag, t->first, AmConfig.max_forwards,
...
int AmSipDialog::send_200_ack(unsigned int inv_cseq, const AmMimeBody *body, const string &hdrs, int flags)
{
    ...
    int res = SipCtrlInterface::send(req, local_tag, remote_tag.empty() || !next_hop_1st_req ? next_hop : "",
