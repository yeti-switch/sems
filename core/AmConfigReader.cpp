...
int AmConfigReader::loadFile(const string &path)
{
    ...
    if (!fp) {
        WARN("could not open configuration file '%s': %s", path.c_str(), strerror(errno));
        return -1;
    }
        ...
        if ((key_beg < key_end) && (val_beg <= val_end)) {
            ...
            if (hasParameter(keyname)) {
                WARN("while loading '%s': overwriting configuration "
                     "'%s' value '%s' with  '%s'\n",
                     path.c_str(), keyname.c_str(), getParameter(keyname).c_str(), val.c_str());
            }
...
{
    return loadFile(add2path(AmConfig.configs_path, 1, string(mod_name + CONFIG_FILE_SUFFIX).c_str()));
}

static int str_get_line(const char **c, const char *end, char *line_buf, size_t line_buf_len)
{
    enum { SGL_LINE = 0, SGL_COMMENT, SGL_CR, SGL_LF };
...
int AmConfigReader::loadString(const char *cfg_lines, size_t cfg_len)
{
    ...
    c = key_beg = key_end = val_beg = val_end = inc_beg = inc_end = 0;
    while ((cursor < cfg_end) && ((ls = str_get_line(&cursor, cfg_end, lb, MAX_CONFIG_LINE)) != -1)) {
...
bool AmConfigReader::getMD5(const string &path, string &md5hash, bool lowercase)
{
    ...
    string file_data((std::istreambuf_iterator<char>(data_file)), std::istreambuf_iterator<char>());
