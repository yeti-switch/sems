    ...
    , payload(NULL)
{
    ...
    for (Parts::const_iterator it = body.parts.begin(); it != body.parts.end(); ++it) {
...
const AmMimeBody &AmMimeBody::operator=(const AmMimeBody &r_body)
{
    ...
    clearParts();
    for (Parts::const_iterator it = r_body.parts.begin(); it != r_body.parts.end(); ++it) {
        parts.push_back(new AmMimeBody(**it));
    }
...
AmContentType::AmContentType(const AmContentType &ct)
    ...
    , mp_boundary(NULL)
{
    for (Params::const_iterator it = ct.params.begin(); it != ct.params.end(); ++it) {
...
const AmContentType &AmContentType::operator=(const AmContentType &r_ct)
{
    ...
    clearParams();
    for (Params::const_iterator it = r_ct.params.begin(); it != r_ct.params.end(); ++it) {
        params.push_back(new Param(**it));
        if ((*it)->type == Param::BOUNDARY)
            mp_boundary = params.back();
    }
...
{
    enum { CT_TYPE = 0, CT_SLASH_SWS, CT_SUBTYPE_SWS, CT_SUBTYPE };
    ...
    for (; c < end; c++) {
        ...
        case CT_SLASH_SWS:
            ...
            default:    DBG("Missing '/' after media type in 'Content-Type' hdr field"); return -1;
            }
            break;
        ...
        case CT_SUBTYPE:
            ...
            case SP:
            case HTAB:
            case SEMICOLON: subtype = string(beg, c - beg); return parseParams(c, end);
            }
            break;
    ...
    case CT_TYPE:
    case CT_SLASH_SWS:
    case CT_SUBTYPE_SWS: DBG("Malformed Content-Type value: <%.*s>", (int)(end - beg), beg); return -1;
...
int AmContentType::parseParams(const char *c, const char *end)
{
    ...
    for (list<sip_avp *>::iterator it_ct_param = avp_params.begin(); it_ct_param != avp_params.end(); ++it_ct_param) {

        DBG("parsed new content-type parameter: <%.*s>=<%.*s>", (*it_ct_param)->name.len, (*it_ct_param)->name.s,
            (*it_ct_param)->value.len, (*it_ct_param)->value.s);

        Param *p = new Param(c2stlstr((*it_ct_param)->name), c2stlstr((*it_ct_param)->value));
...
{
    enum { B_START = 0, B_CR, B_LF, B_HYPHEN, B_BOUNDARY, B_HYPHEN2, B_HYPHEN3, B_CR2, B_LF2, B_MATCHED };
...
int AmMimeBody::parseSinglePart(unsigned char *buf, unsigned int len)
{
    ...
    for (list<sip_header *>::iterator it = hdrs.begin(); it != hdrs.end(); ++it) {
        ...
        DBG("Part header: <%.*s>: <%.*s>", (*it)->name.len, (*it)->name.s, (*it)->value.len, (*it)->value.s);

        if ((*it)->type == sip_header::H_CONTENT_TYPE) {
            sub_part_ct = c2stlstr((*it)->value);
        } else {
            sub_part_hdrs += c2stlstr((*it)->name) + COLSP + c2stlstr((*it)->value) + CRLF;
        }
    }

    if (!sub_part_ct.empty() && (sub_part->parse(sub_part_ct, (unsigned char *)c, end - c) == 0)) {
        // TODO: check errors
...
int AmMimeBody::parseMultipart(const unsigned char *buf, unsigned int len)
{
    ...
    do {
        ...
        if (parseSinglePart(part_beg, part_end - part_beg) < 0) {
            ...
            DBG("Added new part:\n%.*s", part->content_len, part->payload);
        }
...
int AmMimeBody::parse(const string &content_type, const unsigned char *buf, unsigned int len)
{
...
{
    return !lower_cmp_n(st.c_str(), st.length(), subtype.c_str(), subtype.length());
}
...
bool AmContentType::hasContentType(const string &content_type) const
{
    ...
    return !lower_cmp_n(cmp_ct.c_str(), cmp_ct.length(), content_type.c_str(), content_type.length());
}
...
void AmMimeBody::normalizeContentType()
{
    if (ct.isType(MULTIPART)) {
        for (Parts::const_iterator it = parts.begin(); it != parts.end(); ++it)
            (*it)->normalizeContentType();
    } else {
        ct.tolower();
    }
}
