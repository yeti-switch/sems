...
class hep_msg_sensor : public msg_sensor {
    struct hep_chunk {
        u_int16_t vendor_id;
        u_int16_t type_id;
        u_int16_t length;
    ...
    template <typename Type> struct hep_chunked_data {
        ...
        inline void set(const Type &_data) { data = _data; }
    } __attribute__((packed));
    ...
    inline void upd_hep_hdr(struct hep_generic &h, sockaddr_storage *from, sockaddr_storage *to, unsigned int len,
                            uint8_t proto);

  public:
    ~hep_msg_sensor();

    int init(const char *capture_addr, unsigned short capture_port, int capture_id,
             const string &capture_password = string(), bool compression_enabled = false);
