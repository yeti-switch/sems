...
int pcap_logger::log(const char *buf, int len, sockaddr_storage *src_ip, sockaddr_storage *dst_ip, cstring method,
                     int reply_code)
{
    PcapRecorderEvent *event = new PcapRecorderEvent(this, std::vector<char>(buf, buf + len), (struct sockaddr *)src_ip,
                                                     (struct sockaddr *)dst_ip);
    PcapFileRecorderProcessor::instance()->putEvent(event);
    return 0;
}

int pcap_logger::logv4(const char *data, int data_len, struct sockaddr_storage *src, struct sockaddr_storage *dst,
                       size_t addr_len, struct timeval &event_time)
{
    ...
    hdr.udp.chksum =
        _chksum(sum(&hdr.ip.ip_src, 8) + sum(&hdr.udp, 8) + htons(udp_size) + 0x1100 + sum(data, data_len));
...
int pcap_logger::logv6(const char *data, int data_len, struct sockaddr_storage *src, struct sockaddr_storage *dst,
                       size_t addr_len, struct timeval &event_time)
{
    ...
    hdr.udp.chksum =
        _chksum(sum(&hdr.ip.ip6_src, 32) + sum(&hdr.udp, 8) + htons(udp_size) + 0x1100 + sum(data, data_len));
