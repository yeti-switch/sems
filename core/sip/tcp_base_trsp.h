...
class trsp_base_input : public trsp_input {
  ...
  public:
    ...
    {
        if (input_len > MAX_TCP_MSGLEN)
            return 0;
        return MAX_TCP_MSGLEN - input_len;
    }
    void add_input_len(int len) override { input_len += len; }
...
class tcp_base_trsp : public trsp_socket {
  ...
  protected:
    ...
    tcp_base_trsp(trsp_server_socket *server_sock, trsp_worker *server_worker, int sd, const sockaddr_storage *sa,
...
class trsp_socket_factory : public atomic_ref_cnt {
  protected:
...
class trsp_worker : public AmThread {
    struct event_base *evbase;
...
class trsp_statistics {
  ...
  public:
    void add_trsp_statistics(trsp_st_base *stream) { stats.push_back(stream); }
    void dispose()
    {
...
class trsp_server_socket : public trsp_socket {
  protected:
    ...
    trsp_server_socket(unsigned short if_num, unsigned short proto_idx, unsigned int opts,
