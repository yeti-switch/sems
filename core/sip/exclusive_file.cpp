...
int exclusive_file::open(bool &is_new)
{
    ...
    fd = ::open(name.c_str(), O_WRONLY | O_CREAT | O_APPEND, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
    if (fd < 0) {
...
int exclusive_file::open(const char *filename, exclusive_file *&excl_fp, bool &is_new)
{
