...
sip_route::~sip_route()
{
    for (list<route_elmt *>::iterator it = elmts.begin(); it != elmts.end(); ++it)
        delete *it;
}
...
error:
    ...
    switch (st) {
    case RR_QUOTED: DBG("Malformed route header"); return -1;
    ...
    case RR_NXT_ROUTE: return 1; // next route available
    }
...
{
    if (parse_route(fr) < 0) {
        DBG("Could not parse route hf [%.*s]", fr->value.len, fr->value.s);
        return -1;
    }
