...
ssize_t ws_input::recv_callback(wslay_event_context_ptr ctx, uint8_t *data, size_t len, int flags, void *user_data)
{
...
ssize_t ws_input::send_callback(wslay_event_context_ptr ctx, const uint8_t *data, size_t len, int flags,
                                void *user_data)
{
    ws_input *input = (ws_input *)user_data;
...
void ws_input::on_msg_recv_callback(wslay_event_context_ptr ctx, const struct wslay_event_on_msg_recv_arg *arg,
                                    void *user_data)
{
    ws_input *input = (ws_input *)user_data;
...
int ws_input::genmask_callback([[maybe_unused]] wslay_event_context_ptr ctx, uint8_t *buf, size_t len, void *user_data)
{
    // ws_input* input = (ws_input*)user_data;
    (void)user_data;
...
struct wslay_event_callbacks ws_input::callbacks = { ws_input::recv_callback,
...
{
    switch (code) {
    case WSLAY_CONTINUATION_FRAME: return "CONTINUATION_FRAME";
    case WSLAY_TEXT_FRAME:         return "TEXT_FRAME";
    case WSLAY_BINARY_FRAME:       return "BINARY_FRAME";
    case WSLAY_CONNECTION_CLOSE:   return "CONNECTION_CLOSE";
    case WSLAY_PING:               return "PING";
    case WSLAY_PONG:               return "PONG";
    default:                       return "NONE";
    }
}
...
void ws_input::generate_transport_errors()
{
    ...
    */
    ...
    while (!send_q.empty()) {
        ...
        output->copy_addrs(&s_msg.local_ip, &s_msg.remote_ip, s_msg.transport_id);
...
void ws_input::pre_write()
{
    if (!send_q.empty()) {
        tcp_base_trsp::msg_buf *msg   = send_q.front();
        wslay_event_msg         e_msg = { .opcode     = WSLAY_TEXT_FRAME,
...
int ws_input::get_input_free_space()
{
    if (!ws_connected)
        ...
        if (ws_input_len > MAX_TCP_MSGLEN)
            return 0;
...
void ws_input::on_parsed_received_msg(tcp_base_trsp *trsp, sip_msg *s_msg)
{
    if (!ws_connected) {
        ...
        if (err) {
            ...
            if ((err != MALFORMED_FLINE) && (s_msg->type == HTTP_REQUEST) &&
            ...
            if (s_msg->sec_ws_protocol && (s_msg->sec_ws_protocol->value.len != sip_len ||
                ...
                lower_cmp(s_msg->upgrade->value.s, websocket, websocket_len))
            {
                ...
                for (auto &val : conn_values) {
                    ...
                    if (data.size() == upgrade_len && !lower_cmp(data.c_str(), upgrade, upgrade_len)) {
                        has_update = true;
                        break;
                    }
                }
...
ssize_t ws_input::recv_callback([[maybe_unused]] wslay_event_context_ptr ctx, uint8_t *data, size_t len,
...
ssize_t ws_input::on_send_callback([[maybe_unused]] wslay_event_context_ptr ctx, const uint8_t *data, size_t len,
                                   int flags)
{
...
int ws_input::send_reply(sip_msg *req, int reply_code, const cstring &reason)
{
    ...
    sip_header sec_ws_accept(sip_header::H_SEC_WS_ACCEPT,
                             cstring(HTTP_HDR_SEC_WS_ACCEPT, SIP_HDR_LEN(HTTP_HDR_SEC_WS_ACCEPT)),
                             get_sec_ws_accept_data(req->sec_ws_key->value));
...
int ws_input::send_request()
{
    ...
    sip_header sec_ws_key(sip_header::H_SEC_WS_KEY, cstring(HTTP_HDR_SEC_WS_KEY, SIP_HDR_LEN(HTTP_HDR_SEC_WS_KEY)),
                          get_sec_ws_key_data());
    sip_header sec_ws_version(sip_header::H_SEC_WS_VERSION,
                              cstring(HTTP_HDR_SEC_WS_VERSION, SIP_HDR_LEN(HTTP_HDR_SEC_WS_VERSION)), cstr_ws_version);
    sip_header sec_ws_protocol(sip_header::H_SEC_WS_PROTOCOL,
                               cstring(HTTP_HDR_SEC_WS_PROTOCOL, SIP_HDR_LEN(HTTP_HDR_SEC_WS_PROTOCOL)),
                               cstr_ws_protocol);
    sip_header connection(sip_header::H_CONNECTION, cstring(HTTP_HDR_CONNECTION, SIP_HDR_LEN(HTTP_HDR_CONNECTION)),
                          cstr_upgrade);
...
int wss_input::on_tls_record(tcp_base_trsp *trsp, const uint8_t data[], size_t size)
{
    if (size < (size_t)input.get_input_free_space()) {
        ...
        ERROR("message is too big. drop connection. peer %s:%d", trsp->get_peer_ip().data(), trsp->get_peer_port());
    }
    return 0;
}
...
ws_trsp_socket::ws_trsp_socket(trsp_server_socket *server_sock, trsp_worker *server_worker, int sd,
...
{
    ws_server_socket::ws_statistics *ws_stats =
        dynamic_cast<ws_server_socket::ws_statistics *>(server_sock->get_statistics());
    if (ws_stats) {
        ws_stats->incWsConnectedConnectionsCount(this);
        ws_stats->decConnectedConnectionsCount(this);
    }

    DBG("************ on_ws_connect() ***********");
    DBG("new WS connection from %s:%u", get_peer_ip().c_str(), get_peer_port());
    add_write_event();
    DBG("write event added...");
}
...
ws_server_socket::ws_server_socket(short unsigned int if_num, short unsigned int proto_idx, unsigned int opts,
                                   socket_transport transport)
    : trsp_server_socket(if_num, proto_idx, opts, new ws_socket_factory(transport),
                         new ws_statistics(transport, if_num, proto_idx))
{
}
...
ws_server_socket::ws_statistics::ws_statistics(trsp_socket::socket_transport transport, unsigned short if_num,
                                               unsigned short proto_idx)
    : tcp_server_socket::tcp_statistics(transport, if_num, proto_idx)
    , countInWsConnectedConnections(
    ...
    , countOutWsConnectedConnections(
          stat_group(Gauge, "core", "connections")
              .addAtomicCounter()
...
void ws_server_socket::ws_statistics::changeCountConnection(bool remove, tcp_base_trsp *socket)
{
    ...
    if (remove && ws_socket->is_ws_connected()) {
        decWsConnectedConnectionsCount(ws_socket);
    } else {
        tcp_server_socket::tcp_statistics::changeCountConnection(remove, socket);
    }
}

void ws_server_socket::ws_statistics::incWsConnectedConnectionsCount(tcp_base_trsp *socket)
{
    if (socket->is_client())
...
void ws_server_socket::ws_statistics::decWsConnectedConnectionsCount(tcp_base_trsp *socket)
{
    if (socket->is_client())
...
{
    wss_server_socket::wss_statistics *wss_stats =
        dynamic_cast<wss_server_socket::wss_statistics *>(server_sock->get_statistics());
    if (wss_stats) {
        wss_stats->incWssConnectedConnectionsCount(this);
        wss_stats->decTlsConnectedConnectionsCount(this);
    }

    DBG("************ on_wss_connect() ***********");
    DBG("new WSS connection from %s:%u", get_peer_ip().c_str(), get_peer_port());
    add_write_event();
    DBG("write event added...");
}
...
wss_server_socket::wss_server_socket(short unsigned int if_num, short unsigned int proto_idx, unsigned int opts,
    ...
    : trsp_server_socket(if_num, proto_idx, opts, new wss_socket_factory(transport),
                         new wss_statistics(transport, if_num, proto_idx))
{
}

wss_server_socket::wss_statistics::wss_statistics(trsp_socket::socket_transport transport, unsigned short if_num,
                                                  unsigned short proto_idx)
    : tls_server_socket::tls_statistics(transport, if_num, proto_idx)
    , countInWssConnectedConnections(
          stat_group(Gauge, "core", "connections")
              .addAtomicCounter()
              .addLabel("direction", "in")
              ...
              .addLabel("transport", trsp_socket::socket_transport2proto_str(transport))
              .addLabel("protocol", AmConfig.sip_ifs[if_num].proto_info[proto_idx]->ipTypeToStr()))
    , countOutWssConnectedConnections(
          stat_group(Gauge, "core", "connections")
              .addAtomicCounter()
              .addLabel("direction", "out")
              .addLabel("state", "wss_connected")
...
void wss_server_socket::wss_statistics::changeCountConnection(bool remove, tcp_base_trsp *socket)
{
    ...
    if (remove && wss_socket->is_ws_connected()) {
        decWssConnectedConnectionsCount(socket);
    } else
        tls_server_socket::tls_statistics::changeCountConnection(remove, socket);
}

void wss_server_socket::wss_statistics::incWssConnectedConnectionsCount(tcp_base_trsp *socket)
{
    if (socket->is_client())
...
void wss_server_socket::wss_statistics::decWssConnectedConnectionsCount(tcp_base_trsp *socket)
{
    if (socket->is_client())
