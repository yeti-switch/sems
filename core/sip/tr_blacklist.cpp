...
void bl_timer::fire()
{
    DBG_BL("blacklist: %s/%i/%d expired", am_inet_ntop(&addr).c_str(), am_get_port(&addr), addr.transport());
    tr_blacklist::instance()->remove(&addr);
}

bool blacklist_bucket::insert(const bl_addr &addr, unsigned int duration /* ms */, const char *reason)
{
    ...
    DBG_BL("blacklist: added %s/%i/%i (%s/TTL=%.1fs)", am_inet_ntop(&addr).c_str(), am_get_port(&addr),
           addr.transport(), reason, (float)duration / 1000.0);
...
{
    bool              res;
    blacklist_bucket *bucket = get_bucket(hashlittle(addr, SA_len(addr), 0) & BLACKLIST_HT_MASK);
    bucket->lock();
...
void _tr_blacklist::insert(const sockaddr_storage *addr, unsigned int duration, const char *reason)
{
    if (!duration)
        return;
