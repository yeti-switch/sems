...
static uint32_t sum(const void *_data, unsigned _len)
{
    ...
    for (unsigned i = 0; i < len; i++)
        r += data[i];
    if (_len & 1)
        r += (unsigned)((const char *)_data)[_len - 1];
 ...
 */
inline unsigned short udpv4_vhdr_sum(struct udphdr *uh, struct in_addr *src, struct in_addr *dst, unsigned short length)
{
    unsigned sum;
 ...
 */
inline static int mk_udp_hdr(struct udphdr *u, const sockaddr_storage *from, const sockaddr_storage *to,
                             unsigned char *buf, int len, int do_chk)
{
...
int ipip_msg_sensor::feed(const char *buf, int len, sockaddr_storage *from, sockaddr_storage *to,
                          packet_type_t packet_type)
{
    ...
    mk_ip_hdr(&hdr.ip, &SAv4(from)->sin_addr, &SAv4(to)->sin_addr, len + sizeof(hdr.udp), IPPROTO_UDP);
...
int ethernet_msg_sensor::init(const char *ifname, const char *dst_mac)
{
    ...
    if (-1 == ret) {
        ERROR("can't resolve interface name '%s' to index. error: %d", iface_name.c_str(), errno);
        goto error;
    }
    ...
    if (-1 == ret) {
        ERROR("can't get hw address of interface '%s'. error: %d", iface_name.c_str(), errno);
        goto error;
    }

    sensor_src_mac = ether_ntoa((struct ether_addr *)ifr.ifr_hwaddr.sa_data);
    DBG("hw address for '%s' is '%s'", iface_name.c_str(), sensor_src_mac.c_str());
...
int ethernet_msg_sensor::feed(const char *buf, int len, sockaddr_storage *from, sockaddr_storage *to,
                              packet_type_t packet_type)
{
    // int ret;
    struct msghdr snd_msg;
