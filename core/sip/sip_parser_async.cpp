...
static int skip_line_async(parser_state *pst, char *end)
{
    ...
    for (; (c < end) && *c; c++) {
        ...
        case 0 /* START */:
            ...
            case_ST_CR(*c);
        case ST_LF:
        case ST_CRLF: DBG3("first line is skipped"); return 0;
...
static int parse_header_async(sip_header *hdr, parser_state *pst, char *end)
{
    ...
    for (; **c && (*c < end); (*c)++) {
        ...
        case ST_CRLF:
            ...
            case H_NAME:
                ...
                DBG("Illegal CR or LF in header name: <%.*s>", (int)(*c - begin), begin);
    ...
    switch (st) {
    case H_NAME:
    case H_VALUE: DBG("Incomplete header (st=%i;saved_st=%i)", st, saved_st); return UNEXPECTED_EOT;
    case ST_LF:
    case ST_CRLF:
        switch (saved_st) {
        case H_NAME:
    ...
    while (c <= end) {
        ...
        case ST_BODY:
            ...
            if (pst->content_len > end - c) {
                DBG("incomplete body. content_len:%d, data_left:%d", pst->content_len, end - c);
                return UNEXPECTED_EOT;
            } else {
                DBG3("end of body");
                return 0;
        ...
        default: ERROR("unknown stage:%d", stage); return -1;
        } // switch(stage)
        ...
        if (!err) {
            ...
            DBG("incomplete msg error while having tail data. stage:%d, st:%d, tail:%zd", stage, pst->st, end - c);
            return MALFORMED_SIP_MSG;
        } else {
            return err;
        }
