...
void file_msg_logger::on_closed()
{
    // if(upload_destination.empty()) return;
    if (!upload_destination)
        return;
    if (!AmSessionContainer::instance()->postEvent(HTTP_EVENT_QUEUE,
                                                   new HttpUploadEvent(*upload_destination, string(), path, string())))
    {
...
int cf_msg_logger::log(const char *buf, int len, sockaddr_storage *src_ip, sockaddr_storage *dst_ip, cstring method,
                       int reply_code)
{
