...
void sip_trans::retransmit()
{
    ...
    if (logger || sensor) {
        ...
        if (logger) {
            logger->log(retr_buf, retr_len, &src_ip, &retr_addr, msg->u.request->method_str, reply_status);
        }
        if (sensor) {
            sensor->feed(retr_buf, retr_len, &src_ip, &retr_addr, msg_sensor::PTYPE_SIP);
        }
    }
}
...
const char *_state_name_lookup[] = { "0",         "TRYING",    "CALLING",        "PROCEEDING", "PROCEEDING_REL",
...
void sip_trans::reset_timer(trans_timer *t, unsigned int timer_type)
{
    ...
    if (*tp != NULL) {
        ...
        DBG3("Clearing old timer of type %s (timer=%p)(trans=%p)", timer_name((*tp)->type), *tp, this);
...
void trans_timer::fire()
{
    ...
    if (bucket) {
        bucket->lock();
        if (bucket->exist(t)) {
            DBG3("Transaction timer expired: type=%s, trans=%p, eta=%i, t=%i", timer_name(type), t, expires,
                 wheeltimer::instance()->wall_clock);
 ...
 */
void sip_trans::reset_timer(unsigned int timer_type, unsigned int expire_delay /* ms */, unsigned int bucket_id)
{
    ...
    DBG3("New timer of type %s at time=%i (repeated=%i)(trans=%p)", timer_name(timer_type), expires, timer_type >> 16,
         this);
...
{
    DBG3("type=%s (0x%x); msg=%p; to_tag=%.*s;"
         " reply_status=%i; state=%s (%i); retr_buf=%p; timers [%s,%s,%s]\n",
         type_str(), type, msg, to_tag.len, to_tag.s, reply_status, state_str(), state, retr_buf,
