#pragma once
    ...
    (__GNUC__ == 4 && __GNUC_MINOR__ >= 1) &&                                                                          \
        ((defined(__APPLE__) && (defined(__ppc__) || defined(__i386__) || defined(__x86_64__))) ||                     \
         (defined(__linux__) &&                                                                                        \
          ((defined(__i386__) && (defined(__GCC_HAVE_SYNC_COMPARE_AND_SWAP_4))) || defined(__ia64__) ||                \
           defined(__x86_64__) || (defined(__powerpc__) && !defined(__powerpc64__)) || defined(__alpha))))
...
#if HAVE_ATOMIC_CAS
    // ++i;
    unsigned int inc(unsigned int add = 1) { return __sync_add_and_fetch(&i, add); }

    // --i;
    unsigned int dec(unsigned int sub = 1) { return __sync_sub_and_fetch(&i, sub); }
...
#endif
{
  ...
  public:
    ...
    // returns ++ll;
    unsigned long long inc(unsigned long long add = 1) { return __sync_add_and_fetch(&ll, add); }

    // returns --ll;
    unsigned long long dec(unsigned long long sub = 1) { return __sync_sub_and_fetch(&ll, sub); }
...
template <class Reference> class ReferenceGuard {
    Reference *ref;
