...
void SHA256_Update(SHA256_CTX *ctx, const uint8_t *data, size_t len)
{
    size_t i = 0;
    while (len > 0) {
        size_t space = 64 - (ctx->bitcount / 8 % 64);
        size_t chunk = (len < space) ? len : space;
        memcpy(ctx->buffer + (ctx->bitcount / 8 % 64), data + i, chunk);
        ctx->bitcount += chunk * 8;
        if (ctx->bitcount == 0)
            return;
...
{
    uint8_t pad[64] = { 0x80 };
    size_t  padlen  = 64 - (ctx->bitcount / 8 % 64);
    if (padlen < 9)
        padlen += 64;
    uint64_t bitcount = ctx->bitcount;
    for (size_t i = 0; i < 8; ++i)
        pad[padlen - 1 - i] = (bitcount >> (i * 8)) & 0xff;
