    ...
    if (m->type == MT_AUDIO) {
      ...
      if (create_audio) {
        ...
        DBG("[%p] createStreams() created AudioStreamPair for stream %d. %p/%p",
            this, idx, pair.a.getStream(), pair.b.getStream());
	...
	else if(!ignore_relay_streams && canRelay(*m))
    {
      ...
      if (create_relay) {
		...
		DBG("[%p] createStreams() created RelayStreamPair for non-audio stream %d. %p/%p",
			this, idx, &streams_pair->a,&streams_pair->b);
...
void AmB2BMedia::updateStreamPair(AudioStreamPair &pair)
{
  ...
  TRACE("[%p] audio streams %p/%p updated\n",
        this, pair.a.getStream(),pair.b.getStream());
}
  ...
  for (vector<SdpMedia>::const_iterator m = remote_sdp.media.begin(); m != remote_sdp.media.end(); ++m, ++idx) {
    ...
    if (m->type == MT_AUDIO) {
      ...
      DBG("[%p] updateStreams() update AudioStreamPair %p/%p",
          this, astream->a.getStream(),astream->b.getStream());
      ...
      if(a_leg) {
    DBG("[%p] updating A-leg relay_stream %d. %p",this, idx,&relay_stream.a);
        updateRelayStream(&relay_stream.a, a, connection_address, *m, &relay_stream.b);
      }
      else {
    DBG("[%p] updating B-leg relay_stream %d. %p",this, idx,&relay_stream.b);
