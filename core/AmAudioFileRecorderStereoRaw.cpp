...
AmAudioFileRecorderStereoRaw::~AmAudioFileRecorderStereoRaw()
{
    ...
    for (auto &file : files) {
        DBG("~AmAudioFileRecorderStereoRaw(): file %s, offsets %llu-%llu", file.first.c_str(), file.second.begin,
            file.second.end);

        data.header.type             = DATA_META_V2;
        data.data.file_v2.offset     = file.second.begin;
        data.data.file_v2.offset_end = file.second.end ? file.second.end : get_last_ts();

        data.header.size = static_cast<unsigned int>(file.first.size() + sizeof(file_metadata_v2));
    ...
    if (!sync_ctx_id.empty()) {
        if (!AmSessionContainer::instance()->postEvent(HTTP_EVENT_QUEUE, new HttpTriggerSyncContext(sync_ctx_id, 1))) {
...
void AmAudioFileRecorderStereoRaw::writeStereoSamples(unsigned long long ts, unsigned char *samples, size_t size,
