...
unsigned int AmMultiPartyMixer::addChannel(unsigned int external_sample_rate)
{
    ...
    for (std::deque<MixerBufferState>::iterator it = buffer_state.begin(); it != buffer_state.end(); it++) {
        // DBG("XXDebugMixerXX: AmMultiPartyMixer::addChannel(): processing buffer state with sample rate %d",
        // it->sample_rate);
...
void AmMultiPartyMixer::PutChannelPacket(unsigned int channel_id, unsigned long long system_ts, unsigned char *buffer,
                                         unsigned int size)
{
    if (!size)
        return;
...
void AmMultiPartyMixer::GetChannelPacket(unsigned int channel_id, unsigned long long system_ts, unsigned char *buffer,
                                         unsigned int &size, unsigned int &output_sample_rate)
{
    ...
    if (bstate != buffer_state.end() && (channel = bstate->get_channel(channel_id)) != 0) {
        ...
        // DBG("XXDebugMixerXX: GetChannelPacket returned zeroes, ts=%u, last_ts=%u, output_sample_rate=%u", ts,
        // last_ts, output_sample_rate);
    } else {
        /*
...
std::deque<MixerBufferState>::iterator AmMultiPartyMixer::findBufferStateForReading(unsigned int       sample_rate,
...
{
    while (!buffer_state.empty() && (buffer_state.front().last_ts != 0 && buffer_state.front().last_ts < last_ts) &&
