...
void AmCallWatcher::process(AmEvent *ev)
{
    ...
    {
        states_mut.lock();
        DBG("adding  call state '%s'", csu->get_call_id().c_str());

        // check whether already there
        CallStatusMap::iterator it = states.find(csu->get_call_id());
        if (it != states.end()) {
            WARN("implementation error: state '%s' already in list!", csu->get_call_id().c_str());
    ...
    case CallStatusUpdateEvent::Update:
    {
        ...
        if (it != states.end()) {
            ...
            if (it != soft_states.end()) {
                ...
                DBG("received update event for inexistent call '%s'", csu->get_call_id().c_str());
            }
    ...
    case CallStatusUpdateEvent::Obsolete:
    {
        ...
        if (it != states.end()) {
            ...
            soft_states_mut.lock();
            soft_states[csu->get_call_id()] = std::make_pair(cs, now.tv_sec + WATCHER_SOFT_EXPIRE_SECONDS);
            size_t soft_size                = soft_states.size();
            soft_states_mut.unlock();

            DBG("moved call state '%s' to soft-state map (%u states, %u soft-states)", csu->get_call_id().c_str(),
                (unsigned int)s_size, (unsigned int)soft_size);

        } else {
            DBG("received obsolete event for inexistent call '%s'", csu->get_call_id().c_str());
            states_mut.unlock();
        }
    } break;
    }
...
CallStatus *AmCallWatcher::getStatus(const string &call_id)
{
    ...
    if (it != states.end()) {
        ...
        if (it != soft_states.end()) {
            ...
            DBG("erased call state '%s' (%u in list).", call_id.c_str(), (unsigned int)soft_states.size());
        } else {
            DBG("state for call '%s' not found.", call_id.c_str());
        }
        soft_states_mut.unlock();
    }
    return res;
...
void AmCallWatcherGarbageCollector::run()
{
    ...
    while (true) {
        ...
        if (erased) {
            DBG("cleared old soft-states (%u soft-states remaining)", (unsigned int)garbage.size());
        }
        mut.unlock();
    }
}
