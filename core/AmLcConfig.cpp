...
int AmLcConfig::finalizeIpConfig(ConfigContainer* config)
{
    ...
    for(auto if_iterator = config->sip_ifs.begin(); if_iterator != config->sip_ifs.end(); if_iterator++) {
        ...
        for(auto& info : if_iterator->proto_info) {
            ...
            if (insertSIPInterfaceMapping(config, if_iterator->name, *info,if_iterator - config->sip_ifs.begin()) < 0 ||
...
int AmLcConfig::insertSIPInterfaceMapping(ConfigContainer* config, const std::string& ifname, const SIP_info& intf, int idx) {
    //SIP_If_names[intf.name] = idx;
    const std::string &if_local_ip = intf.local_ip;

    std::map<std::string,unsigned short>::iterator it = config->local_sip_ip2if.find(if_local_ip);
    if(it == config->local_sip_ip2if.end()) {
        config->local_sip_ip2if.emplace(if_local_ip,idx);
    } else {
        // two interfaces on the sample IP - the one on port 5060 has priority
        if (intf.local_port == 5060)
            config->local_sip_ip2if.insert(make_pair(if_local_ip,idx));
    }
    return 0;
}
