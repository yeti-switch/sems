...
struct SdpFingerPrint {
    ...
    SdpFingerPrint() {}
    SdpFingerPrint(const SdpFingerPrint &fp)
        : hash(fp.hash)
        , value(fp.value)
    {
    }
...
struct SdpZrtpHash {
    ...
    SdpZrtpHash(const SdpZrtpHash &hash)
        : is_use(hash.is_use)
        , hash(hash.hash)
    {
    }
...
struct SdpMedia {
     ...
     */
    void calcAnswer(const AmPayloadProvider *payload_prov, SdpMedia &answer) const;

    void set_mode_if_missed(bool _send, bool _recv);

    bool is_dtls_udptl() const { return transport == TP_UDPTLSUDPTL; }

    bool is_udptl() const { return transport == TP_UDPTL; }

    bool is_dtls_srtp() const { return transport == TP_UDPTLSRTPSAVP || transport == TP_UDPTLSRTPSAVPF; }

    bool is_simple_srtp() const { return transport == TP_RTPSAVP || transport == TP_RTPSAVPF; }
