...
template <class Parent> class StatLabelsContainer {
  protected:
  ...
  public:
    ...
    const map<string, string> &getLabelsUnsafe() { return labels; }
...
class StatCounterInterface {
  public:
    using iterate_func_type = std::function<void(unsigned long long value,
...
class AtomicCounter : public atomic_int64, public StatCounterInterface, public StatLabelsContainer<AtomicCounter> {
...
class FunctionCounter : public StatCounterInterface, public StatLabelsContainer<FunctionCounter> {
  public:
...
class FunctionGroupCounter : public StatCounterInterface {
  public:
...
class StatCountersGroupsInterface {
  public:
    enum Type { Counter, Gauge, Histogram, Summary, Unknown };

    using iterate_counters_callback_type = std::function<void(unsigned long long value,
                                                              /*unsigned long long timestamp,*/
                                                              const map<string, string> &)>;
  ...
  public:
    ...
    StatCountersGroupsInterface &setHelp(const string &help)
    {
        help_ = help;
        return *this;
    }
...
class StatsCountersGroupsContainerInterface {
  public:
    ...
    using iterate_groups_callback_type =
        std::function<void(const std::string &name, StatCountersGroupsInterface &group)>;
...
class StatCountersSingleGroup final : public StatCountersGroupsInterface, public StatsCountersGroupsContainerInterface {
...
class AmStatistics : public StatLabelsContainer<AmStatistics> {
  private:
    ...
    struct GroupContainerEntry {
        ...
        GroupContainerEntry(StatsCountersGroupsContainerInterface *groups_container, bool managed_by_am_statistics)
  ...
  protected:
    ...
    void dispose()
    {
        for (auto &it : counters_groups_containers) {
            if (it.second.managed_by_am_statistics)
                delete it.second.groups_container;
        }
    }
...
#define stat_group(type, grouping_name, name)                                                                          \
    statistics::instance()->group(StatCountersSingleGroup::type, grouping_name, name)
