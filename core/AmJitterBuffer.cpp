 ...
 */
bool AmJitterBuffer::get(unsigned int ts, unsigned int ms, ShortSample *out_buf, unsigned int *out_size,
                         unsigned int *out_ts)
{
    bool retval = true;
    ...
    if (!m_tsDeltaInited || m_forceResync) {
        ...
        if (ts_less()(ts + m_tsDelta, m_lastTs)) {
            ...
            m_tsDelta += m_lastTs - ts + ms;
#ifdef DEBUG_PLAYOUTBUF
            DBG("Jitter buffer resynced forward (-> %d rel)", m_tsDelta - m_tsDeltaStart);
#endif
            m_delayCount = 0;
            ...
            if (m_delayCount > RESYNC_THRESHOLD) {
                ...
                m_tsDelta -= d / 2;
#ifdef DEBUG_PLAYOUTBUF
                DBG("Jitter buffer resynced backward (-> %d rel)", m_tsDelta - m_tsDeltaStart);
#endif
            } else
                ++m_delayCount;
        } else {
