...
int PortMap::prepare(const std::string &iface_name, unsigned short lo, unsigned short hi, const std::string &family,
                     const std::string &transport)
{
    ...
    start_edge_bit_it        = (ports_state_begin_it * BITS_PER_LONG > low_port) ? 0 : low_port % BITS_PER_LONG;
    start_edge_bit_it_parity = start_edge_bit_it % 2;

    end_edge_bit_it = (ports_state_end_it * BITS_PER_LONG > high_port) ? high_port % BITS_PER_LONG + 1 : BITS_PER_LONG;
    ...
    memset(ports_state_start_addr, mask, (ports_state_end_addr - ports_state_start_addr) * sizeof(unsigned long));
...
bit_is_aquired:
    ...
    next_it = it + 1;
    if (next_it > ports_state_end_addr)
        next_it = ports_state_start_addr;

    ports_state_current_addr.compare_exchange_strong(current_it, next_it);
...
bool PortMap::match_addr(const sockaddr_storage &ss)
{
    ...
    if (ss.ss_family == AF_INET) {
        ...
        return IN6_ARE_ADDR_EQUAL(&(SAv6(&ss))->sin6_addr, &(SAv6(&saddr))->sin6_addr);
    }
    return false;
}
