...
static unsigned int pcm16_bytes2samples(long, unsigned int num_bytes)
{
    return num_bytes / 2;
}

static unsigned int pcm16_samples2bytes(long, unsigned int num_samples)
{
    return num_samples * 2;
}

static unsigned int tevent_bytes2samples(long, unsigned int num_bytes)
{
    return num_bytes;
}

static unsigned int tevent_samples2bytes(long, unsigned int num_samples)
{
    return num_samples;
}

static amci_codec_t _codec_pcm16 = {
    CODEC_PCM16,
    nullptr,
    nullptr,
    nullptr,
...
static amci_codec_t _codec_tevent = {
...
{
    AmPluginFactory* plugin = nullptr; // default: not loaded
    ...
    FactoryCreate fc = nullptr;
    amci_exports_t* exports = static_cast<amci_exports_t *>(dlsym(h_dl,"amci_exports"));
    ...
    if((fc = reinterpret_cast<FactoryCreate>(
        dlsym(h_dl,FACTORY_SESSION_EXPORT_STR))) != nullptr)
    {
        plugin = static_cast<AmPluginFactory*>(fc());
        if(loadAppPlugIn(plugin))
            goto error;
        has_sym = true;
        if (nullptr != plugin) plugins.push_back(plugin);
    }

    if((fc = reinterpret_cast<FactoryCreate>(
        dlsym(h_dl,FACTORY_SESSION_EVENT_HANDLER_EXPORT_STR))) != nullptr)
    {
        plugin = static_cast<AmPluginFactory*>(fc());
    ...
    if((fc = reinterpret_cast<FactoryCreate>(
        dlsym(h_dl,FACTORY_PLUGIN_EXPORT_STR))) != nullptr)
    {
        plugin = static_cast<AmPluginFactory*>(fc());
    ...
    if((fc = reinterpret_cast<FactoryCreate>(
        dlsym(h_dl,FACTORY_PLUGIN_CLASS_EXPORT_STR))) != nullptr)
    {
        plugin = static_cast<AmPluginFactory*>(fc());
    ...
    if((fc = reinterpret_cast<FactoryCreate>(
        dlsym(h_dl,FACTORY_LOG_FACILITY_EXPORT_STR))) != nullptr)
    {
        plugin = static_cast<AmPluginFactory*>(fc());
...
void AmPlugIn::getPayloads(vector<SdpPayload>& pl_vec) const
{
    ...
    if(te_pl_it != payloads.end()) {
        pl_vec.insert(
            pl_vec.begin() + static_cast<long>(te_pl_pos),
            SdpPayload(
                te_pl_it->first,
                te_pl_it->second->name,
                te_pl_it->second->advertised_sample_rate,
                te_pl_it->second->channels==2?2:0));
    }
}
...
bool AmPlugIn::registerDIInterface(const string& di_name,
