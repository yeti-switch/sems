...
const char *log_level2str[] = { "ERROR", "WARNING", "INFO", "DEBUG", "DEBUG2", "DEBUG3" };
...
class SyslogLogFac : public AmLoggingFacility {
  ...
  public:
    ...
    void log(int, pid_t, pid_t, const char *, const char *, int, const char *, int) override;

    static SyslogLogFac &instance()
    {
        if (!_instance)
            _instance = new SyslogLogFac();
        return *_instance;
    }
...
bool SyslogLogFac::setFacility(const char *str, const char *name)
{
    ...
    } else if (strlen(str) == 6 && !strncmp(str, "LOCAL", 5) && isdigit(str[5]) && str[5] - '0' < 8) {
...
void SyslogLogFac::log(int level, [[maybe_unused]] pid_t pid, pid_t tid, const char *func, const char *file, int line,
                       const char *msg, int msg_len)
{
    static const int log2syslog_level[] = { LOG_ERR, LOG_WARNING, LOG_INFO, LOG_DEBUG, LOG_DEBUG, LOG_DEBUG };
#ifdef _DEBUG
    ...
    syslog(log2syslog_level[level], "%s: %s [#%lx] [%s %s:%d]", log_level2str[level], msg, (unsigned long)tid, func,
           file, line);
#else
    syslog(log2syslog_level[level], "[#%lx] [%s, %s:%d] %s: %.*s", (unsigned long)tid, func, file, line,
           log_level2str[level], msg_len, msg);
#endif
#else /* NO_THREADID_LOG */
#ifdef LOG_LOC_DATA_ATEND
    syslog(log2syslog_level[level], "%s: %s [%s] [%s:%d]", log_level2str[level], msg, func, file, line);
#else
    syslog(log2syslog_level[level], "[%s, %s:%d] %s: %s", func, file, line, log_level2str[level], msg);
#endif
#endif /* NO_THREADID_LOG */

#else /* !_DEBUG */
#ifdef LOG_LOC_DATA_ATEND
    syslog(log2syslog_level[level], "%s: %s [%s:%d]", log_level2str[level], msg, file, line);
#else
    syslog(log2syslog_level[level], "[%u/%s:%d] %s: %s", tid, file, line, log_level2str[level], msg);
#endif
  ...
  public:
             ...
             int msg_len_)
    {
        ...
        fprintf(stderr,
                ...
                now.tv_sec, now.tv_usec, LOC_DATA, log_level2str[level_], msg_len_, msg_);
