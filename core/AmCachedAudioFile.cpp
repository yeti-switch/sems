...
int AmFileCache::load(const std::string &filename)
{
    ...
    if ((fd = open(name.c_str(), O_RDONLY)) == -1) {
        ERROR("while opening file '%s' for caching.", filename.c_str());
        return -1;
    }

    if (fstat(fd, &sbuf) == -1) {
        ERROR("cannot stat file '%s'.", name.c_str());
        close(fd);
        return -2;
    }

    if ((data = mmap((caddr_t)0, sbuf.st_size, PROT_READ, MAP_PRIVATE, fd, 0)) == (caddr_t)(-1)) {
        ERROR("cannot mmap file '%s'.", name.c_str());
        close(fd);
        return -3;
    }
    ...
    , loop(false)
{
    ...
    if (!f_fmt) {
        ERROR("while trying to determine the format of '%s'", cache->getFilename().c_str());
        return;
    }
    fmt.reset(f_fmt);
    ...
    if (iofmt->mem_open && !(ret = (*iofmt->mem_open)((unsigned char *)cache->getData(), cache->getSize(), &ofpos, &fd,
...
int AmCachedAudioFile::read(unsigned int user_ts, unsigned int size)
{
    ...
    if (ret > 0 && (unsigned int)ret < size) {
        DBG("0-stuffing packet: adding %i bytes (packet size=%i)", size - ret, size);
