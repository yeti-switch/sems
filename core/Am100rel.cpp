...
int Am100rel::onRequestIn(const AmSipRequest &req)
{
    ...
    if (req.method == SIP_METH_INVITE) {
        ...
        case REL100_SUPPORTED_NOT_ANNOUNCED:
            if (key_in_list(getHeader(req.hdrs, SIP_HDR_SUPPORTED, SIP_HDR_SUPPORTED_COMPACT), SIP_EXT_100REL) ||
                key_in_list(getHeader(req.hdrs, SIP_HDR_REQUIRE), SIP_EXT_100REL))
            {
        ...
        case REL100_REQUIRE: /* if support is required, reject if UAC doesn't */
                  ...
                  key_in_list(getHeader(req.hdrs, SIP_HDR_REQUIRE), SIP_EXT_100REL)))
            {
                ...
                AmBasicSipDialog::reply_error(req, 421, SIP_REPLY_EXTENSION_REQUIRED,
                                              SIP_HDR_COLSP(SIP_HDR_REQUIRE) SIP_EXT_100REL CRLF, dlg->getMsgLogger());
                if (hdl)
                    hdl->onFailure();
        ...
        case REL100_DISABLED:
            ...
            break;
        default:
            ERROR("BUG: unexpected value `%d' for '" SIP_EXT_100REL "' switch. callid: %s", uas_state,
                  req.callid.c_str());
#ifndef NDEBUG
            abort();
#endif
        ...
        if (uas_state != REL100_REQUIRE) {
            WARN("unexpected PRACK received while " SIP_EXT_100REL " not active. callid: %s", req.callid.c_str());
            // let if float up
        } else if (rseq_1st <= req.rseq && req.rseq <= rseq) {
            if (req.rseq == rseq) {
                rseq_confirmed = true; // confirmed
                AmSessionContainer::instance()->postEvent(dlg->getLocalTag(), new ProvisionalReplyConfirmedEvent());
            }
            // else: confirmation for one of the pending 1xx
            DBG("%sRSeq (%u) confirmed. callid: %s", (req.rseq == rseq) ? "latest " : "", rseq, req.callid.c_str());
        }
    }
    return 1;
}
...
int Am100rel::onReplyIn(const AmSipReply &reply)
{
    ...
    if (dlg->getStatus() != AmSipDialog::Trying && dlg->getStatus() != AmSipDialog::Proceeding &&
        dlg->getStatus() != AmSipDialog::Early && dlg->getStatus() != AmSipDialog::Connected)
    {
        return 1;
    }
    ...
    if (100 < reply.code && reply.code < 200 && reply.cseq_method == SIP_METH_INVITE) {
        ...
        case REL100_SUPPORTED_NOT_ANNOUNCED:
            ...
            // no break!
                ...
                break;
        case REL100_REQUIRE:
            if (!key_in_list(getHeader(reply.hdrs, SIP_HDR_REQUIRE), SIP_EXT_100REL) || !reply.rseq) {
                ERROR(SIP_EXT_100REL " not supported or no positive RSeq value in "
                                     "(reliable) 1xx. callid: %s",
                      reply.callid.c_str());
                dlg->bye();
                if (hdl)
                    hdl->onFailure();
                ...
                if (hdl)
                    ((AmSipDialogEventHandler *)hdl)->onInvite1xxRel(reply);
        ...
        case REL100_DISABLED:
            ...
            break;
        default:
            ERROR("BUG: unexpected value `%d' for " SIP_EXT_100REL " switch. callid: %s", uac_state,
                  reply.callid.c_str());
#ifndef NDEBUG
            abort();
#endif
        ...
        if (300 <= reply.code) {
            ...
            WARN("received '%d' for " SIP_METH_PRACK " method. callid: %s", reply.code, reply.callid.c_str());
        }
...
void Am100rel::onRequestOut(AmSipRequest &req)
{
    ...
    case REL100_DISABLED:
    case REL100_IGNORED:
    case REL100_SUPPORTED_NOT_ANNOUNCED: return;
    case REL100_SUPPORTED:
        if (!key_in_list(getHeader(req.hdrs, SIP_HDR_REQUIRE), SIP_EXT_100REL))
    ...
    case REL100_REQUIRE:
        ...
        break;
    default: ERROR("BUG: unexpected reliability switch value of '%d'. callid: %s", uac_state, req.callid.c_str());
    }
}

void Am100rel::onReplyOut(AmSipReply &reply)
{
    if (uas_state == REL100_IGNORED || uas_state == REL100_SUPPORTED_NOT_ANNOUNCED) {
        return;
    }
...
void Am100rel::onTimeout(const AmSipRequest &req, const AmSipReply &rpl)
{
    ...
    INFO("reply <%s> timed out (not PRACKed). callid: %s", rpl.print().c_str(), req.callid.c_str());
    if (100 < rpl.code && rpl.code < 200 && uas_state == REL100_REQUIRE && rseq == rpl.rseq &&
        rpl.cseq_method == SIP_METH_INVITE)
    {
        INFO("reliable %d reply timed out; rejecting request. callid: %s", rpl.code, req.callid.c_str());
        if (hdl)
            hdl->onNoPrack(req, rpl);
    } else {
