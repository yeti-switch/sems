...
{
    DBG("arg(): arg_ptr: %p, arg_ptr, amarg: %p", arg_ptr, dynamic_cast<AmArg *>(arg_ptr));
    ...
    : type(Undef)
{
    assertStruct();
    INC_AMARGSIZE(sizeof(*this));
    for (std::map<std::string, AmArg>::iterator it = v.begin(); it != v.end(); it++) {
        INC_AMARGSIZE(strlen(it->first.c_str()));
...
void AmArg::assertType(Type expected_type) const
{
    if (type != expected_type) {
        ERROR("type mismatch: expected: %d; received: %d.", expected_type, type);
        throw TypeMismatchException();
    }
}
...
size_t AmArg::getAllocatedSize()
{
    ...
    switch (type) {
    case CStr: size += strlen(v_cstr); break;
    case Blob: size += v_blob->len + sizeof(*v_blob); break;
    case Array:
    ...
    case Struct:
        ...
        break;
    case Reference: size += v_ref->arg().getAllocatedSize(); break;
    default:        break;
    }
    return size;
}
...
std::string AmArg::print() const
{
    ...
    switch (type) {
    case Undef:    return "";
    case Int:      return v_int < 0 ? "-" + long2str(abs(v_int)) : long2str(v_int);
    case LongLong: return longlong2str(v_long);
    case Bool:     return v_bool ? "true" : "false";
    case Double:
        if (v_double == std::numeric_limits<double>::infinity())
            return "\"inf\"";
        return double2str(v_double);
    case CStr:    return "'" + string(v_cstr) + "'";
    case AObject: return "<Object>";
    case ADynInv: return "<DynInv>";
    case Blob:    s = "<Blob of size:" + int2str(v_blob->len) + ">";
    case Array:
        s = "[";
    ...
    case Struct:
        s = "{";
        for (AmArg::ValueStruct::const_iterator it = v_struct->begin(); it != v_struct->end(); it++) {
            s += "'" + it->first + "': ";
            s += print(it->second);
            s += ", ";
        }
        ...
        s += "}";
        return s;
    case Reference: s = print(v_ref->arg()); return s;
    default:        break;
    }
    return "<UNKONWN TYPE>";
}
...
std::vector<std::string> AmArg::enumerateKeys() const
{
    ...
    for (ValueStruct::iterator it = v_struct->begin(); it != v_struct->end(); it++) {
...
void AmArg::assertArrayFmt(const char *format) const
{
    ...
    try {
        for (size_t i = 0; i < fmt_len; i++) {
            switch (format[i]) {
            case 'i':
                get(i).assertType(Int);
                got += 'i';
                break;
            case 'l':
                get(i).assertType(LongLong);
                got += 'l';
                break;
            case 't':
                get(i).assertType(Bool);
                got += 't';
                break;
            case 'f':
                get(i).assertType(Double);
                got += 'f';
                break;
            case 's':
                get(i).assertType(CStr);
                got += 's';
                break;
            case 'o':
                get(i).assertType(AObject);
                got += 'o';
                break;
            case 'd':
                get(i).assertType(ADynInv);
                got += 'd';
                break;
            case 'a':
                get(i).assertType(Array);
                got += 'a';
                break;
            case 'b':
                get(i).assertType(Blob);
                got += 'b';
                break;
            case 'u':
                get(i).assertType(Struct);
                got += 'u';
                break;
            default:
                got += '?';
                ERROR("ignoring unknown format type '%c'", format[i]);
                break;
            }
        }
    } catch (...) {
        ERROR("parameter mismatch: expected '%s', got '%s...'", format, got.c_str());
        throw;
    }
}
...
bool operator==(const AmArg &lhs, const AmArg &rhs)
{
    ...
    case AmArg::Blob:
    {
        return (lhs.v_blob->len == rhs.v_blob->len) && !memcmp(lhs.v_blob->data, rhs.v_blob->data, lhs.v_blob->len);
    } break;
...
{
    switch (a.getType()) {
    case AmArg::Undef:    return "";
    case AmArg::CStr:     return a.asCStr();
    case AmArg::Int:      return int2str(a.asInt());
    case AmArg::LongLong: return longlong2str(a.asLongLong());
    case AmArg::Double:   return double2str(a.asDouble());
    case AmArg::Bool:     return int2str(a.asBool());
    default:              throw std::string("can't convert arg to string");
    }
}
