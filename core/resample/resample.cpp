...
inline float ResampleSincMono::downsample(float *src, unsigned int current, unsigned int curfrc,
                                          unsigned long long sinc_increment, float scale)
{
...
int ResampleSincMono::resample(signed short *dst, float rate, unsigned num_samples)
{
    ...
    if (rate < 1.0) { // downsampling, need to scale sinc apropriatly
        ...
        while (num_samples > 0) {
            ...
            if (current >= samples.size() - filter_delay)
                break;
...
inline float ResampleSincStereo::downsample(float *src, unsigned int current, unsigned int curfrc,
                                            unsigned long long sinc_increment, float scale)
{
    long long sinc_current;
    float     samp;
...
int ResampleSincStereo::resample(signed short *dst, float rate, unsigned num_samples)
{
    ...
    if (rate < 1.0) { // downsampling, need to scale sinc apropriatly
        ...
        while (num_samples > 0) {
            ...
            if ((current * 2) >= samples.size() - filter_delay - 1)
                break;
...
Resample *ResampleFactory::createResampleObj(bool doPad, float maxRatio, interpolType interpol_type,
