...
class ResampleSincMono : public Resample {
  ...
  private:
    ...
    static inline float downsample(float *src, unsigned int current, unsigned int curfrc,
...
class ResampleFactory {
  public:
    enum interpolType { INTERPOL_LINEAR, INTERPOL_SINC };

    enum sampleType { SAMPLE_MONO, SAMPLE_STEREO };
