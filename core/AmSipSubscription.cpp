...
const char *__sub_state_str[] = { "init", "notify_wait", "pending", "active", "terminated" };

SingleSubscription::SingleSubscription(AmSipSubscription *subs, Role role, const string &event, const string &id)
...
{
    DBG("[%p] tag=%s;role=%s timer_id = %s", this, dlg()->getLocalTag().c_str(), role ? "Notifier" : "Subscriber",
    ...
    case SUBSCRIPTION_EXPIRE:
        if (subs->ev_q) {
            AmEvent *ev = new SingleSubTimeoutEvent(subs->dlg->getLocalTag(), timer_id, this);
...
string SingleSubscription::getNotifyHeaders()
{
    ...
    case SubState_active:
        ...
        hdrs += CRLF;
        break;
    case SubState_pending: hdrs += SIP_HDR_COLSP(SIP_HDR_SUBSCRIPTION_STATE) "pending" CRLF; break;
    default:               break;
    }
    return hdrs;
}

void SingleSubscription::sendReferNotify(AmBasicSipDialog *sip_dlg, string &body, bool terminate_sub,
                                         const string &reason)
{
    if (role != Notifier) {
...
SingleSubscription *AmSipSubscription::newSingleSubscription(SingleSubscription::Role role, const string &event,
                                                             const string &id)
{
    return new SingleSubscription(this, role, event, id);
}

SingleSubscription *AmSipSubscription::makeSubscription(const AmSipRequest &req, bool uac)
{
    SingleSubscription::Role role = uac ? SingleSubscription::Subscriber : SingleSubscription::Notifier;
...
{
    return "[" + str2json(event) + "," + str2json(id) + "," + (role == Subscriber ? str2json("SUB") : str2json("NOT")) +
...
bool AmSipSubscription::subscriptionExists(SingleSubscription::Role role, const string &event, const string &id)
{
    return findSubscription(role, event, id) != subs.end();
}

AmSipSubscription::Subscriptions::iterator AmSipSubscription::findSubscription(SingleSubscription::Role role,
                                                                               const string &event, const string &id)
{
    Subscriptions::iterator match = subs.end();
    bool                    no_id = id.empty() && (event == "refer");

    DBG("searching for event='%s'; id='%s'; no_id=%i", event.c_str(), id.c_str(), no_id);

    for (Subscriptions::iterator it = subs.begin(); it != subs.end(); it++) {

        SingleSubscription *sub = *it;

        DBG("role='%s';event='%s';id='%s'", sub->role ? "Notifier" : "Subscriber", sub->event.c_str(), sub->id.c_str());

        if ((sub->role == role) && (sub->event == event) && (no_id || (sub->id == id))) {
...
AmSipSubscription::Subscriptions::iterator AmSipSubscription::createSubscription(const AmSipRequest &req, bool uac)
{
...
void AmSipSubscription::removeSubFromUASCSeqMap(Subscriptions::iterator sub)
{
    for (CSeqMap::iterator i = uas_cseq_map.begin(); i != uas_cseq_map.end();) {
        if (i->second == sub) {
            ...
            if (req) {
                DBG("found request(cseq=%i): replying 481 to pending UAS transaction", req->cseq);
 ...
 * if none, create one
 */
AmSipSubscription::Subscriptions::iterator AmSipSubscription::matchSubscription(const AmSipRequest &req, bool uac)
{
    if ((!uac && req.to_tag.empty()) || (uac && dlg->getRemoteTag().empty()) || (req.method == SIP_METH_REFER) ||
        subs.empty())
    {
...
bool AmSipSubscription::onReplyIn(const AmSipRequest &req, const AmSipReply &reply)
{
    // UAC side
...
void AmSipSubscription::onReplySent(const AmSipRequest &req, const AmSipReply &reply)
{
    // UAS side
...
bool AmSipSubscription::sendReferNotify(AmBasicSipDialog *sip_dlg, const string &id, string &body, bool terminate_sub,
                                        const string &reason)
{
    Subscriptions::iterator sub_it = findSubscription(SingleSubscription::Notifier, "refer", id);
    if (sub_it == subs.end()) {
...
SIPSubscriptionEvent::SIPSubscriptionEvent(SubscriptionStatus status, const string &handle, unsigned int expires,
...
AmSipSubscriptionDialog::AmSipSubscriptionDialog(const AmSipSubscriptionInfo &info, const string &sess_link,
                                                 AmEventQueue *ev_q)
...
void AmSipSubscriptionDialog::onNotify(const AmSipRequest &req, SingleSubscription *sub)
{
    assert(sub);
    ...
    SIPSubscriptionEvent *sub_ev = new SIPSubscriptionEvent(SIPSubscriptionEvent::SubscribeFailed, local_tag);
    ...
    case SingleSubscription::SubState_active:
        ...
        sub_ev->expires = sub->getExpires();
        break;
    case SingleSubscription::SubState_terminated: sub_ev->status = SIPSubscriptionEvent::SubscribeTerminated; break;
    default:                                      break;
    }
...
void AmSipSubscriptionDialog::onFailureReply(const AmSipReply &reply, SingleSubscription *sub)
{
    assert(sub);
    SIPSubscriptionEvent *sub_ev =
        new SIPSubscriptionEvent(SIPSubscriptionEvent::SubscribeFailed, local_tag, 0, reply.code, reply.reason);
...
void AmSipSubscriptionDialog::onTimeout(int timer_id, SingleSubscription *sub)
{
    ...
    SIPSubscriptionEvent *sub_ev = new SIPSubscriptionEvent(SIPSubscriptionEvent::SubscriptionTimeout, local_tag);
