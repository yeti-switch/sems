...
inline bool ts_less::operator()(const unsigned int &l, const unsigned int &r) const
{
    return (l - r > (1U << 31));
}

inline bool sys_ts_less::operator()(const unsigned long long &l, const unsigned long long &r) const
{
...
template <typename T> void SampleArray<T>::clear(unsigned int start_ts, unsigned int end_ts)
{
...
template <typename T> void SampleArray<T>::write(unsigned int ts, T *buffer, unsigned int size)
{
...
template <typename T> void SampleArray<T>::read(unsigned int ts, T *buffer, unsigned int size)
{
    unsigned int off = ts & (SIZE_MIX_BUFFER - 1);
...
template <typename T> void SampleArray<T>::put(unsigned int ts, T *buffer, unsigned int size)
{
...
template <typename T> void SampleArray<T>::get(unsigned int ts, T *buffer, unsigned int size)
{
    // assert(size <= SIZE_MIX_BUFFER);

    if (!init || !ts_less()(ts, last_ts) || !ts_less()(last_ts - SIZE_MIX_BUFFER, ts + size)) {
