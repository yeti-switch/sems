...
struct B2ABConnectLegEvent : public B2ABEvent {
    ...
    B2ABConnectLegEvent(const string &remote_party, const string &remote_uri, const string &local_party,
                        const string &local_uri, const string &callgroup, const string &headers = "")
...
class AmB2ABCallerSession : public AmB2ABSession {
  public:
    enum CalleeStatus { None = 0, NoReply, Early, Ringing, Connected };

  private:
    // Callee Status
    CalleeStatus callee_status;

    virtual void setupCalleeSession(AmB2ABCalleeSession *callee_session, B2ABConnectLegEvent *ev);
  ...
  public:
    ...
    void connectCallee(const string &remote_party, const string &remote_uri, const string &local_party,
...
class AmB2ABCalleeSession : public AmB2ABSession {
  ...
  public:
    AmB2ABCalleeSession(const string &other_local_tag, AmSessionAudioConnector *callers_connector);
    ~AmB2ABCalleeSession();

    void onEarlySessionStart();
    void onSessionStart();
    void onSipReply(const AmSipRequest &req, const AmSipReply &rep, AmSipDialog::Status old_dlg_status);
