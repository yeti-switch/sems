...
void AmDtmfEventQueue::putDtmfAudio(bool &dtmf_detected, const unsigned char *buf, int size,
...
{
    switch (type) {
    case Dtmf::DTMF_RELAY: parseDtmfRelayBody(request_body); break;
    case Dtmf::DTMF:       parseDtmfBody(request_body); break;
    }
}
...
void AmSipDtmfEvent::parseLine(const string &line)
{
    ...
    if (line.length() > KeySignal.length() && line.substr(0, KeySignal.length()) == KeySignal) {
        string event(line.substr(KeySignal.length(), string::npos));
        m_event = str2id(event);
    } else if (line.length() > KeyDuration.length() && line.substr(0, KeyDuration.length()) == KeyDuration) {
...
void AmDtmfDetector::process(AmEvent *evt)
{
    ...
    case Dtmf::SOURCE_RTP:
        ...
        //            break;
    case Dtmf::SOURCE_SIP: m_sipDetector.process(dynamic_cast<AmSipDtmfEvent *>(event)); break;
    }
    evt->processed = true;
}
...
void AmDtmfDetector::registerKeyReleased(int event, int volume, Dtmf::EventSource source, const struct timeval &start,
                                         const struct timeval &stop, bool has_eventid, unsigned int event_id)
{
...
void AmDtmfDetector::registerKeyPressed(int event, int volume, Dtmf::EventSource type, bool has_eventid,
                                        unsigned int event_id)
{
#ifdef EXCESSIVE_DTMF_DEBUGINFO
    DBG("registerKeyPressed(%d, .., %s, %u); m_eventPending=%s, m_currentEvent=%d, "
        "m_current_eventid=%u,m_current_eventid_i=%s\n",
        event, has_eventid ? "true" : "false", event_id, m_eventPending ? "true" : "false", m_currentEvent,
        m_current_eventid, m_current_eventid_i ? "true" : "false");
#endif
    struct timeval tm;
    ...
    if (!m_eventPending) {
        ...
        // Old event has not been sent yet
        // push out it now
        if ((m_currentEvent != event) || (has_eventid && m_current_eventid_i && (event_id != m_current_eventid))) {
#ifdef EXCESSIVE_DTMF_DEBUGINFO
        ...
        long delta_msec = (tm.tv_sec - m_lastReportTime.tv_sec) * 1000 + (tm.tv_usec - m_lastReportTime.tv_usec) / 1000;
...
void AmDtmfDetector::checkTimeout()
{
    ...
    if (m_eventPending) {
        if (m_sipEventReceived && m_rtpEventReceived && m_inbandEventReceived) {
            ...
            gettimeofday(&tm, NULL);
            long delta_msec =
                (tm.tv_sec - m_lastReportTime.tv_sec) * 1000 + (tm.tv_usec - m_lastReportTime.tv_usec) / 1000;
...
void AmDtmfDetector::reportEvent()
{
    ...
    long duration =
        (m_lastReportTime.tv_sec - m_startTime.tv_sec) * 1000 + (m_lastReportTime.tv_usec - m_startTime.tv_usec) / 1000;

    DBG("AmDtmfDetector::reportEvent() event: %d, duration: %ld, pending: %d, sip: %d, rtp: %d, inband: %d",
        m_currentEvent, duration, m_eventPending, m_sipEventReceived, m_rtpEventReceived, m_inbandEventReceived);

    Dtmf::EventSource source = Dtmf::SOURCE_DETECTOR;
    if (m_rtpEventReceived)
        source = Dtmf::SOURCE_RTP;
    else if (m_inbandEventReceived)
        source = Dtmf::SOURCE_INBAND;
    else if (m_sipEventReceived)
        source = Dtmf::SOURCE_SIP;

    m_dtmfSink->postDtmfEvent(new AmDtmfEvent(m_currentEvent, duration, m_currentVolume, source));
...
void AmRtpDtmfDetector::process(AmRtpDtmfEvent *evt)
{
    ...
    // ITU-T Q.24A)
    {
        ...
        if (!m_eventPending) {
            ...
            if ((evt->event() != m_currentEvent) || (m_currentTS_i && (evt->ts() != m_currentTS))) {
...
#endif
        m_keysink->registerKeyPressed(m_currentEvent, m_currentVolume, Dtmf::SOURCE_RTP, true, m_currentTS);
    }
}
...
#endif
        m_keysink->registerKeyReleased(m_currentEvent, m_currentVolume, Dtmf::SOURCE_RTP, m_startTime, end_time, true,
                                       m_currentTS);
...
void AmSemsInbandDtmfDetector::isdn_audio_goertzel_relative()
{
    ...
    for (int k = 0; k < REL_NCOEFF; k++) {
        ...
        */
        ...
        m_result[k] = ((sk * sk) >> REL_AMP_BITS) - ((((rel_cos2pik[k] * sk) >> 15) * sk2) >> REL_AMP_BITS) +
...
void AmSemsInbandDtmfDetector::isdn_audio_calc_dtmf(bool &dtmf_detected, const signed short *buf, int len,
                                                    unsigned int ts)
{
    int c;
...
int AmSemsInbandDtmfDetector::streamPut(bool &dtmf_detected, const unsigned char *samples, unsigned int size,
                                        unsigned long long system_ts)
{
    unsigned long long user_ts = system_ts * ((unsigned long long)SAMPLERATE / 100) / (WALLCLOCK_RATE / 100);
...
int AmSpanDSPInbandDtmfDetector::streamPut(bool &dtmf_detected, const unsigned char *samples, unsigned int size,
                                           unsigned long long system_ts)
{
