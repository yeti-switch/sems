...
int main(int argc, char *argv[])
{
    ...
    if (!codec) {
        ERROR("can't load codec with id %d for payload %s", payload->codec_id, payload->name);
        return EXIT_FAILURE;
    }

    INFO("will use payload %s with codec_id %d, payload_id: %d", payload->name, payload->codec_id, payload->payload_id);
