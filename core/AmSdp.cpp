...
static void parse_sdp_attr(AmSdp* sdp_msg, char* s, char** next_line);
...
void AmSdp::print(string& body) const
{
  string out_buf = "v="+int2str(version)+"\r\n"
    "o="+origin.user+" "+long2str(origin.sessId)+" "+
    long2str(origin.sessV)+" IN ";
...
static void parse_sdp_attr(AmSdp* sdp_msg, char* s, char** next_line)
{
  ...
  *next_line = line_end;
  ...
  if(attr == "rtpmap"){
    ...
    if (parsing) {
      next = parse_until(attr_line, line_end,'\r');
      if(next < line_end){
	string value(attr_line, int(next-attr_line)-1);
	if (value == "active") {
