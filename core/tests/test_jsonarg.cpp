...
FCTMF_SUITE_BGN(test_jsonarg) {
    ...
    FCT_TEST_BGN(json_number_e_powneg1) {
      ...
      AmArg rpc_params;
      fct_chk(json2arg(s.c_str(), rpc_params));
      // DBG("res: %s, type %d",  AmArg::print(rpc_params["result"]).c_str(), rpc_params["result"].getType());
    ...
    FCT_TEST_BGN(json_tofro_equality) {
      ...
      // fct_chk(a1==a2);
      // DBG("a1 = '%s', a2 = '%s', ", AmArg::print(a1).c_str(), AmArg::print(a2).c_str());
    } FCT_TEST_END();
