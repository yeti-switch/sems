...
static const char *seq_desc[] = { "00-7F",
                                  "C2-DF,80-BF",
                                  "E0,A0-BF,80-BF",
                                  "E1-EC,80-BF,80-BF",
                                  "ED,80-9F,80-BF",
...
static const char *utf8_state2seq_description[] = { seq_desc[0], seq_desc[1], seq_desc[2], seq_desc[2], seq_desc[3],
                                                    seq_desc[3], seq_desc[4], seq_desc[4], seq_desc[5], seq_desc[5],
                                                    seq_desc[6], seq_desc[6], seq_desc[6], seq_desc[7], seq_desc[7],
                                                    seq_desc[7], seq_desc[8], seq_desc[8], seq_desc[8] };

static const int utf8_state2byte_num[]     = { 1, 2, 2, 3, 2, 3, 2, 3, 2, 3, 2, 3, 4, 2, 3, 4, 2, 3, 4 };
static const int utf8_state2seq_byte_num[] = { 1, 2, 3, 3, 3, 3, 3, 3, 3, 3, 4, 4, 4, 2, 4, 4, 4, 4, 4 };

#define VALIDATE_RANGE(condition_state)                                                                                \
    case condition_state:                                                                                              \
        if (IS_IN(c, utf8_state2transition[condition_state].start, utf8_state2transition[condition_state].end)) {      \
            ...
            DBG("unexpected value %02X at %d byte in %d bytes sequence. "                                              \
                ...
                "pattern: %s",                                                                                         \
                c, utf8_state2byte_num[st], utf8_state2seq_byte_num[st], utf8_state2transition[condition_state].start, \
                utf8_state2transition[condition_state].end, utf8_state2seq_description[st]);                           \
...
#define SEQUENCE_CLASSIFIER_CASE(value, next_state)                                                                    \
    case value: st = next_state; continue;
...
#define INVALID_SEQUENCE_START_ACTION(st)                                                                              \
    ;                                                                                                                  \
    return false;
    ...
    for (const unsigned char c : s) {
        ...
        default: ERROR("unknown state: %d", st); return false;
        }
    }
    ...
    if (st != ST_COMPLETED) {
        DBG("incompleted utf8 multibyte sequence. "
            ...
            utf8_state2seq_description[st], utf8_state2byte_num[st] - 1);
        return false;
    }
    return true;
...
#define INVALID_SEQUENCE_START_ACTION()                                                                                \
    ...
    if (st != ST_COMPLETED) {
        ...
        DBG("incompleted utf8 multibyte sequence. "
            "pattern: %s, last checked byte in the sequence: %d",
            utf8_state2seq_description[st], bytes_to_trim);
        pos -= bytes_to_trim;
        s.erase(pos, bytes_to_trim);
    }
    ...
    if (modified) {
        DBG("fixup_utf8_inplace(): erased %ld invalid bytes. "
            "resulting size: %ld. "
            "resulting string: \"%s\"",
            original_size - s.length(), s.length(), s.c_str());
    }
    return modified;
#undef INVALID_SEQUENCE_ACTION
#undef INVALID_SEQUENCE_START_ACTION
