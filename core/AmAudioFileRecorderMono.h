...
struct AudioRecorderSamplesEvent : AudioRecorderEvent {
    unsigned char data[AUDIO_BUFFER_SIZE];
    size_t        data_size;
    int           sample_rate;

    AudioRecorderSamplesEvent(const string &recorder_id, const unsigned char *samples, size_t len,
...
class AmAudioFileRecorderMono : public AmAudioFileRecorder {
...
#define RecorderPutSamples(id, buffer, size, rate)                                                                     \
    AmAudioFileRecorderProcessor::instance_unsafe()->putEvent(new AudioRecorderSamplesEvent(id, buffer, size, rate));
