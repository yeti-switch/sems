...
AmMediaState* AmMediaDtlsState::init(const AmArg& args)
{
    ...
    transport->addConnection(
        transport->getConnFactory()->createDtlsConnection(address, port, dtls_context));

    return this;
}

AmMediaState* AmMediaDtlsState::update(const AmArg& args)
{
    string address = args["address"].asCStr();
    int port = args["port"].asInt();
    is_dtls_srtp = args["dtls_srtp"].asBool();
    ...
    } else { // if not dtls+srtp consider as dtls+udptl
