...
AmSessionContainer::AmSessionContainer()
    ...
    , CPSHardLimit(0)
{
    stat_group(Gauge, "core", "avg_session_num").addFunctionCounter([]() -> unsigned long long {
        return AmSession::getAvgSessionNum();
    });

    stat_group(Gauge, "core", "max_session_num").addFunctionCounter([]() -> unsigned long long {
    ...
    session_num_group.addFunctionCounter([]() -> unsigned long long { return AmSession::getSessionNum(); });

    stat_group(Gauge, "core", "dead_sessions_count").addFunctionCounter([]() -> unsigned long long {
...
bool AmSessionContainer::clean_sessions()
{
    ...
    try {
        ...
        /*DBG("Session cleaner: dead sessions container processed. n_sessions contains: %ld  sessions. swap it with dead
           sessions", n_sessions.size());*/
...
void AmSessionContainer::stopAndQueue(AmSession *s)
{
    ...
    if (AmConfig.log_sessions) {
        INFO("session cleaner about to stop %s", s->getLocalTag().c_str());
    }
    ...
    try {
        ...
        if (session.get() != 0) {
            ...
            case AmSessionContainer::ShutDown:
                throw AmSession::Exception(AmConfig.shutdown_mode_err_code, AmConfig.shutdown_mode_err_reason);
            case AmSessionContainer::AlreadyExist: throw AmSession::Exception(482, SIP_REPLY_LOOP_DETECTED);
            default:                               ERROR("adding session to session container"); throw string(SIP_REPLY_SERVER_INTERNAL_ERROR);
            }

            if (session->autoSendInviteUAC()) {
#ifdef USE_MONITORING
                MONITORING_LOG5(req.from_tag, "app", app_name.c_str(), "dir", "out", "from", req.from.c_str(), "to",
                                req.to.c_str(), "ruri", req.r_uri.c_str());
#endif
            ...
            {
                MONITORING_LOG2(req.from_tag, "app", app_name.c_str(), "dir", "out");
            }
#endif

            if (AmConfig.log_sessions) {
                INFO("Starting UAC session %s app %s", req.from_tag.c_str(), app_name.c_str());
            }
...
void AmSessionContainer::startSessionUAS(AmSipRequest &req)
{
    try {
        ...
        if (session.get() != 0) {
            ...
            if (AmConfig.log_sessions) {
                INFO("Starting UAS session %s", local_tag.c_str());
            }
            ...
            MONITORING_LOG4(local_tag.c_str(), "dir", "in", "from", req.from.c_str(), "to", req.to.c_str(), "ruri",
                            req.r_uri.c_str());
...
bool AmSessionContainer::postEvent(const string &callid, const string &remote_tag, const string &via_branch,
                                   AmEvent *event)
{
...
AmSession *AmSessionContainer::createSession(const AmSipRequest &req, string &app_name, bool is_uac,
                                             AmArg *session_params)
{
    ...
    if (AmConfig.session_limit && AmConfig.session_limit <= AmSession::session_num) {

        DBG("session_limit %d reached. Not creating session.", AmConfig.session_limit);
        if (session_limit_reject)
            session_limit_reject->inc(1);

        AmSipDialog::reply_error(req, AmConfig.session_limit_err_code, AmConfig.session_limit_err_reason);
        return NULL;
    }

    if (check_and_add_cps()) {
        AmSipDialog::reply_error(req, AmConfig.cps_limit_err_code, AmConfig.cps_limit_err_reason);
        return NULL;
    }
    ...
    if (!session_factory) {
        if (!is_uac) {
            ERROR("src_ip: %s callid: %s ruri: %s - no session factory for application '%s'", req.remote_ip.c_str(),
                  req.callid.c_str(), req.r_uri.c_str(), app_name.c_str());
...
AmSessionContainer::AddSessionStatus AmSessionContainer::addSession(const string &callid, const string &remote_tag,
                                                                    ...
                                                                    AmSession *session)
{
    ...
    if (AmEventDispatcher::instance()->addEventQueue(local_tag, (AmEventQueue *)session, callid, remote_tag,
                                                     via_branch))
    {
        return Inserted;
    }
...
AmSessionContainer::AddSessionStatus AmSessionContainer::addSession(const string &local_tag, AmSession *session)
{
    if (_container_closed.get())
        return ShutDown;

    if (AmEventDispatcher::instance()->addEventQueue(local_tag, (AmEventQueue *)session)) {
        return Inserted;
    }
