...
TEST(UriParser, headers) {
    AmUriParser p;
    size_t end;
    ASSERT_TRUE(p.parse_contact("\"Mr. Watson\" <mailto:watson@bell-telephone.com?Replaces:%20lkancskjd%3Bto-tag=3123141ab%3Bfrom-tag=kjhkjcsd> ;q=0.1", 0, end));
    ASSERT_TRUE(p.display_name=="Mr. Watson");
    ASSERT_TRUE(p.uri_user=="watson");
    ASSERT_TRUE(p.uri_host=="bell-telephone.com");
...
TEST(UriParser, params_dname7) {
    AmUriParser p;
    size_t end;
    ASSERT_TRUE(p.parse_contact("<sip:alice@atlanta.com>;+g.3gpp.icsi-ref=\"urn%3Aurn-7%3A3gpp-service.ims.icsi.mmtel\";video;mobility=\"mobile\"", 0, end));
