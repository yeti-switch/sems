...
static void default_message_handler(int level, const char *text, void *user);

static message_handler_func_t __span_message = &default_message_handler;
static error_handler_func_t __span_error = NULL;
static void *__span_message_user_data = NULL;
static void *__span_error_user_data = NULL;
...
SPAN_DECLARE(int) span_log(logging_state_t *s, int level, const char *format, ...)
{
    ...
    if (span_log_test(s, level))
    {
        ...
        if (s->span_error  &&  level == SPAN_LOG_ERROR)
            s->span_error(msg, s->error_user_data);
        else if (__span_error  &&  level == SPAN_LOG_ERROR)
            __span_error(msg, s->error_user_data);
        else if (s->span_message)
            s->span_message(level, msg, s->message_user_data);
        else if (__span_message)
            __span_message(level, msg, s->message_user_data);
        /*endif*/
        va_end(arg_ptr);
        return  1;
    }
...
SPAN_DECLARE(void) span_log_set_message_handler(logging_state_t *s, message_handler_func_t func, void *user)
{
    s->span_message = func;
    s->message_user_data = user;
}
/*- End of function --------------------------------------------------------*/

SPAN_DECLARE(void) span_log_set_error_handler(logging_state_t *s, error_handler_func_t func, void *user)
{
    s->span_error = func;
    s->error_user_data = user;
}
/*- End of function --------------------------------------------------------*/

SPAN_DECLARE(void) span_set_message_handler(message_handler_func_t func, void *user)
{
    __span_message = func;
    __span_message_user_data = user;
}
/*- End of function --------------------------------------------------------*/

SPAN_DECLARE(void) span_set_error_handler(error_handler_func_t func, void *user)
{
    __span_error = func;
    __span_error_user_data = user;
}
/*- End of function --------------------------------------------------------*/
...
SPAN_DECLARE(logging_state_t *) span_log_init(logging_state_t *s, int level, const char *tag)
{
    ...
    s->message_user_data = __span_message_user_data;
    s->error_user_data = __span_error_user_data;
    s->level = level;
    s->tag = tag;
    s->protocol = NULL;
